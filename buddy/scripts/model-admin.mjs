...
用法：
  pnpm --dir nimi-mods/buddy run model:validate -- --model assets/models/hiyori/hiyori_pro_t11.model3.json
  pnpm --dir nimi-mods/buddy run model:add -- --id hiyori --label "日和（Hiyori）" --model assets/models/hiyori/hiyori_pro_t11.model3.json
...
function runAdd(flags) {
  ...
  info('完成。下一步建议运行：pnpm --dir nimi-mods/buddy run verify');
}
