import { NARRATIVE_ACTION_HINT_BY_REASON_CODE, NARRATIVE_ENGINE_DATA_API_AUDIT_APPEND, NARRATIVE_ENGINE_DATA_API_CONTEXT_RESOLVE, NARRATIVE_ENGINE_DATA_API_PROJECTION_RENDER_INPUT, NARRATIVE_ENGINE_DATA_API_SPINE_APPEND, NARRATIVE_ENGINE_DATA_API_TURN_BY_ID, NARRATIVE_ENGINE_DATA_API_TURN_LATEST, NARRATIVE_ENGINE_DATA_API_TURN_RESULT_UPSERT, NARRATIVE_ENGINE_DATA_API_TURN_WINDOW, NARRATIVE_REASON_CODES, NARRATIVE_SPINE_EVENT_TYPES, NARRATIVE_VISIBILITY_VALUES, } from '../contracts.js';
import { createNarrativeFlowId, emitNarrativeLog } from '../logging.js';
import { processNarrativeTurn } from '../pipeline/process-turn.js';
import { buildNarrativeRenderInput, collectProjectionSourceEventIds, } from '../projection/render-input.js';
import { NarrativeAuditAppendQuerySchema, NarrativeContextResolveQuerySchema, NarrativeProjectionQuerySchema, NarrativeRunCancelQuerySchema, NarrativeRunReplayQuerySchema, NarrativeTurnByIdQuerySchema, NarrativeTurnLatestQuerySchema, NarrativeTurnWindowQuerySchema, } from '../schemas.js';
import { appendNarrativeRunEvent, appendNarrativeSpine, getLatestNarrativeTurn, getNarrativeProjectionByTurnId, getNarrativeRunNextSeq, getNarrativeSpineByStoryId, getNarrativeTurnById, getNarrativeTurnWindow, replayNarrativeRunEvents, resolveNarrativeContext, upsertNarrativeContext, } from '../store/repository.js';
import type { NarrativeAiClient, NarrativeRunEvent, NarrativeSpineEvent, NarrativeTurnRecord, } from '../types.js';
...
export async function registerNarrativeDataCapabilities(input: {
    ...
    await hookClient.data.register({
        ...
        handler: async (query) => {
            ...
            const existingIds = new Set(getNarrativeSpineByStoryId(storyId).map((event) => toString(event.id)));
