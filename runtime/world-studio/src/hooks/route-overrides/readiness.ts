...
export function evaluateRouteBindingReadiness(binding: RuntimeRouteBinding | null | undefined, routeOptions: RuntimeRouteOptionsSnapshot | null): RouteReadinessResult {
    if (!binding) {
        return {
            ready: false,
...
export function areDistillRoutesReady(bindings: Pick<DistillRouteBindingMap, 'coarse' | 'fine'>, routeOptions: RuntimeRouteOptionsSnapshot | null): boolean {
    return (evaluateRouteBindingReadiness(bindings.coarse, routeOptions).ready
        && evaluateRouteBindingReadiness(bindings.fine, routeOptions).ready);
}
function mapActionHintForEmbedding(_actionHint: RouteReadinessResult['actionHint']): EmbeddingReadinessResult['actionHint'] {
...
export function evaluateEmbeddingReadiness(input: {
    ...
    const routeReadiness = evaluateRouteBindingReadiness(input.runtimeDefaultRouteBinding, input.routeOptions);
