...
export function useWorldStudioRouteBindingDerived(input: {
    ...
    const effectiveCoarseRouteBinding = useMemo(() => input.bindingMap.coarse || input.runtimeDefaultRouteBinding || null, [input.bindingMap.coarse, input.runtimeDefaultRouteBinding]);
    const effectiveFineRouteBinding = useMemo(() => input.bindingMap.fine || input.runtimeDefaultRouteBinding || null, [input.bindingMap.fine, input.runtimeDefaultRouteBinding]);
    const effectiveCoarseRouteSummary = useMemo(() => formatRouteBindingSummary(effectiveCoarseRouteBinding), [effectiveCoarseRouteBinding]);
    const effectiveFineRouteSummary = useMemo(() => formatRouteBindingSummary(effectiveFineRouteBinding), [effectiveFineRouteBinding]);
    ...
    const activeCoarseRouteConnector = useMemo(() => input.routeOptions?.connectors.find((item) => item.id === activeCoarseRouteConnectorId)
        || input.routeOptions?.connectors[0]
        || null, [activeCoarseRouteConnectorId, input.routeOptions?.connectors]);
    const activeFineRouteConnector = useMemo(() => input.routeOptions?.connectors.find((item) => item.id === activeFineRouteConnectorId)
        || input.routeOptions?.connectors[0]
        || null, [activeFineRouteConnectorId, input.routeOptions?.connectors]);
    ...
    const coarseRouteReadiness = useMemo(() => evaluateRouteBindingReadiness(effectiveCoarseRouteBinding, input.routeOptions), [effectiveCoarseRouteBinding, input.routeOptions]);
    const fineRouteReadiness = useMemo(() => evaluateRouteBindingReadiness(effectiveFineRouteBinding, input.routeOptions), [effectiveFineRouteBinding, input.routeOptions]);
    ...
    const routeConfigActionHint = !coarseRouteReadiness.ready
        ...
        : !fineRouteReadiness.ready
            ...
            : 'none';
    const embeddingReadiness = useMemo(() => evaluateEmbeddingReadiness({
