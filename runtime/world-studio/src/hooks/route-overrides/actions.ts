...
import { normalizeRuntimeRouteSource, type RuntimeRouteBinding, type RuntimeRouteOptionsSnapshot, type RuntimeRouteSource } from "@nimiplatform/sdk/mod";
...
export function useWorldStudioRouteBindingActions(input: {
    ...
    const resolveEffectiveRouteBindings = useCallback((options: ResolveRouteBindingsInput) => {
        ...
        if (options.mode !== 'failed' || !options.retryWithFineRoute)
            return normalized;
