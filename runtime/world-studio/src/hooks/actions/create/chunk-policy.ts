...
import { type RuntimeRouteBinding, type RuntimeRouteModelProfile, type RuntimeRouteOptionsSnapshot } from "@nimiplatform/sdk/mod";
...
function inferTemplateContextTokens(model: string): number | undefined {
    ...
    if (normalized.startsWith('claude-') || tail.startsWith('claude-'))
        return 200000;
    if (normalized.startsWith('gpt-4') || tail.startsWith('gpt-4'))
        return 128000;
    if (normalized.startsWith('o3') || tail.startsWith('o3'))
        return 128000;
    if (normalized.startsWith('llama') || tail.startsWith('llama'))
        return 8192;
    if (normalized.startsWith('qwen') || tail.startsWith('qwen'))
        return 32768;
    if (normalized.startsWith('deepseek') || tail.startsWith('deepseek'))
        return 64000;
    return undefined;
}
...
function resolveBindingModelProfiles(binding: RuntimeRouteBinding | null, routeOptions: RuntimeRouteOptionsSnapshot | null): RuntimeRouteModelProfile[] {
    if (!binding || !routeOptions)
        return [];
...
function resolveBindingContext(input: {
    ...
    contextSource: ChunkPolicyContextSource;
} {
...
export function resolveAdaptiveChunkPolicy(input: {
    ...
    const effectiveContextTokens = (() => {
        ...
        if (typeof coarseContext === 'number')
            return coarseContext;
        if (typeof fineContext === 'number')
            return fineContext;
    ...
    const contextSource = (() => {
        ...
        if (typeof coarseContext === 'number')
            return coarse.contextSource;
        if (typeof fineContext === 'number')
            return fine.contextSource;
