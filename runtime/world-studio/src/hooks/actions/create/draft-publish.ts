...
function normalizeAgentRules(value: unknown): {
...
export async function saveWorldDraft(input: WorldStudioCreateActionsInput, _options?: DraftTaskOptions): Promise<void> {
    ...
    try {
        ...
        input.taskController.completeTask(started.taskId, worldStudioMessage('task.draftSaved', 'Draft {{draftId}} saved', { draftId }));
        emitWorldStudioLog({
            level: 'info',
...
export async function publishWorldDraft(input: WorldStudioCreateActionsInput, _options?: DraftTaskOptions): Promise<void> {
    ...
    try {
        ...
        const syncCharacters = toUniqueStringArray(input.snapshot.agentSync.selectedCharacterIds);
        ...
        if (syncCharacters.length > 0) {
            const characterByName = new Map(input.snapshot.knowledgeGraph.characters
            ...
            for (const createdItem of createdItems) {
                ...
                if (!Number.isFinite(index) || !agentId)
                    continue;
            ...
            const failedSuffix = failedCount > 0
                ...
                : '';
            syncNotice = worldStudioMessage('notice.agentSyncSummary', ' Agent sync: {{createdCount}}/{{totalCount}} created{{failedSuffix}}.', {
                createdCount,
        ...
        input.taskController.completeTask(started.taskId, worldStudioMessage('task.worldPublished', 'World published: {{worldId}}', { worldId }));
        ...
        if (isBackend500) {
            input.setNotice(worldStudioMessage('notice.publishBackendFailedDraftSaved', 'Publish request reached backend but failed (HTTP 500). Draft {{draftId}} is still saved.', { draftId: input.selectedDraftId || '' }));
        }
    }
}
