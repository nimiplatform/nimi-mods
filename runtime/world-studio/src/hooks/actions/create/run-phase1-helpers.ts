...
import { toFailedChunkIndices, toTerminalChunkTaskMap, } from '../../../services/event-graph-map.js';
...
export function resolveRetryChunks(input: WorldStudioCreateActionsInput, allChunks: string[], mode: 'all' | 'failed', forcedRetryErrorCode?: string | null): {
    chunksToRun: string[];
    ...
    const chunkTasks = input.phase1?.chunkTasks
        ...
        || [];
    const failedChunkIndices = toFailedChunkIndices(chunkTasks, allChunks.length, input.retryScope, activeRetryErrorCode);
    if (failedChunkIndices.length === 0) {
        if (activeRetryErrorCode) {
            input.setNotice(worldStudioMessage('notice.noFailedChunksMatchedCode', 'No failed chunks matched code={{code}} under scope={{scope}}.', {
                code: activeRetryErrorCode,
                scope: input.retryScope,
            }));
        }
        else {
            input.setNotice(worldStudioMessage('notice.noFailedChunksMatchedScope', 'No failed chunks matched retry scope: {{scope}}.', { scope: input.retryScope }));
        }
        return { chunksToRun: [] };
    }
    ...
    if (chunksToRun.length === 0) {
        input.setNotice(worldStudioMessage('notice.noFailedChunksToRetry', 'No failed chunks to retry.'));
        return { chunksToRun: [] };
    }
    return {
        chunksToRun,
...
export function mergeRetryPhase1Result(input: WorldStudioCreateActionsInput, allChunks: string[], chunksToRun: string[], mode: 'all' | 'failed', result: Phase1Result): Phase1Result {
    if (mode !== 'failed')
        return result;
    ...
    const mergedFinalDraftAccumulator = mergeFinalDraftAccumulator(basePhase1.finalDraftAccumulator || createEmptyFinalDraftAccumulator(), result.finalDraftAccumulator || createEmptyFinalDraftAccumulator());
    return {
        ...result,
...
export function buildParseJobStartState(chunkPolicy?: WorldStudioParseJobState['chunkPolicy']): WorldStudioParseJobState {
...
export async function resolveCreatePhase1RouteBindings(input: WorldStudioCreateActionsInput, mode: 'all' | 'failed'): Promise<{
