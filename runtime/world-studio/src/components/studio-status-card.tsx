...
export function StudioStatusCard(props: StudioStatusCardProps) {
    ...
    const toTaskStatusLabel = (status: WorldStudioTaskRecord['status']): string => {
        if (status === 'RUNNING')
            return t('studioStatus.taskRunning');
        if (status === 'PAUSE_REQUESTED')
            return t('studioStatus.taskPauseRequested');
        if (status === 'PAUSED')
            return t('studioStatus.taskPaused');
        if (status === 'CANCEL_REQUESTED')
            return t('studioStatus.taskCancelRequested');
        if (status === 'CANCELED')
            return t('studioStatus.taskCanceled');
        if (status === 'FAILED')
            return t('studioStatus.taskFailed');
    ...
    return (<section className={embedded ? '' : 'ui-sync-card ui-sync-card-inset p-3'}>
      ...
      {props.activeTask ? (<div className="ui-sync-toolbar mt-2 px-2 py-1.5">
          ...
          {props.activeTask.message ? (<p className="mt-1 text-xs text-gray-600">{props.activeTask.message}</p>) : null}
          <div className="mt-2 flex flex-wrap gap-2">
            {props.activeTask.canPause && props.onPauseTask ? (<button type="button" className="ui-sync-btn ui-sync-btn-secondary rounded-md border border-gray-300 bg-white px-2.5 py-1 text-xs font-semibold text-gray-700" onClick={() => { props.onPauseTask?.(); }}>
                {t('studioStatus.pause')}
              </button>) : null}
            {props.activeTask.canResume && props.onResumeTask ? (<button type="button" className="ui-sync-btn ui-sync-btn-secondary rounded-md border border-gray-300 bg-white px-2.5 py-1 text-xs font-semibold text-gray-700" onClick={() => { props.onResumeTask?.(); }}>
                {t('studioStatus.resume')}
              </button>) : null}
            {props.activeTask.canCancel && props.onCancelTask ? (<button type="button" className="ui-sync-btn rounded-md border border-red-200 bg-white px-2.5 py-1 text-xs font-semibold text-red-700" onClick={() => { props.onCancelTask?.(); }}>
                {t('studioStatus.cancel')}
              </button>) : null}
          </div>
          {props.activeTask.atomic ? (<p className="mt-2 text-[11px] text-gray-500">{t('studioStatus.atomicHint')}</p>) : null}
          {props.expertMode && props.activeTask.checkpoint ? (<p className="mt-2 text-[11px] text-gray-500">
              checkpoint v{props.activeTask.checkpoint.checkpointVersion}
              {' · '}
              {props.activeTask.checkpoint.step}
            </p>) : null}
        </div>) : (<p className="mt-2 text-xs text-gray-600">{t('studioStatus.noActiveTask')}</p>)}
      ...
      <div className="ui-sync-soft-card mt-2 px-2 py-1.5 text-xs text-gray-600">
        ...
        {typeof props.futureEventsCount === 'number' ? (<p>{t('studioStatus.futureEvents')}: {props.futureEventsCount}</p>) : null}
      </div>

      {props.expertMode ? (<div className="ui-sync-toolbar mt-2 px-2 py-1.5 text-xs text-slate-700">
          ...
          {props.parsePhase ? (<p>
              ...
              {typeof props.parseChunkProcessed === 'number' && typeof props.parseChunkTotal === 'number' ? (` · ${props.parseChunkProcessed}/${props.parseChunkTotal} ${t('studioStatus.chunks')}`) : ''}
            </p>) : null}
          ...
          {props.terminalTopFailure ? (<p>{t('studioStatus.topFailure')}: {props.terminalTopFailure.code} × {props.terminalTopFailure.count}</p>) : null}
        </div>) : null}

      {props.recentTasks.length > 0 ? (<details className="ui-sync-code-panel mt-2 px-2 py-1.5">
          <summary className="cursor-pointer text-xs font-semibold text-gray-700">{t('studioStatus.recentTasks')}</summary>
          <div className="mt-2 space-y-1 text-xs text-gray-600">
            {props.recentTasks.slice(0, 5).map((task) => (<p key={task.id}>
                {task.label}
                {' · '}
      ...
      {props.conflictReloadSummary ? (<div className="ui-sync-alert ui-sync-alert-info mt-2 px-2 py-1.5 text-xs text-sky-700">
          <p className="font-semibold">{t('studioStatus.recentReloadSummary')}</p>
          <p className="mt-1">{props.conflictReloadSummary}</p>
        </div>) : null}
      {props.notice ? (<p className="ui-sync-alert ui-sync-alert-success mt-2 px-2 py-1 text-xs text-emerald-700">{props.notice}</p>) : null}
      {mappedError.summary ? (<div className="ui-sync-alert ui-sync-alert-danger mt-2 px-2 py-1 text-xs text-red-700">
          <p>{mappedError.summary}</p>
          {mappedError.detail ? (<details className="mt-1">
      ...
      {(props.onResetDraft || props.onReload) ? (<div className="mt-3 flex gap-2">
          {props.onResetDraft ? (<button type="button" className="ui-sync-btn ui-sync-btn-secondary rounded-md border border-gray-300 px-2.5 py-1 text-xs font-semibold text-gray-700" onClick={props.onResetDraft}>
              {t('studioStatus.resetDraft')}
            </button>) : null}
          {props.onReload ? (<button type="button" className="ui-sync-btn ui-sync-btn-secondary rounded-md border border-gray-300 px-2.5 py-1 text-xs font-semibold text-gray-700" onClick={props.onReload}>
