...
export function RouteCapabilityControls(props: RouteCapabilityControlsProps) {
    ...
    return (<div className="space-y-2">
      ...
      <div>
        ...
        <select value={props.activeSource} onChange={(event) => props.onRouteSourceChange(props.profile, normalizeRuntimeRouteSource(event.target.value))} className="h-8 w-full rounded-md border border-gray-300 bg-white px-2 text-xs text-gray-900">
      ...
      <div>
        ...
        <select value={props.activeConnectorId} disabled={props.activeSource !== 'cloud'} onChange={(event) => props.onRouteConnectorChange(props.profile, event.target.value)} className="h-8 w-full rounded-md border border-gray-300 bg-white px-2 text-xs text-gray-900 disabled:bg-gray-100 disabled:text-gray-400">
          <option value="">--</option>
      ...
      <div>
        ...
        }} placeholder={t('routeCapabilityControls.modelPlaceholder')} className="h-8 w-full rounded-md border border-gray-300 bg-white px-2 text-xs text-gray-900 outline-none transition-colors focus:border-brand-500 focus:ring-1 focus:ring-brand-500 disabled:bg-gray-100 disabled:text-gray-400"/>
        ...
        {props.activeSource === 'local' && props.modelOptions.length === 0 ? (<div className="mt-2 rounded-md border border-amber-200 bg-amber-50 p-2 space-y-2">
            <p className="text-[11px] text-amber-800">{t('routeCapabilityControls.emptyLocalHint')}</p>
            <div className="flex gap-2">
              <button type="button" className="ui-sync-btn ui-sync-btn-secondary rounded-md border border-amber-300 bg-white px-2 py-1 text-[11px] font-semibold text-amber-800" onClick={() => setActiveTab('runtime')}>
                {t('routeCapabilityControls.goInstallModels')}
              </button>
              <button type="button" className="ui-sync-btn ui-sync-btn-secondary rounded-md border border-amber-300 bg-white px-2 py-1 text-[11px] font-semibold text-amber-800" onClick={() => props.onRouteSourceChange(props.profile, 'cloud')}>
                {t('routeCapabilityControls.switchToCloud')}
              </button>
            </div>
          </div>) : null}
      </div>
      {!readiness.ready ? (<p className="text-[11px] text-rose-700">
          {readiness.message} ({readiness.reasonCode})
        </p>) : null}
      <button type="button" className="ui-sync-btn ui-sync-btn-secondary w-full rounded-md border border-gray-300 bg-white px-2.5 py-1 text-xs font-semibold text-gray-700" onClick={() => props.onClearRouteOverride(props.profile)}>
