      ...
      <div className="min-h-0 flex-1 overflow-y-auto rounded-[10px] border border-gray-200 bg-white">
        <section className="px-3 py-3 text-xs">
          ...
          {openSection === 'actions' ? (<div className="mt-3">
              <MaintenanceActionsCard selectedWorldId={props.selectedWorldId} editorSnapshotVersion={props.editorSnapshotVersion || '-'} eventSyncMode={props.eventSyncMode} working={props.working} hasMaintenanceConflict={props.hasMaintenanceConflict} remoteMaintenanceSnapshotVersion={props.remoteMaintenanceSnapshotVersion || '-'} onSaveMaintenance={(payload) => { props.onSaveMaintenance(payload); }} onSyncEvents={(payload) => { props.onSyncEvents(payload); }} onSyncLorebooks={props.onSyncLorebooks} onRefreshResources={props.onRefreshResources} onReloadRemoteForConflict={props.onReloadRemoteForConflict} onAdoptRemoteSnapshot={props.onAdoptRemoteSnapshot} embedded showTitle={false}/>
        ...
        <section className="border-t border-gray-200 px-3 py-3 text-xs">
          ...
          {openSection === 'status' ? (<div className="mt-3">
              <StudioStatusCard mode="MAINTAIN" activeTask={props.activeTask} recentTasks={props.recentTasks} expertMode={props.expertMode} coarseRouteSummary={props.effectiveCoarseRouteSummary} fineRouteSummary={props.effectiveFineRouteSummary} primaryEventCount={props.primaryEventCount} secondaryEventCount={props.secondaryEventCount} missingPrimaryEvidenceCount={props.missingPrimaryEvidenceCount} eventCharacterCoverage={props.eventCharacterCoverage} eventLocationCoverage={props.eventLocationCoverage} terminalChunkSuccess={props.terminalChunkSuccess} terminalChunkTotal={props.terminalChunkTotal || 0} terminalChunkFailed={props.terminalChunkFailed} terminalTopFailure={props.terminalTopFailure} conflictReloadSummary={props.conflictReloadSummary} notice={props.notice} error={props.error} onPauseTask={props.onPauseTask} onResumeTask={props.onResumeTask} onCancelTask={props.onCancelTask} embedded showTitle={false}/>
            </div>) : null}
        </section>
      </div>
    </div>);
