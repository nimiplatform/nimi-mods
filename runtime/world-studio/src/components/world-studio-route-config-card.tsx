...
export function WorldStudioRouteConfigCard(props: {
    ...
    return (<div>
      ...
      <div className="mt-3 space-y-2">
        <RouteCapabilityControls profile="coarse" title={t('routeConfig.coarseTitle')} activeSource={props.activeCoarseRouteSource} activeConnectorId={props.activeCoarseRouteConnectorId} binding={props.effectiveCoarseRouteBinding} connectors={props.routeConnectors} modelOptions={props.coarseRouteModelOptions} readiness={props.coarseRouteReadiness} onRouteSourceChange={props.onRouteSourceChange} onRouteConnectorChange={props.onRouteConnectorChange} onRouteModelChange={props.onRouteModelChange} onClearRouteOverride={props.onClearRouteBinding} onOpenRuntimeSetup={props.onOpenRuntimeSetup}/>
        <RouteCapabilityControls profile="fine" title={t('routeConfig.fineTitle')} activeSource={props.activeFineRouteSource} activeConnectorId={props.activeFineRouteConnectorId} binding={props.effectiveFineRouteBinding} connectors={props.routeConnectors} modelOptions={props.fineRouteModelOptions} readiness={props.fineRouteReadiness} onRouteSourceChange={props.onRouteSourceChange} onRouteConnectorChange={props.onRouteConnectorChange} onRouteModelChange={props.onRouteModelChange} onClearRouteOverride={props.onClearRouteBinding} onOpenRuntimeSetup={props.onOpenRuntimeSetup}/>
      </div>

      <button type="button" className="ui-sync-btn ui-sync-btn-secondary mt-2 w-full rounded-md border border-gray-300 bg-white px-2.5 py-1 text-xs font-semibold text-gray-700" onClick={() => props.onClearRouteBinding('all')}>
      ...
      <div className="mt-3 rounded-md border border-gray-200 bg-gray-50 p-2">
        ...
        {props.embeddingIndexLastBuiltAt ? (<p className="mt-1 text-[11px] text-gray-500">
            {t('routeConfig.embeddingLastBuiltAt')}: {props.embeddingIndexLastBuiltAt}
          </p>) : null}
        {props.embeddingIndexErrorMessage ? (<p className="mt-1 text-[11px] text-rose-700">{props.embeddingIndexErrorMessage}</p>) : null}
        <div className="mt-2 flex gap-2">
          <button type="button" className="ui-sync-btn ui-sync-btn-secondary rounded-md border border-gray-300 bg-white px-2.5 py-1 text-[11px] font-semibold text-gray-700" onClick={() => {
            void props.onRebuildEmbeddingIndex();
        }}>
            {t('routeConfig.rebuildEmbeddingIndex')}
          </button>
          {!props.embeddingReadiness.healthy && props.embeddingReadiness.actionHint !== 'none' && props.onOpenRuntimeSetup ? (<button type="button" className="ui-sync-btn ui-sync-btn-secondary rounded-md border border-gray-300 bg-white px-2.5 py-1 text-[11px] font-semibold text-gray-700" onClick={() => props.onOpenRuntimeSetup?.()}>
      ...
      <div className="mt-3 border-t border-gray-200 pt-3">
        ...
        <button type="button" className={`ui-sync-btn mt-2 rounded-md border px-3 py-1.5 text-xs font-semibold ${props.expertMode
            ? 'ui-sync-btn-selected border-brand-200 bg-brand-50 text-brand-700'
            : 'ui-sync-btn-secondary border-gray-300 bg-white text-gray-700'}`} onClick={() => props.onSetExpertMode(!props.expertMode)}>
