...
export function NoAccessPanel(props: NoAccessPanelProps) {
    ...
    return (<div className="ui-sync-root h-full overflow-auto p-4">
      ...
      <div className="mx-auto max-w-xl rounded-[24px] border border-amber-200 bg-amber-50/95 p-5 text-sm text-amber-900 shadow-[0_18px_40px_rgba(217,119,6,0.08)]">
        ...
        <button type="button" onClick={props.onRetry} className="ui-sync-btn mt-4 rounded-md border border-amber-400 bg-white px-3 py-1.5 text-xs font-semibold text-amber-800">
          {t('noAccess.retry')}
        </button>
      </div>
    </div>);
