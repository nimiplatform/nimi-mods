...
import type { ChunkTaskResult, QualityGateResult, WorldStudioCharacterProfile, WorldStudioKnowledgeGraphDraft, } from '../../contracts.js';
...
function CountCard(props: {
    ...
    return (<div className="ui-sync-metric-card px-3 py-2">
...
function gateTagStyle(status: QualityGateResult['status']): string {
    if (status === 'PASS')
        return 'ui-sync-alert ui-sync-alert-success border-emerald-200 bg-emerald-50 text-emerald-700';
    if (status === 'WARN')
        return 'ui-sync-alert ui-sync-alert-warning border-amber-200 bg-amber-50 text-amber-700';
...
function gateLabel(status: QualityGateResult['status']): string {
    if (status === 'PASS')
        return 'phase1.gate.pass';
    if (status === 'WARN')
        return 'phase1.gate.warn';
    return 'phase1.gate.block';
}
function renderCharacterProfiles(profiles: WorldStudioCharacterProfile[], t: (key: string, values?: Record<string, unknown>) => string) {
    ...
    return (<div className="mt-2 grid gap-2 md:grid-cols-2">
      {profiles.slice(0, 12).map((profile) => (<div key={profile.name} className="ui-sync-soft-card p-2.5">
          <p className="text-xs font-semibold text-gray-900">{profile.name}</p>
          {profile.aliases.length > 0 ? (<p className="mt-1 text-[11px] text-gray-600">{t('phase1.characterProfile.aliases')}: {profile.aliases.join(' / ')}</p>) : null}
...
export function Phase1Panel(props: Phase1PanelProps) {
    ...
    return (<section className="ui-sync-card ui-sync-card-inset p-4">
      ...
      {qualityGate ? (<div className={`mt-3 rounded-lg border p-3 ${gateTagStyle(qualityGate.status)}`}>
          ...
          {qualityGate.issues.length > 0 ? (<div className="mt-2 space-y-1">
              {qualityGate.issues.slice(0, 6).map((issue, index) => (<p key={`${issue.code}:${index}`} className="text-[11px]">
      ...
      <div className="ui-sync-soft-card mt-3 p-3">
        ...
        {narrativeArc ? (<div className="mt-2 space-y-1 text-xs text-gray-700">
          ...
          </div>) : (<p className="mt-1 text-xs text-gray-500">{t('phase1.narrativeArcEmpty')}</p>)}
      </div>
      ...
      {props.expertMode ? (<>
          {qualityGate ? (<div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-4">
