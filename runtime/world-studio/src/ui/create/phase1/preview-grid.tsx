...
function PreviewList(props: {
    ...
    return (<div className="ui-sync-card p-3">
      <h4 className="text-xs font-semibold text-gray-900">{props.title}</h4>
      <div className="mt-2 space-y-2">
        {props.items.length === 0 ? (<p className="text-xs text-gray-500">{props.emptyText}</p>) : (props.items.slice(0, 8).map((item, index) => {
            if (!item || typeof item !== 'object')
                return null;
            return (<div key={`${props.title}-${index}`} className="ui-sync-soft-card px-2 py-1.5">
...
export function Phase1PreviewGrid(props: {
    ...
    return (<div className="mt-3 grid gap-3 xl:grid-cols-2">
      <PreviewList title={t('phase1.previewEventGraph')} items={primaryEvents} emptyText={t('phase1.previewPrimaryEventsEmpty')} renderItem={(item, index) => {
            ...
            return (<>
              ...
              <div className="mt-1 flex flex-wrap items-center gap-2">
                <span className={`rounded px-1.5 py-0.5 text-[10px] font-semibold ${evidenceCount > 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
      ...
      <PreviewList title={t('phase1.previewPrimaryEvents')} items={primaryEvents} emptyText={t('phase1.previewPrimaryEventsEmpty')} renderItem={(item, index) => (<>
            <p className="text-xs font-medium text-gray-900">{String(item.title || t('phase1.previewPrimaryEventFallback', { index: index + 1 }))}</p>
            <p className="mt-0.5 text-[11px] text-gray-600">{String(item.summary || item.cause || '')}</p>
      ...
      <PreviewList title={t('phase1.previewSecondaryEvents')} items={secondaryEvents} emptyText={t('phase1.previewSecondaryEventsEmpty')} renderItem={(item, index) => (<>
      ...
      <PreviewList title={t('phase1.previewTimeline')} items={props.graph.timeline} emptyText={t('phase1.previewTimelineEmpty')} renderItem={(item, index) => (<>
      ...
      <PreviewList title={t('phase1.previewCharacters')} items={props.graph.characters} emptyText={t('phase1.previewCharactersEmpty')} renderItem={(item, index) => (<>
