...
export function EventGraphEditorHeader(props: EventGraphEditorHeaderProps) {
    ...
    return (<>
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {!props.readonly ? (<div className="flex items-center gap-2">
            <button type="button" className="ui-sync-btn ui-sync-btn-secondary rounded border border-gray-300 bg-white px-2 py-1 text-xs font-semibold text-gray-700" onClick={props.onAddPrimary}>
              {t('eventGraphEditor.addPrimaryEvent')}
            </button>
            <button type="button" className="ui-sync-btn ui-sync-btn-secondary rounded border border-gray-300 bg-white px-2 py-1 text-xs font-semibold text-gray-700 disabled:opacity-50" disabled={!props.canAddSecondary} onClick={props.onAddSecondary}>
      ...
      {!props.readonly ? (<div className="ui-sync-toolbar mt-3 p-2.5">
          <p className="text-xs font-semibold text-gray-700">{t('eventGraphEditor.batchRepair')}</p>
          <div className="mt-2 flex flex-wrap gap-2">
            <button type="button" className="ui-sync-btn ui-sync-btn-secondary rounded border border-gray-300 bg-white px-2 py-1 text-[11px] font-semibold text-gray-700 disabled:opacity-50" onClick={props.onApplyEvidenceTemplate} disabled={props.missingEvidencePrimaryCount === 0}>
              {t('eventGraphEditor.fillMissingEvidence')}
            </button>
            <button type="button" className="ui-sync-btn ui-sync-btn-secondary rounded border border-gray-300 bg-white px-2 py-1 text-[11px] font-semibold text-gray-700 disabled:opacity-50" onClick={props.onRepairSecondaryParents} disabled={!props.canRepairSecondaryParents}>
              {t('eventGraphEditor.repairSecondaryParents')}
            </button>
            <button type="button" className="ui-sync-btn ui-sync-btn-secondary rounded border border-gray-300 bg-white px-2 py-1 text-[11px] font-semibold text-gray-700 disabled:opacity-50" onClick={props.onPruneInvalidDependencies} disabled={!props.canPruneInvalidDependencies}>
              {t('eventGraphEditor.pruneInvalidDependencies')}
            </button>
          </div>
        </div>) : null}
      <div className={`ui-sync-alert mt-3 p-2.5 ${props.diagnosticsHasIssues
            ? 'ui-sync-alert-warning border-amber-200 bg-amber-50'
            : 'ui-sync-alert-success border-emerald-200 bg-emerald-50'}`}>
        <p className={`text-xs font-semibold ${props.diagnosticsHasIssues ? 'text-amber-700' : 'text-emerald-700'}`}>
          {t('eventGraphEditor.diagnostics')} {props.diagnosticsHasIssues ? `· ${t('eventGraphEditor.actionRequired')}` : `· ${t('eventGraphEditor.healthy')}`}
        </p>
        {props.diagnosticsHasIssues ? (<div className="mt-1 space-y-1">
            {props.diagnosticsIssueLines.map((line) => (<p key={`event-graph-diag-${line}`} className="text-[11px] text-amber-700">
                {line}
              </p>))}
            {props.missingDependencySample ? (<p className="text-[11px] text-amber-700">
          ...
          </div>) : (<p className="mt-1 text-[11px] text-emerald-700">
