...
export function RelationEditor(props: RelationEditorProps) {
    ...
    return (<label className="block text-xs text-gray-700">
      <span className="mb-1 block font-medium text-gray-700">{props.label}</span>
      <input className="h-9 w-full rounded-md border border-gray-300 px-2 text-xs text-gray-900" value={text} placeholder={props.placeholder || t('shared.commaSeparated')} onChange={(event) => {
