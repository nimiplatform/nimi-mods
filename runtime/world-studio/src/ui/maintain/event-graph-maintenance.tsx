...
export function EventGraphMaintenance(props: EventGraphMaintenanceProps) {
    ...
    return (<div className="space-y-3">
      <EventGraphEditor title={t('eventGraphMaintenance.title')} events={props.events} onChange={props.onEventsChange} layout={props.layout} onLayoutChange={props.onLayoutChange}/>
      ...
      <section className="ui-sync-card ui-sync-card-inset p-4">
        ...
        <div className="grid gap-3 md:grid-cols-[220px_1fr]">
          <label className="text-xs text-gray-700">
            ...
            <select className="h-9 w-full rounded-md border border-gray-300 px-2 text-xs" value={props.syncMode} onChange={(event) => props.onSyncModeChange(event.target.value === 'replace' ? 'replace' : 'merge')}>
