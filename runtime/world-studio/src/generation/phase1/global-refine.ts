import { canonicalizeCharacterNames, normalizeZhCharacterName, } from '../../engine/character/normalize-zh.js';
import { isSyntheticEntityName } from '../../engine/errors.js';
import { computeTemporalOrder } from '../../services/temporal-order.js';
import type { CharacterPoint, CharacterRelationPoint, EventNodeDraft, WorldStudioCharacterProfile, WorldStudioKnowledgeGraphDraft, WorldStudioNarrativeArc, } from '../../engine/types.js';
...
function normalizeCharacterPoints(characters: CharacterPoint[], canonicalNames: string[], aliasMap: Record<string, string>): CharacterPoint[] {
    ...
    characters.forEach((item) => {
        ...
        byCanonical.set(canonicalName, {
            ...
            significance: clamp01(Math.max(Number(previous.significance) || 0, Number(record.significance) || 0.5), 0.5),
        });
    });
    canonicalNames.forEach((name) => {
        if (byCanonical.has(name))
            return;
...
function normalizeEventCharacters(events: EventNodeDraft[], aliasMap: Record<string, string>): EventNodeDraft[] {
    return events.map((event) => ({
        ...event,
        characterRefs: uniqueStrings((event.characterRefs || [])
            .map((name) => toCanonicalName(name, aliasMap))
            .filter(Boolean)),
    }));
}
function normalizeCharacterRelations(relations: CharacterRelationPoint[], aliasMap: Record<string, string>): CharacterRelationPoint[] {
...
export function runPhase1GlobalRefine(graph: WorldStudioKnowledgeGraphDraft): Phase1GlobalRefineResult {
