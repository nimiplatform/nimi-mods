import type { CharacterPoint, CharacterRelationPoint, ChunkExtraction, EventNodeDraft, LocationPoint, Phase1Character, Phase1Option, TimelinePoint, WorldStudioKnowledgeGraphDraft, } from './types.js';
import { deriveNeedsEvidence, normalizeEventHorizon, } from '../services/event-horizon.js';
import { asRecord, clamp01 } from "@nimiplatform/sdk/mod";
...
function mergeEventsByKey(events: EventNodeDraft[], keyOf: (event: EventNodeDraft, index: number) => string): EventNodeDraft[] {
...
function dedupeEventsBySemanticAlias(events: EventNodeDraft[], aliasOf: (event: EventNodeDraft) => string): EventNodeDraft[] {
...
function mergeEvents(extractions: ChunkExtraction[]): {
...
function collectCharactersFromEvents(events: EventNodeDraft[]): CharacterPoint[] {
    ...
    return uniqueBy(names.map((name) => ({
        name,
...
function collectTimelineFromEvents(events: EventNodeDraft[]): TimelinePoint[] {
    const labels = events
        ...
        .filter(Boolean);
    return uniqueBy(labels.map((label, index) => ({
        id: `timeline:${index + 1}`,
        label,
...
function mergeCharacters(primary: CharacterPoint[], eventFallback: CharacterPoint[], relationFallback: CharacterPoint[]): CharacterPoint[] {
    return uniqueBy([...primary, ...eventFallback, ...relationFallback], (item, index) => normalizeId(item.name || item.id || `char-${index + 1}`));
}
function mergeLocations(primary: LocationPoint[], eventFallback: LocationPoint[]): LocationPoint[] {
    return uniqueBy([...primary, ...eventFallback], (item, index) => normalizeId(item.name || item.id || `loc-${index + 1}`));
}
function mergeTimeline(primary: TimelinePoint[], eventFallback: TimelinePoint[]): TimelinePoint[] {
    return uniqueBy([...primary, ...eventFallback], (item, index) => normalizeId(item.id || item.label || `timeline-${index + 1}`));
}
export function mergeExtractions(extractions: ChunkExtraction[]): WorldStudioKnowledgeGraphDraft {
    const primaryTimeline = uniqueBy(extractions.flatMap((item) => item.timeline), (item, index) => normalizeId(asRecord(item).id || asRecord(item).label || `timeline-${index + 1}`));
    const primaryLocations = uniqueBy(extractions.flatMap((item) => item.locations), (item, index) => normalizeId(asRecord(item).name || asRecord(item).id || `loc-${index + 1}`));
    const primaryCharacters = uniqueBy(extractions.flatMap((item) => item.characters), (item, index) => normalizeId(asRecord(item).name || asRecord(item).id || `char-${index + 1}`));
    const characterRelations = uniqueBy(extractions.flatMap((item) => item.characterRelations), (item, index) => {
...
export function toCharacterCandidates(characters: Array<Record<string, unknown>>): Phase1Character[] {
    return uniqueBy(characters
        .map((item) => ({
        name: String(item.name || '').trim(),
        summary: String(item.summary || item.description || ''),
        significance: clamp01(item.significance, 0.5),
    }))
        .filter((item) => item.name.length > 0 && !isPlaceholderEntityName(item.name)), (item) => normalizeId(item.name)).slice(0, 24);
}
