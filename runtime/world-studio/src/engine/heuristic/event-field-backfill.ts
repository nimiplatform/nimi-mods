...
import type { ChunkExtraction, EventNodeDraft, WorldStudioKnowledgeGraphDraft, } from '../types.js';
...
const TIME_SNIPPET_PATTERN = /(?:公元前?\d{1,4}年|(?:18|19|20)\d{2}年?(?:\d{1,2}月(?:\d{1,2}[日号])?)?|(?:18|19|20)\d{2}(?:-\d{1,2}(?:-\d{1,2})?)?|(?:jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\.?\s+\d{1,2},?\s+(?:18|19|20)\d{2}|第[0-9一二三四五六七八九十百千〇零两]+(?:天|日|周|月|年)|[0-9一二三四五六七八九十百千〇零两]+(?:天|日|周|月|年)后|翌日|次日|当晚|同年|同月|当日|次年|隔日)/gi;
...
function extractMentionsFromScope(input: {
    ...
    for (const candidate of sortCandidates(input.candidates)) {
        if (!containsInText(input.eventScope, candidate))
            continue;
        if (!containsInText(input.source, candidate))
            continue;
...
function resolveFallbackParentEventId(input: {
    ...
    if (currentParent && input.primaryIdSet.has(currentParent))
        return currentParent;
    const validDependencyParent = uniqueStrings((Array.isArray(input.event.dependsOnEventIds) ? input.event.dependsOnEventIds : [])
        .map((item) => normalizeText(item))).find((eventId) => input.primaryIdSet.has(eventId));
    if (validDependencyParent)
        return validDependencyParent;
...
function resolveTimeRef(input: {
    ...
    if (existing)
        return existing;
    const timelineMatch = input.timelineCandidates.find((candidate) => (containsInText(input.eventScope, candidate) && containsInText(input.source, candidate)));
    if (timelineMatch)
    ...
    if (!parentTime)
        return '';
    if (containsInText(input.source, parentTime))
        return parentTime;
    if (input.timelineCandidateSet.has(parentTime))
        return parentTime;
    return '';
}
function pruneInvalidDependencies(graph: {
    ...
    allEvents.forEach((event) => {
        ...
        if (!eventId)
            return;
        const dependencies = uniqueStrings((Array.isArray(event.dependsOnEventIds) ? event.dependsOnEventIds : [])
            .map((item) => normalizeText(item))).filter((depId) => depId !== eventId && idSet.has(depId));
...
function backfillEventFieldsInternal(input: {
    ...
    const normalizedPrimary = input.events.primary.map((event) => {
        ...
        const existingCharacterRefs = sanitizeExistingEntityRefs(Array.isArray(event.characterRefs) ? event.characterRefs : []);
        const existingLocationRefs = sanitizeExistingEntityRefs(Array.isArray(event.locationRefs) ? event.locationRefs : []);
    ...
    const normalizedSecondary = input.events.secondary.map((event) => {
        ...
        const existingDeps = uniqueStrings((Array.isArray(event.dependsOnEventIds) ? event.dependsOnEventIds : [])
            .map((item) => normalizeText(item))).filter((depId) => depId !== selfEventId);
        const dependsOnEventIds = (existingDeps.length > 0
            ? existingDeps
...
export function backfillChunkExtractionEventFields(extraction: ChunkExtraction, sourceText: string): ChunkExtraction {
...
export function backfillKnowledgeGraphEventFields(graph: WorldStudioKnowledgeGraphDraft, sourceText: string): WorldStudioKnowledgeGraphDraft {
    const events = backfillEventFieldsInternal({
