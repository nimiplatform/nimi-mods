import type { DraftPatch, FinalDraftAccumulator, WorldStudioAgentDraft, } from './types.js';
...
export function mergeRecordPreferIncoming(existing: Record<string, unknown>, incoming: Record<string, unknown>): Record<string, unknown> {
...
function mergeDraftArray(existing: Array<Record<string, unknown>>, incoming: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
...
export function buildFinalDraftAccumulatorSlice(accumulator: FinalDraftAccumulator, options?: {
    maxLorebooks?: number;
...
export function applyDraftPatch(accumulator: FinalDraftAccumulator, patch: DraftPatch): {
    ...
    const nextWorld = patch.world
        ...
        : accumulator.world;
    if (!valuesEqual(nextWorld, accumulator.world))
    ...
    if (!valuesEqual(nextWorldview, accumulator.worldview))
        changedFields.push('worldview');
    ...
    if (!valuesEqual(nextWorldLorebooks, accumulator.worldLorebooks))
        changedFields.push('worldLorebooks');
    ...
    if (!valuesEqual(nextFutureEvents, accumulator.futureHistoricalEvents))
        changedFields.push('futureHistoricalEvents');
...
export function resolveAccumulatorAgentDrafts(accumulator: FinalDraftAccumulator, selectedCharacters: string[]): WorldStudioAgentDraft[] {
