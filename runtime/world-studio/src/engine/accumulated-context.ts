import type { AccumulatedCharacter, AccumulatedEvent, AccumulatedLocation, AccumulatedRelation, AccumulatedState, AccumulatedTimeline, } from './types.js';
...
export function compressAccumulatedState(state: AccumulatedState, tokenBudget: number): string {
    ...
    tryAppendLines(`KNOWN_TIMELINE (${state.timeline.length} total):`, buildTimelineLines(state.timeline));
    tryAppendLines(`KNOWN_CHARACTERS (${state.characters.length} total):`, buildCharacterLines(state.characters));
    tryAppendLines(`KNOWN_EVENTS (${state.events.primary.length} primary, ${state.events.secondary.length} secondary):`, buildEventLines(state.events.primary));
    tryAppendLines(`KNOWN_LOCATIONS (${state.locations.length} total):`, buildLocationLines(state.locations));
    // Secondary events (lower priority)
    if (state.events.secondary.length > 0) {
        tryAppendLines('KNOWN_SECONDARY_EVENTS:', buildEventLines(state.events.secondary));
    }
    tryAppendLines(`KNOWN_RELATIONS (${state.characterRelations.length} total):`, buildRelationLines(state.characterRelations));
