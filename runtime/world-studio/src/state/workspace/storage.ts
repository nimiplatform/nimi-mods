...
function computeSelectionDelta(before: string[], after: string[]): {
    added: string[];
    removed: string[];
} {
...
export async function readSnapshotFromStorage(userId: string): Promise<WorldStudioWorkspaceSnapshot | null> {
    ...
    try {
        ...
        const parsedKnowledgeEvents = normalizeEventsDraft((parsed.knowledgeGraph as {
            events?: {
                primary?: unknown[];
                secondary?: unknown[];
            };
        } | undefined)?.events || {});
        const parsedEventsDraft = (parsedEventsDraftRaw.primary.length > 0 || parsedEventsDraftRaw.secondary.length > 0)
        ...
        const snapshot: WorldStudioWorkspaceSnapshot = {
            ...
            agentSync: {
                ...
                draftsByCharacter: (parsed.agentSync && typeof parsed.agentSync === 'object'
                    ? asRecord((parsed.agentSync as {
                        draftsByCharacter?: unknown;
                    }).draftsByCharacter)
...
export async function persistSnapshotToStorage(userId: string, snapshot: WorldStudioWorkspaceSnapshot): Promise<void> {
    ...
    const selectedCharactersDelta = computeSelectionDelta(previousPersisted.selectedCharacters, normalizedSelectedCharacters);
    const agentSyncSelectedCharacterIdsDelta = computeSelectionDelta(previousPersisted.agentSyncSelectedCharacterIds, normalizedAgentSyncSelectedCharacterIds);
    lastPersistedSelectionStateByUser.set(normalizedUserId, {
