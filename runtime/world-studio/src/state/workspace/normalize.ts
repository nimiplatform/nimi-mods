...
function normalizeAgentDraft(value: unknown): WorldStudioAgentDraft | null {
    ...
    const normalizeAgentRules = (input: unknown): WorldStudioAgentDraft['rules'] | undefined => {
        ...
        if (String(ruleRecord.format || '').trim() !== 'rule-lines-v1')
            return undefined;
...
export function normalizeEventsDraft(value: unknown): {
...
export function parseEventsDraftFromText(text: string): {
    primary: EventNodeDraft[];
    secondary: EventNodeDraft[];
} {
...
function normalizeTaskRecord(value: unknown): WorldStudioTaskRecord | null {
    ...
    const allowedStatus = (LIVE_TASK_STATUSES.has(status as never)
        || TERMINAL_TASK_STATUSES.has(status as never)) ? status : 'FAILED';
...
export function recoverTaskStateAfterReload(taskState: WorldStudioTaskState): WorldStudioTaskState {
    ...
    if (!activeTask)
        return taskState;
    if (!LIVE_TASK_STATUSES.has(activeTask.status as never))
        return taskState;
...
export function syncSnapshot(snapshot: WorldStudioWorkspaceSnapshot): WorldStudioWorkspaceSnapshot {
    ...
    const parseJob = {
        ...
        chunkProcessed: Math.max(0, Number(snapshot.parseJob?.chunkProcessed) || (Number(snapshot.parseJob?.chunkCompleted) || 0) + (Number(snapshot.parseJob?.chunkFailed) || 0)),
