...
function computeArrayDiff(before: string[], after: string[]): {
    added: string[];
    removed: string[];
} {
...
export const useWorldStudioWorkspaceStore = create<WorldStudioWorkspaceStore>((set, get) => ({
    ...
    patchSnapshot: (patch) => set((state) => {
        ...
        const snapshot = syncSnapshot({
            ...
            agentSync: {
                ...
                draftsByCharacter: (() => {
                    ...
                    Object.entries(incoming).forEach(([name, value]) => {
                        ...
                        if (!normalizedName || !value || typeof value !== 'object')
                            return;
            ...
            embeddingIndex: {
                ...
                entries: (() => {
                    ...
                    const record = patch.embeddingIndex as {
                        entries?: unknown;
                    };
        ...
        const shouldLogSelectionPatch = Boolean(patchSelectedCharacters
