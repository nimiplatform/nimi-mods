...
test('maintain workbench removes mutations from the primary editor workflow', () => {
  ...
  assert.match(worldBasePanelSource, /readOnly/);
  assert.doesNotMatch(worldBasePanelSource, /next\.clockConfig\s*=/);
  assert.match(worldviewPanelSource, /label="languages"/);
  assert.match(worldviewPanelSource, /worldview\.groups\.timeTitle/);
  assert.match(worldviewPanelSource, /worldview\.groups\.cultureTitle/);
  assert.equal((maintainSource.match(/Refresh Remote/g) || []).length, 1);
  assert.doesNotMatch(agentsPanelSource, /Save Agent Metadata/);
  assert.doesNotMatch(agentsPanelSource, /Create Missing Draft Agents/);
