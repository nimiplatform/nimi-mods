...
export function BuddyWorkbench(props: Props) {
    ...
    const { modelState, modelError, messages, isGenerating, isRecording, streamingText, showRestReminder, selectedModelId, textRouteOptions, ttsRouteOptions, sttRouteOptions, textRouteBinding, ttsRouteBinding, sttRouteBinding, routeOptionsLoading, voiceModeEnabled, ttsVoiceOptions, ttsVoicesLoading, selectedTtsVoiceId, activeAudioMessageId, audioStatusByMessageId, audioErrorByMessageId, mountCanvas, loadModel, tapModel, selectModel, setVoiceModeEnabled, setRouteSource, setRouteConnector, setRouteModel, setSelectedTtsVoiceId, sendMessage, playAssistantMessageAudio, startRecording, stopRecording, dismissRestReminder, retry, } = props;
    ...
    return (<div className="relative flex h-full w-full flex-col overflow-x-hidden overflow-y-auto bg-[linear-gradient(180deg,_#f8fbff_0%,_#edf5ff_42%,_#f7fbff_100%)]">
      ...
      <div className="relative z-20 flex items-center justify-between gap-4 border-b border-white/60 bg-white/72 px-6 py-4 backdrop-blur">
        ...
        <div className="flex items-center gap-2 text-xs text-slate-600">
          <button type="button" onClick={() => setShowRoutePanel((value) => !value)} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-700 transition hover:border-emerald-300 hover:text-emerald-700">
          ...
          <select value={selectedModelId} onChange={(event) => selectModel(event.target.value)} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-700 outline-none">
            {BUDDY_MODELS.map((model) => (<option key={model.id} value={model.id}>
                {model.label}
              </option>))}
          </select>
        </div>
      ...
      <div className="relative z-10 flex flex-1 flex-col gap-4 px-4 py-4 lg:px-6">
        <div className="relative min-h-[44rem] flex-1 overflow-hidden rounded-[36px] border border-white/70 bg-[linear-gradient(180deg,_rgba(255,255,255,0.76)_0%,_rgba(238,249,255,0.68)_100%)] shadow-[0_24px_80px_rgba(148,163,184,0.16)]">
          ...
          <div className="absolute right-6 top-6 z-20 flex flex-col items-end gap-3">
            <button type="button" onClick={() => setShowRoutePanel((value) => !value)} className="pointer-events-auto rounded-full border border-white/75 bg-white/78 px-4 py-2 text-sm text-slate-700 shadow-sm backdrop-blur transition hover:border-emerald-300 hover:text-emerald-700">
              {showRoutePanel ? t('BuddyPage.closeConsole') : t('BuddyPage.openConsole')}
            </button>
          </div>

          {showRoutePanel && (<div className="absolute inset-y-4 right-4 z-30 flex w-[21rem] flex-col gap-4 rounded-[30px] border border-white/70 bg-white/84 p-4 shadow-[0_24px_60px_rgba(148,163,184,0.16)] backdrop-blur">
              <div className="flex items-start justify-between gap-3">
                <div className="text-base font-semibold text-slate-900">{t('BuddyPage.consoleTitle')}</div>
                <button type="button" onClick={() => setShowRoutePanel(false)} className="rounded-full border border-slate-200 px-3 py-1.5 text-xs text-slate-500">
              ...
              <div className="rounded-[24px] border border-emerald-100 bg-[linear-gradient(135deg,_rgba(236,253,245,0.9)_0%,_rgba(240,249,255,0.95)_100%)] p-4">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <button type="button" onClick={() => setVoiceModeEnabled(!voiceModeEnabled)} className={`rounded-full px-3 py-1.5 text-xs font-medium transition ${voiceModeEnabled
              ...
              <div className="rounded-[24px] border border-slate-100 bg-slate-50/80 p-4">
                ...
                <div className="grid gap-3 text-xs text-slate-500">
                  <label className="grid gap-1">
                    ...
                    <select value={selectedModelId} onChange={(event) => selectModel(event.target.value)} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-700 outline-none">
                      {BUDDY_MODELS.map((model) => (<option key={model.id} value={model.id}>
                          {model.label}
                        </option>))}
                    </select>
              ...
              <div className="grid gap-3 overflow-y-auto pr-1">
            ...
            }} onChangeSource={(source) => setRouteSource('text', source)} onChangeConnector={(connectorId) => setRouteConnector('text', connectorId)} onChangeModel={(model) => setRouteModel('text', model)}/>
                <RouteSelector value={{
                label: t('RouteSelector.ttsRoute'),
                binding: ttsRouteBinding,
                options: ttsRouteOptions,
                loading: routeOptionsLoading,
            }} onChangeSource={(source) => setRouteSource('tts', source)} onChangeConnector={(connectorId) => setRouteConnector('tts', connectorId)} onChangeModel={(model) => setRouteModel('tts', model)}/>
                <div className="rounded-[24px] border border-slate-100 bg-slate-50/80 p-4">
                  <div className="mb-3 text-sm font-semibold text-slate-900">{t('BuddyPage.ttsVoice')}</div>
                  <div className="grid gap-2">
                    <select value={selectedTtsVoiceId} onChange={(event) => setSelectedTtsVoiceId(event.target.value)} disabled={ttsVoicesLoading || ttsVoiceOptions.length === 0} className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-700 outline-none disabled:cursor-not-allowed disabled:opacity-60">
                      {ttsVoiceOptions.length === 0 ? (<option value="">
                          {ttsVoicesLoading ? t('BuddyPage.voiceLoading') : t('BuddyPage.voiceUnavailable')}
                        </option>) : (ttsVoiceOptions.map((voice) => (<option key={voice.id} value={voice.id}>
                ...
                loading: routeOptionsLoading,
            }} onChangeSource={(source) => setRouteSource('stt', source)} onChangeConnector={(connectorId) => setRouteConnector('stt', connectorId)} onChangeModel={(model) => setRouteModel('stt', model)}/>
              </div>
            </div>)}

            {modelState === 'loading' && (<div className="absolute inset-0 z-10 flex items-center justify-center">
                  <div className="rounded-xl bg-white/85 px-6 py-3 text-sm text-slate-500 shadow-sm backdrop-blur">
                  {t('BuddyPage.modelLoading')}
                  </div>
              </div>)}

            {modelState === 'error' && (<div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3">
                <div className="rounded-xl bg-red-50 px-6 py-3 text-sm text-red-600">
                  {t('BuddyPage.modelError')}{modelError ? `: ${modelError}` : ''}
                </div>
                <button className="rounded-lg bg-blue-500 px-4 py-2 text-sm text-white hover:bg-blue-600" onClick={retry}>
                  {t('BuddyPage.retry')}
                </button>
              </div>)}

            <Live2DCanvas onReady={onCanvasReady} onTap={(event) => tapModel(event.clientX, event.clientY)}/>
          <div className="absolute bottom-6 right-6 z-20 w-[min(34rem,calc(100%-3rem))]">
            <ChatOverlay messages={messages} streamingText={streamingText} voiceModeEnabled={voiceModeEnabled} isGenerating={isGenerating} isRecording={isRecording} input={input} activeAudioMessageId={activeAudioMessageId} audioStatusByMessageId={audioStatusByMessageId} audioErrorByMessageId={audioErrorByMessageId} onInputChange={setInput} onInputKeyDown={handleInputKeyDown} onSubmit={handleSubmit} onPlayAssistantMessageAudio={playAssistantMessageAudio} inputActions={(<VoiceButton isRecording={isRecording} onStart={startRecording} onStop={stopRecording}/>)}/>
          </div>
      ...
      {showRestReminder && (<div className="absolute inset-0 z-30 flex items-center justify-center bg-black/20 backdrop-blur-sm">
          <div className="mx-4 max-w-sm rounded-2xl bg-white p-6 text-center shadow-xl">
            ...
            <button className="rounded-lg bg-blue-500 px-6 py-2 text-sm text-white hover:bg-blue-600" onClick={dismissRestReminder}>
