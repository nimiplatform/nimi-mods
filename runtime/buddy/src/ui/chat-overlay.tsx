...
export function ChatOverlay({
  ...
  return (
    <div className="flex min-h-0 max-h-[23rem] flex-col overflow-hidden rounded-[28px] border border-white/30 bg-white/14 shadow-[0_16px_42px_rgba(31,38,135,0.06)]">
      <div className="flex items-center justify-between border-b border-white/24 bg-white/10 px-4 py-3">
        <div>
          <div className="text-[16px] font-semibold text-slate-800">{t('ChatOverlay.title')}</div>
          <div className="text-[13px] text-slate-500">
            {voiceModeEnabled ? t('ChatOverlay.autoVoiceEnabled') : t('ChatOverlay.manualPlay')}
          </div>
        </div>
        <div className="rounded-full border border-white/24 bg-white/18 px-3 py-1 text-[12px] font-medium text-slate-500">
      ...
      >
        {visibleMessages.length === 0 && !streamingText && (
          <div className="flex h-full min-h-28 items-center justify-center rounded-[22px] border border-dashed border-white/20 bg-white/8 px-6 text-center text-[14px] leading-6 text-slate-500">
        ...
        {visibleMessages.map((msg) => (
          ...
          >
            <div
              className={`max-w-[88%] rounded-[22px] px-4 py-3 text-[14px] shadow-[0_8px_24px_rgba(31,38,135,0.05)] ${
                msg.role === 'user'
                  ...
                  : isGenerationFailureMessage(msg.content)
                    ...
                    : 'border border-white/24 bg-white/20 text-slate-800'
              }`}
            >
              ...
              {msg.role === 'assistant' && (
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  <button
                    ...
                    className={`rounded-full border px-3 py-1.5 text-[13px] transition ${
                      activeAudioMessageId === msg.id
                        ...
                        : 'border-slate-200/45 bg-white/45 text-slate-700 hover:border-emerald-300 hover:text-emerald-700'
                    }`}
                  >
                    {activeAudioMessageId === msg.id
                  ...
                  {msg.emotion && (
                    <span className="rounded-full bg-emerald-50/90 px-2.5 py-1 text-[12px] uppercase tracking-[0.12em] text-emerald-600">
                      {msg.emotion}
                    </span>
                  )}
                  {audioStatusByMessageId[msg.id] === 'error' && (
                    <span className="max-w-full break-all text-[12px] text-rose-500">
        ...
        {streamingText && (
          <div className="mb-3 flex justify-start">
            <div className="max-w-[88%] rounded-[22px] border border-white/24 bg-white/20 px-4 py-3 text-[14px] text-slate-800 shadow-[0_8px_24px_rgba(31,38,135,0.05)]">
      ...
      <form
        onSubmit={onSubmit}
        className="flex items-center gap-3 border-t border-white/24 bg-white/10 px-4 py-4"
      >
        {inputActions}
        <input
          type="text"
          ...
          className="min-w-0 flex-1 bg-transparent px-2 text-[14px] text-slate-800 outline-none placeholder:text-slate-400 disabled:opacity-50"
        />
        <button
          type="submit"
          disabled={!input.trim() || isGenerating}
          className="rounded-full bg-gradient-to-r from-emerald-400 via-teal-400 to-cyan-400 px-6 py-3 text-[14px] font-medium text-white shadow-[0_12px_24px_rgba(16,185,129,0.22)] transition-opacity disabled:opacity-40"
        >
