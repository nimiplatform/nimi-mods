...
import { MOD_ID, TAB_ID, REST_REMINDER_INTERVAL_MS, REST_REMINDER_IDLE_RESET_MS, DEFAULT_BUDDY_MODEL_ID, type BuddyModelId, BUDDY_MODELS, } from '../contracts.js';
...
import { compileMessages, extractEmotion, createDialogueHistory, type ChatMessage, } from '../services/dialogue-engine.js';
import { buildBuddySystemPrompt, getBuddyPromptProfile } from '../services/prompt-template.js';
import { playAudioSource, detectMimeType, recordVoice, concatBytes, } from '../services/voice-engine.js';
...
import { createHookClient, logRendererEvent, createModRuntimeClient, type RuntimeCanonicalCapability, type RuntimeRouteBinding, type RuntimeRouteOptionsSnapshot, type RuntimeRouteSource } from "@nimiplatform/sdk/mod";
...
function chooseBindingBySource(source: RuntimeRouteSource, options: RuntimeRouteOptionsSnapshot | null, previous: RuntimeRouteBinding | null): RuntimeRouteBinding | null {
    if (!options)
        return previous;
    if (source === 'local') {
...
function chooseBindingByConnector(connectorId: string, options: RuntimeRouteOptionsSnapshot | null, previous: RuntimeRouteBinding | null): RuntimeRouteBinding | null {
    if (!options)
        return previous;
...
function chooseBindingByModel(model: string, previous: RuntimeRouteBinding | null): RuntimeRouteBinding | null {
...
function firstArtifactWithAudio(artifacts: Array<{
    bytes?: Uint8Array;
    uri?: string;
    mimeType?: string;
}>): {
...
export function useBuddyController(sdkRuntimeContext: unknown): BuddyControllerState & BuddyControllerActions {
    ...
    useEffect(() => {
        ...
        setAudioStatusByMessageId((current) => {
            const next = Object.fromEntries(Object.entries(current).filter(([key]) => keepIds.has(key)));
    ...
    const sendMessage = useCallback(async (text: string) => {
        if (!runtimeRef.current || isGenerating)
            return;
        resetRestTimer();
        ...
        try {
            const compiled = compileMessages(historyRef.current.messages, buildBuddySystemPrompt(getBuddyPromptProfile(selectedModelId)));
