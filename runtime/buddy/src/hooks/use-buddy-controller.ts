...
export function useBuddyController(
  ...
  const appendAssistantNotice = useCallback((content: string) => {
    const latest = historyRef.current.messages[historyRef.current.messages.length - 1];
    if (latest?.role === 'assistant' && latest.content === content) {
      setMessages([...historyRef.current.messages]);
      return;
    }
    historyRef.current.addAssistant(content);
    setMessages([...historyRef.current.messages]);
  }, []);
  ...
  const startRecording = useCallback(async () => {
    ...
    if (typeof navigator === 'undefined' || !navigator.mediaDevices || typeof navigator.mediaDevices.getUserMedia !== 'function') {
      appendAssistantNotice(t('Errors.micUnsupported'));
      return;
    }
    setIsRecording(true);
    try {
      ...
      const reason = resolveRecordingStartErrorCode(error);
      if (reason === 'permission') {
        appendAssistantNotice(t('Errors.micPermissionDenied'));
        return;
      }
      if (reason === 'device-missing') {
        appendAssistantNotice(t('Errors.micDeviceMissing'));
        return;
      }
      if (reason === 'device-unavailable') {
        appendAssistantNotice(t('Errors.micUnavailable'));
        return;
      }
      appendAssistantNotice(t('Errors.micStartFailed'));
    }
  }, [appendAssistantNotice, isRecording, t]);
  ...
  const stopRecording = useCallback(async () => {
    ...
    try {
      ...
      if (blob.size === 0) {
        appendAssistantNotice(t('Errors.recordingEmpty'));
        return;
      }
      ...
      if (transcribed.trim()) {
        ...
        appendAssistantNotice(t('Errors.transcriptionEmpty'));
      }
    } catch {
      setIsRecording(false);
      appendAssistantNotice(t('Errors.transcriptionFailed'));
    }
  }, [appendAssistantNotice, sendMessage, t]);
