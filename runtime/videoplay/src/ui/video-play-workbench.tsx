...
import type { EpisodeRecord, FallbackAuditRecord, ReleasePackage, VideoPlayRebuildImpactPreview, VideoPlayStageAdvancePlan, VideoPlayPipelineStageProgress, VideoPlayRunEvent, VideoPlayWorkbenchStageProgress, VideoStoryPackage, VideoStorySummary, } from '../types.js';
import { VIDEOPLAY_PIPELINE_CHAIN, type VideoPlayOperationType, type VideoPlayPipelineStep, type VideoPlayWorkbenchStage, type VideoStorySourceMode, } from '../contracts.js';
import { listManualRerunSteps, workbenchStageSteps, } from '../workbench/stage-flow.js';
...
function statusTone(status: string): string {
    if (status === 'COMPLETED' || status === 'APPROVED' || status === 'ADJUSTED')
...
export function VideoPlayWorkbench(props: VideoPlayWorkbenchProps) {
    ...
    const canPublish = Boolean(props.selectedReleaseCandidate
    ...
    const canRun = Boolean(!props.loading
        && props.routeReady
        && props.selectedStory
        && props.storyPackage
        && !props.storyPackageLoading);
    const canRerun = Boolean(!props.loading && props.stageProgress.length > 0 && !rerunBlockedByPreview);
    const renderQueueEvents = props.runEvents.filter((event) => event.step === 'asset-render' && event.details?.phase === 'batch-queue-execute');
    const manualRerunSteps = listManualRerunSteps();
    const selectedStageRow = props.workbenchStages.find((stage) => stage.stage === props.selectedWorkbenchStage) || null;
    const renderCoverageEvent = [...props.runEvents]
        .reverse()
        .find((event) => event.step === 'asset-render' && typeof event.details?.coverage === 'number');
    return (<div className="ui-sync-root flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
      <header className="ui-sync-shell-header flex shrink-0 flex-col gap-3 border-b border-gray-200 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button type="button" onClick={props.onRunPipeline} disabled={!canRun} className="ui-sync-btn ui-sync-btn-primary rounded-md bg-blue-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50">
            {t('action.runPipeline')}
          </button>
          <button type="button" onClick={props.onRerunStep} disabled={!canRerun} className="ui-sync-btn ui-sync-btn-secondary rounded-md border border-gray-300 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50">
            {t('action.rerunStep')}
          </button>
          <button type="button" onClick={props.onAdvanceStage} disabled={!canAdvance} className="ui-sync-btn ui-sync-btn-secondary rounded-md border border-gray-300 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50">
            {t('action.advanceStage')}
          </button>
          <button type="button" onClick={props.onCancelRun} className="ui-sync-btn rounded-md border border-rose-300 bg-white px-3 py-1.5 text-xs font-medium text-rose-700 hover:bg-rose-50">
            {t('action.cancelRun')}
          </button>
        </div>
      </header>
      <div className="ui-sync-shell-main videoplay-shell-main flex min-h-0 min-w-0 flex-1 flex-col gap-3 overflow-y-auto p-3 lg:grid lg:grid-cols-[minmax(240px,280px)_minmax(0,1fr)_minmax(260px,320px)] lg:grid-rows-[minmax(0,1fr)] lg:overflow-hidden xl:grid-cols-[minmax(280px,320px)_minmax(0,1fr)_minmax(300px,360px)]">
        <aside className="ui-sync-pane ui-sync-pane-side videoplay-shell-side min-h-[220px] overflow-y-auto rounded-xl border border-gray-200 bg-white p-3 lg:min-h-0">
          <section className="space-y-2 rounded-lg border border-gray-200 p-3">
            ...
            <label className="block text-xs text-gray-600">
              {t('label.worldId')}
              <input value={props.worldId} onChange={(event) => props.onWorldIdChange(event.target.value)} className="mt-1 w-full rounded-md border border-gray-300 px-2 py-1 text-xs"/>
            </label>
            <label className="block text-xs text-gray-600">
              {t('label.projectId')}
              <input value={props.projectId} onChange={(event) => props.onProjectIdChange(event.target.value)} className="mt-1 w-full rounded-md border border-gray-300 px-2 py-1 text-xs"/>
            </label>
            <label className="block text-xs text-gray-600">
              {t('label.story')}
              <select value={props.selectedStoryId} onChange={(event) => props.onSelectStory(event.target.value)} className="mt-1 w-full rounded-md border border-gray-300 px-2 py-1 text-xs">
            ...
            <label className="block text-xs text-gray-600">
              {t('label.sourceMode')}
              <select value={props.sourceMode} onChange={(event) => props.onSourceModeChange(event.target.value as VideoStorySourceMode)} className="mt-1 w-full rounded-md border border-gray-300 px-2 py-1 text-xs">
            ...
            <label className="block text-xs text-gray-600">
              ...
              <input value={props.ingestCursorStart} onChange={(event) => props.onIngestCursorStartChange(event.target.value)} className="mt-1 w-full rounded-md border border-gray-300 px-2 py-1 text-xs"/>
            </label>
            <p className="text-[11px] text-gray-500">
              {t('label.storyId')}: <span className="font-medium text-gray-700">{props.selectedStory?.storyId || '-'}</span>
            </p>
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={props.onReloadStoryPackage} className="rounded-md border border-gray-300 px-2 py-1 text-xs font-medium text-gray-700 hover:bg-gray-100">
                {t('action.reloadPackage')}
              </button>
              <button type="button" onClick={props.onRefresh} className="rounded-md border border-gray-300 px-2 py-1 text-xs font-medium text-gray-700 hover:bg-gray-100">
                {t('action.refresh')}
              </button>
            </div>
          </section>
          ...
          <section className="mt-3 space-y-2 rounded-lg border border-gray-200 p-3">
            ...
            {props.storyPackageError ? (<div className="rounded-md border border-rose-200 bg-rose-50 px-2 py-1 text-xs text-rose-700">
          ...
          <section className="mt-3 space-y-2 rounded-lg border border-gray-200 p-3">
            ...
            <div className="max-h-52 space-y-1 overflow-y-auto">
              {props.episodes.map((episode) => (<button key={episode.episodeId} type="button" onClick={() => props.onSelectEpisode(episode.episodeId)} className={`w-full rounded-md border px-2 py-1 text-left text-xs ${props.selectedEpisodeId === episode.episodeId ? 'border-blue-400 bg-blue-50' : 'border-gray-200 bg-white hover:bg-gray-50'}`}>
          ...
          <section className="mt-3 space-y-2 rounded-lg border border-gray-200 p-3">
            ...
            <div className="max-h-40 space-y-1 overflow-y-auto">
              {props.runs.map((run) => (<div key={run.runId} className="rounded-md border border-gray-200 px-2 py-1 text-xs">
        ...
        <main className="ui-sync-pane ui-sync-pane-main videoplay-shell-center min-h-[320px] min-w-0 overflow-y-auto rounded-xl border border-gray-200 bg-white p-3 lg:min-h-0">
          <section className="rounded-lg border border-gray-200 bg-white p-3">
            ...
            <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
              {props.workbenchStages.map((stage) => (<button key={stage.stage} type="button" onClick={() => props.onSelectWorkbenchStage(stage.stage)} className={`ui-sync-btn rounded-md border px-2 py-1 text-left text-xs ${props.selectedWorkbenchStage === stage.stage
            ...
            : t('pipeline.advanceBlocked')}
              ...
              {!props.stageAdvancePlan.allowed && props.stageAdvancePlan.actionHint ? (<p className="text-rose-600">
                  {props.stageAdvancePlan.reasonCode}: {props.stageAdvancePlan.actionHint}
                </p>) : null}
            </div>
            <div className="mt-2 grid grid-cols-1 gap-2 lg:grid-cols-2">
              {props.stageProgress.map((stage) => (<div key={stage.step} className="rounded-md border border-gray-200 px-2 py-1 text-xs">
          ...
          <section className="mt-3 rounded-lg border border-gray-200 bg-white p-3">
            ...
            {props.rebuildPreview ? (<div className="mt-2 rounded-md border border-amber-200 bg-amber-50 px-2 py-1 text-xs text-amber-800">
                ...
                {isPipelineStep(props.rebuildPreview.recommendedRerunStep) ? (<p>{t('workbench.rebuildChainStepAvailable')}</p>) : null}
                ...
                {!props.rebuildPreview.confirmed ? (<button type="button" onClick={props.onConfirmRebuildPreview} className="mt-1 rounded-md border border-amber-300 bg-white px-2 py-1 text-[11px] font-medium text-amber-800 hover:bg-amber-100">
            ...
            <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-3">
              <label className="text-xs text-gray-600">
                ...
                <select value={props.rerunStep} onChange={(event) => props.onRerunStepChange(event.target.value as VideoPlayPipelineStep)} className="mt-1 w-full rounded-md border border-gray-300 px-2 py-1 text-xs">
                  {manualRerunSteps.map((step) => (<option key={step} value={step}>{pipelineStepLabel(step)}</option>))}
                </select>
              </label>
              <label className="text-xs text-gray-600">
                {t('label.operation')}
                <select value={props.operationType} onChange={(event) => props.onOperationTypeChange(event.target.value as VideoPlayOperationType)} className="mt-1 w-full rounded-md border border-gray-300 px-2 py-1 text-xs">
                  {props.operationOptions.map((option) => (<option key={option.value} value={option.value}>{option.label}</option>))}
                </select>
              </label>
              <div className="flex items-end">
                <button type="button" onClick={props.onApplyOperation} disabled={!props.selectedEpisode} className="w-full rounded-md border border-gray-300 bg-white px-2 py-1 text-xs font-medium text-gray-700 hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50">
            ...
            <label className="mt-2 block text-xs text-gray-600">
              ...
              <textarea value={props.operationPayload} onChange={(event) => props.onOperationPayloadChange(event.target.value)} rows={4} className="mt-1 w-full rounded-md border border-gray-300 px-2 py-1 text-xs"/>
            </label>
          </section>
          ...
          <section className="mt-3 rounded-lg border border-gray-200 bg-white p-3">
            ...
            <div className="mt-2 max-h-[360px] space-y-1 overflow-y-auto">
              {props.selectedEpisode?.storyboard.shotPlans.map((shot) => (<div key={shot.shotId} className="rounded-md border border-gray-200 px-2 py-1 text-xs">
        ...
        <aside className="ui-sync-pane ui-sync-pane-right videoplay-shell-right min-h-[220px] overflow-y-auto rounded-xl border border-gray-200 bg-white p-3 lg:min-h-0">
          <section className="space-y-2 rounded-lg border border-gray-200 p-3">
            ...
            {props.routeStatuses.map((route) => (<div key={route.capability} className="rounded-md border border-gray-200 px-2 py-1 text-xs">
          ...
          <section className="mt-3 space-y-2 rounded-lg border border-gray-200 p-3">
            ...
            {props.storyPackage ? (<>
                ...
                {props.storyPackage.sourceMode === 'textplay-enriched-story' ? (<p className="text-xs text-amber-700">
          ...
          <section className="mt-3 space-y-2 rounded-lg border border-gray-200 p-3">
            ...
            {props.selectedEpisode?.quality.gates.map((gate) => (<div key={gate.gate} className="rounded-md border border-gray-200 px-2 py-1 text-xs">
          ...
          <section className="mt-3 space-y-2 rounded-lg border border-gray-200 p-3">
            ...
            {renderCoverageEvent ? (<p className="text-xs text-gray-600">
              ...
              </p>) : (<p className="text-xs text-gray-500">{t('empty.noRenderCoverage')}</p>)}
            <div className="max-h-32 space-y-1 overflow-y-auto">
              {renderQueueEvents.map((event) => (<div key={`${event.runId}:${event.seq}`} className="rounded-md border border-gray-200 px-2 py-1 text-xs">
          ...
          <section className="mt-3 space-y-2 rounded-lg border border-gray-200 p-3">
            ...
            <button type="button" onClick={props.onPublish} disabled={!canPublish} className="w-full rounded-md bg-emerald-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50">
              {t('action.publish')}
            </button>
            <div className="max-h-24 space-y-1 overflow-y-auto">
              {props.releases.map((release) => (<div key={release.releaseId} className="rounded-md border border-gray-200 px-2 py-1 text-xs">
          ...
          <section className="mt-3 space-y-2 rounded-lg border border-gray-200 p-3">
            ...
            <div className="max-h-24 space-y-1 overflow-y-auto">
              {props.fallbackAudits.map((audit, index) => (<div key={`${audit.stage}:${index}`} className="rounded-md border border-gray-200 px-2 py-1 text-xs">
          ...
          <section className="mt-3 space-y-2 rounded-lg border border-gray-200 p-3">
            ...
            <div className="max-h-40 space-y-1 overflow-y-auto">
              {props.runEvents.map((event) => (<div key={`${event.runId}:${event.seq}`} className="rounded-md border border-gray-200 px-2 py-1 text-xs">
          ...
          {props.error ? (<section className="mt-3 rounded-lg border border-rose-200 bg-rose-50 p-3 text-xs text-rose-700">
