...
import type { VideoPlayOperationType, VideoPlayPipelineStep, VideoPlayReasonCode, VideoPlayRetryClass, VideoPlayRouteStage, VideoPlayWorkbenchStage, VideoPlayWorkbenchStageStatus, VideoStorySourceMode, } from './contracts.js';
import { type HookClient, type ModRuntimeClient, type RuntimeCanonicalCapability, type RuntimeRouteBinding } from "@nimiplatform/sdk/mod";
...
export type VideoPlayRunEventType = 'run.start' | 'step.start' | 'step.chunk' | 'step.complete' | 'step.error' | 'run.complete' | 'run.error' | 'run.canceled';
...
export type VideoPlayPipelineLifecycleStatus = 'RUNNING' | 'PAUSED' | 'FAILED' | 'CANCELED' | 'COMPLETED';
export type VideoPlayPipelineStageStatus = 'PENDING' | 'RUNNING' | 'PAUSED' | 'COMPLETED' | 'FAILED' | 'CANCELED';
