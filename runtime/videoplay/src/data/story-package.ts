...
import { VIDEOPLAY_DATA_API_CORE_AGENT_MEMORY_RECALL_FOR_ENTITY, VIDEOPLAY_DATA_API_WORLD_EVENTS_LIST, VIDEOPLAY_DATA_API_WORLD_LOREBOOKS_LIST, VIDEOPLAY_DATA_API_WORLD_NARRATIVE_CONTEXTS_LIST, VIDEOPLAY_DATA_API_WORLD_SCENES_LIST, VIDEOPLAY_REASON, VIDEOPLAY_STORY_SOURCE_MODE, type VideoStorySourceMode, } from '../contracts.js';
import { VideoPlayError } from '../errors.js';
import { createHash } from '../id.js';
import { NarrativeProjectionRenderInputSchema, NarrativeTurnWindowSchema, VideoStoryDetailSchema, VideoStoryPackageSchema, VideoStorySummarySchema, } from '../schemas.js';
import type { NarrativeProjectionRenderInput, NarrativeTurnWindow, VideoStoryDetail, VideoStoryPackage, VideoStorySummary, } from '../types.js';
...
function scoreLorebook(input: {
    detail: VideoStoryDetail;
    row: z.infer<typeof WorldLorebookRowSchema>;
}): number {
    const tokens = unique([
...
function scoreScene(input: {
    detail: VideoStoryDetail;
    row: z.infer<typeof WorldSceneRowSchema>;
}): number {
    const tokens = unique([
        ...tokenize(input.detail.title),
        ...tokenize(input.detail.summary),
...
function sortPrimaryEvents(rows: z.infer<typeof WorldEventRowSchema>[], worldId: string): z.infer<typeof WorldEventRowSchema>[] {
    const filtered = rows.filter((row) => {
        ...
        if (rowWorldId && rowWorldId !== worldId)
            return false;
...
function buildGapWarnings(input: {
    ...
    if (!input.hasCanon)
        warnings.push('missing-canon-context');
    if (!input.hasStory)
        warnings.push('missing-story-context');
    if (!input.hasSubject)
        warnings.push('missing-subject-context');
    if (!input.hasRelation)
        warnings.push('missing-relation-context');
    if (!input.hasScene)
        warnings.push('missing-scene-context');
    return warnings;
}
...
export async function loadVideoStoryPackage(input: {
    ...
    const storyContext = contexts.find((row) => row.scope === 'STORY' && (!row.storyId || row.storyId === detail.storyId))
        ...
        || null;
    const subjectContext = contexts.find((row) => row.scope === 'SUBJECT' && (toText(row.subjectId) === detail.primaryAgentId || toText(row.scopeKey) === detail.primaryAgentId)) || contexts.find((row) => row.scope === 'SUBJECT') || null;
