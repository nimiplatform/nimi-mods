import { VIDEOPLAY_DATA_API_ASSET_BATCH_UPSERT, VIDEOPLAY_DATA_API_CORE_AGENT_MEMORY_RECALL_FOR_ENTITY, VIDEOPLAY_DATA_API_EPISODE_UPSERT, VIDEOPLAY_PROMPT_ID, VIDEOPLAY_PIPELINE_CHAIN, VIDEOPLAY_QUALITY_RULE, VIDEOPLAY_REASON, VIDEOPLAY_RETRY_CLASS, type VideoPlayPipelineStep, type VideoPlayReasonCode, type VideoPlayRouteStage, } from '../contracts.js';
...
import { AssetRenderOutputSchema, AudioDesignOutputSchema, CandidateSelectionOutputSchema, CharacterCastingOutputSchema, EditComposeOutputSchema, EpisodePlanSchema, NarrativeProjectionRenderInputSchema, NarrativeTurnWindowSchema, QualityGateReportSchema, ReleasePackageSchema, RunEventSchema, ScenePlanningOutputSchema, ScreenplaySchema, StoryboardSchema, VideoStoryPackageSchema, } from '../schemas.js';
import { AUDIO_DESIGN_POLICY, CANDIDATE_SELECTION_POLICY, CHARACTER_CASTING_POLICY, DEFAULT_SEGMENTATION_POLICY, EDIT_COMPOSE_POLICY, QUALITY_GATE_POLICY, SCENE_PLANNING_POLICY, SEGMENTATION_POLICY_BOUNDS, } from '../policy.js';
import { runPromptCanaryCases } from '../prompt/canary.js';
import { resolvePromptLocale, renderPromptTemplate, type VideoPlayPromptLocale, validatePromptVariables, } from '../prompt/registry.js';
import type { AssetRenderOutput, AudioDesignOutput, BgmTrack, CandidateSelectionOutput, CharacterBrief, CharacterCastingOutput, EditComposeOutput, EpisodeRecord, FallbackAuditRecord, NarrativeProjectionRenderInput, NarrativeTurn, NarrativeTurnWindow, QualityGateReport, RenderedAsset, ReleasePackage, RouteInvokeInput, ScenePlanningOutput, ScreenplayBeat, ScreenplayOutput, SegmentationOutput, SegmentedEpisode, SegmentationPolicy, SfxLayer, SelectedTimelineSegment, StoryboardOutput, StoryboardShot, VideoPlayPipelineDeps, VideoPlayPipelineExecutionControl, VideoPlayPipelineInput, VideoPlayPipelineLifecycleStatus, VideoPlayPipelineCheckpoint, VideoPlayPipelineResult, VideoPlayPipelineStageProgress, VideoPlayRunEvent, VideoPlayRunEventType, } from '../types.js';
...
function computeStepInputHash(step: VideoPlayPipelineStep, snapshot: RuntimeSnapshot, input: VideoPlayPipelineInput): string {
    switch (step) {
...
function fallbackForStep(step: VideoPlayPipelineStep): {
    reasonCode: VideoPlayReasonCode;
    actionHint: string;
    stage: string;
} {
