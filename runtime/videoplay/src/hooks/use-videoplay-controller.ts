...
import { VIDEOPLAY_DATA_API_ASSET_BATCH_UPSERT, VIDEOPLAY_DATA_API_EPISODE_UPSERT, VIDEOPLAY_DATA_API_RELEASE_PUBLISH, VIDEOPLAY_MOD_ID, VIDEOPLAY_OPERATION_TYPE, VIDEOPLAY_PIPELINE_CHAIN, VIDEOPLAY_REASON, VIDEOPLAY_STORY_SOURCE_MODE, VIDEOPLAY_WORKBENCH_STAGE, type VideoPlayOperationType, type VideoPlayPipelineStep, type VideoPlayReasonCode, type VideoPlayWorkbenchStage, type VideoStorySourceMode, } from '../contracts.js';
import { toVideoPlayError } from '../errors.js';
import { createHash, createUlid } from '../id.js';
import { listPlayableVideoStories, loadVideoStoryPackage, } from '../data/story-package.js';
import { runVideoPlayEpisodeProduction } from '../pipeline/orchestrator.js';
import { buildGeneratedVoiceAssets, buildManualLipSyncAssets, } from '../operations/voice-assets.js';
import { applyCreatorOperation } from '../storage/operations.js';
import type { AudioDesignOutput, CandidateSelectionOutput, CharacterCastingOutput, EpisodeRecord, FallbackAuditRecord, ReleasePackage, RenderedAsset, VideoPlayPipelineCheckpoint, VideoPlayRebuildImpactPreview, VideoPlayStageAdvancePlan, VideoPlayPipelineStageProgress, VideoPlayRunEvent, VideoPlayWorkbenchStageProgress, VideoStoryPackage, VideoStorySummary, } from '../types.js';
import type { VideoPlayWorkbenchProps } from '../ui/video-play-workbench.js';
import { buildRebuildImpactPreview, computeStageAdvancePlan, deriveWorkbenchStageProgress, } from '../workbench/stage-flow.js';
import { createVideoPlayRuntimeAiClient } from '../runtime-ai-client.js';
import { useModTranslation, createHookClient, createModRuntimeClient } from "@nimiplatform/sdk/mod";
...
function toControllerError(error: unknown, fallback: {
...
export function useVideoPlayController(): VideoPlayWorkbenchProps {
    ...
    const [selectedWorkbenchStage, setSelectedWorkbenchStage] = useState<VideoPlayWorkbenchStage>(VIDEOPLAY_WORKBENCH_STAGE.STORY_SOURCE);
    ...
    const selectedStory = useMemo(() => stories.find((item) => item.storyId === selectedStoryId) || null, [stories, selectedStoryId]);
    const selectedEpisode = useMemo(() => episodes.find((item) => item.episodeId === selectedEpisodeId) || null, [episodes, selectedEpisodeId]);
    const routeReady = useMemo(() => routeStatuses.every((item) => item.ready), [routeStatuses]);
    ...
    const activeBranchName = selectedEpisode
        ...
        : '-';
    const workbenchStages = useMemo<VideoPlayWorkbenchStageProgress[]>(() => deriveWorkbenchStageProgress({
    ...
    const stageAdvancePlan = useMemo<VideoPlayStageAdvancePlan>(() => computeStageAdvancePlan({
    ...
    const executePipeline = useCallback(async (input: {
        ...
        try {
            ...
            const checkpointFromError = (pipelineError && typeof pipelineError === 'object'
                ? (pipelineError as {
                    details?: {
    ...
    const handleApplyOperation = useCallback(async () => {
        ...
        try {
            ...
            if (operationType === VIDEOPLAY_OPERATION_TYPE.SELECT_CANDIDATE
            ...
            if (operationType === VIDEOPLAY_OPERATION_TYPE.SELECT_BGM_TRACK
    ...
    const operationOptions = useMemo(() => (Object.values(VIDEOPLAY_OPERATION_TYPE) as VideoPlayOperationType[]).map((value) => ({
        value,
