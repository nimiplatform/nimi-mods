import { pickNarrativeRelationContextRow, pickNarrativeStoryContextRow, pickNarrativeSubjectContextRow, resolveNarrativeContextStoryAnchor, type NarrativeEngineModule, } from '../../../../modules/narrative-engine/src/index.js';
import { TEXTPLAY_DATA_API_CORE_AGENT_MEMORY_RECALL_FOR_ENTITY, TEXTPLAY_DATA_API_WORLD_EVENTS_LIST, TEXTPLAY_DATA_API_WORLD_LOREBOOKS_LIST, TEXTPLAY_DATA_API_WORLD_NARRATIVE_CONTEXTS_LIST, TEXTPLAY_DATA_API_WORLD_SCENES_LIST, } from '../contracts.js';
import { NarrativeTurnLatestLookupRequestSchema, NarrativeTurnLatestLookupResponseSchema, TextplayMemoryRecallResponseSchema, TextplayWorldEventListResponseSchema, TextplayWorldLorebookListResponseSchema, TextplayWorldNarrativeContextListResponseSchema, TextplayWorldSceneListResponseSchema, type TextplayMemoryRecallResponse, type TextplayWorldEventRow, type TextplayWorldLorebookRow, type TextplayWorldNarrativeContextRow, type TextplayWorldSceneRow, } from './schemas.js';
import type { TextplayStoryDetail, TextplayStorySummary, TextplayStartupPackage, TextplayStartupPolicy, } from '../types.js';
...
function scoreScene(input: {
    ...
    let score = 0;
    for (const token of tokens) {
        if (sceneId.includes(token))
            score += 2;
        if (sceneName.includes(token))
            score += 3;
        if (sceneDescription.includes(token))
            score += 1;
    }
    return score;
}
function buildStorySummary(event: TextplayWorldEventRow): string {
...
function buildStoryMaterialSummary(event: TextplayWorldEventRow, horizon: TextplayStorySummary['eventHorizon']): string {
...
function resolvePrimaryAgentId(input: {
    ...
    const allowGlobalCandidate = (candidate: string): boolean => (storyParticipantHints.length === 0 || storyParticipantHints.includes(candidate));
...
export async function loadStoryStartupPackage(input: {
    ...
    const selectedScene = (scenes.find((scene) => toText(scene.id) === recommendedSceneId)
    ...
    return {
        ...
        cast: {
            primaryAgentId: resolvedPrimaryAgentId,
            participants: unique(resolvedPrimaryAgentId
