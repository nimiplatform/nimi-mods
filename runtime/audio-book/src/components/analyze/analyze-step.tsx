...
export function AnalyzeStep(props: AnalyzeStepProps) {
    ...
    const availableChatModels = selectedChatConnector
        ? selectedChatConnector.models.filter((model, index, models) => {
            ...
            if (!normalized || normalized === 'cloud/default' || lower === 'local-model' || lower.endsWith('/local-model'))
                return false;
            if (['tts', 'speech', 'audio', 'voice', 'embedding', 'embed', 'rerank'].some((hint) => lower.includes(hint)))
                return false;
      ...
      <div className="flex min-h-0 flex-1 gap-0">
        ...
        <div className="flex w-80 shrink-0 flex-col border-r border-gray-100 px-6 py-6">
          ...
          <div className="mb-4 rounded-lg border border-gray-200 bg-gray-50 p-3">
            ...
            {ttsRoute.loading ? (<p className="text-xs text-gray-400">{t('analyze.loadingProviders')}</p>) : ttsRoute.error ? (<p className="text-xs text-red-500">{ttsRoute.error}</p>) : ttsRoute.chatConnectors.length === 0 ? (<p className="text-xs text-gray-500">{t('analyze.noLlmConnectors')}</p>) : (<Select value={ttsRoute.chatSelection.connectorId} onValueChange={(v) => ttsRoute.selectChatConnector(v)} options={ttsRoute.chatConnectors.map((c) => ({
                value: c.id,
                label: c.label || c.id,
          ...
          <div className="mb-4 rounded-lg border border-gray-200 bg-gray-50 p-3">
            ...
            {ttsRoute.loading ? (<p className="text-xs text-gray-400">{t('analyze.loadingModels')}</p>) : ttsRoute.error ? (<p className="text-xs text-red-500">{ttsRoute.error}</p>) : !selectedChatConnector ? (<p className="text-xs text-gray-500">{t('analyze.selectProviderFirst')}</p>) : availableChatModels.length === 0 ? (<p className="text-xs text-gray-500">{t('analyze.noSelectableModels')}</p>) : (<Select value={ttsRoute.chatSelection.model || availableChatModels[0] || ''} onValueChange={(value) => ttsRoute.selectChatModel(value)} options={availableChatModels.map((model) => ({
                value: model,
                label: model,
          ...
          {/* Start / progress */}
          {!analysisRunning && !hasResults && (<button type="button" onClick={onStart} className="mb-4 flex w-full items-center justify-center gap-2 rounded-md bg-indigo-600 px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-indigo-700">
          ...
          {analysisRunning && progress && (<div className="mb-4">
              ...
              <Progress value={progress.completedChapters} max={progress.totalChapters}/>
              <p className="mt-2 text-[11px] text-gray-500">
                {t('analyze.segmentsDetected', { segments: progress.segmentsSoFar, characters: progress.charactersSoFar })}
              </p>
            </div>)}

          {analysisRunning && !progress && (<div className="mb-4 flex items-center justify-center gap-2 py-6">
              <div className="h-4 w-4 animate-spin rounded-full border-2 border-indigo-600 border-t-transparent"/>
              <span className="text-sm text-gray-500">{t('analyze.startingAnalysis')}</span>
            </div>)}

          {/* Character chips */}
          {hasResults && (<div className="mb-4">
              <h4 className="mb-2 text-xs font-semibold text-gray-700">{t('analyze.charactersHeader', { count: characters.length })}</h4>
              <div className="flex flex-wrap gap-1.5">
                {characters.map((ch) => (<span key={ch.name} className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-medium ${ch.tier === 'major'
          ...
          {/* Re-analyze button */}
          {hasResults && !analysisRunning && (<button type="button" onClick={onStart} className="mt-auto flex w-full items-center justify-center gap-1.5 rounded-md border border-gray-200 px-4 py-2 text-xs font-medium text-gray-500 transition-colors hover:bg-gray-50">
              {t('analyze.reAnalyze')}
            </button>)}
        </div>
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          {segments.length > 0 ? (<>
              ...
              <div className="min-h-0 flex-1 overflow-y-auto px-6 py-2">
                {segments.slice(0, 50).map((seg) => (<div key={seg.id} className="border-b border-gray-50 py-2 last:border-b-0">
                    <div className="flex items-center gap-2">
                      <SegmentTypeBadge type={seg.type} showIcon/>
                      <span className="text-[11px] font-medium text-gray-500">{seg.speaker}</span>
                      {seg.emotion && (<span className="text-[10px] italic text-gray-400">{seg.emotion}</span>)}
                    </div>
                    <p className="mt-0.5 text-[13px] leading-relaxed text-gray-700">{seg.text}</p>
                  </div>))}
                {segments.length > 50 && (<p className="py-3 text-center text-xs text-gray-400">
