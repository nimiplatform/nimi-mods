...
export function StepIndicator(props: StepIndicatorProps) {
    ...
    return (<nav className="flex items-center gap-1">
      {steps.map((step, i) => {
            ...
            if (isCurrent)
                pillClass = 'bg-indigo-50 text-indigo-600 font-semibold';
            else if (isClickable)
                pillClass = 'text-gray-500 hover:text-gray-700';
            return (<React.Fragment key={step}>
            {i > 0 && (<div className="mx-0.5 h-px w-4 bg-gray-200"/>)}
            <button type="button" disabled={!isClickable} onClick={() => isClickable && onStepClick(step)} className={`rounded-full px-3 py-1.5 text-xs transition-colors ${pillClass} ${isClickable ? 'cursor-pointer' : 'cursor-default'}`}>
              {label}
            </button>
          </React.Fragment>);
        })}
