...
export function CastStep(props: CastStepProps) {
    const { characters, castings, selectedCharacter, previewPlaying, ttsClient, ttsRoute, onSelectCharacter, onUpdateCasting, onPreviewVoice, onAutoRecommend, } = props;
    ...
    return (<div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="shrink-0 border-b border-gray-100 bg-gray-50 px-6 py-3">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            {ttsRoute.loading ? (<p className="text-xs text-gray-400">{t('cast.loadingProviders')}</p>) : ttsRoute.error ? (<p className="text-xs text-red-500">{ttsRoute.error}</p>) : ttsRoute.ttsConnectors.length === 0 ? (<p className="text-xs text-gray-500">{t('cast.noTtsConnectors')}</p>) : (<Select value={ttsRoute.ttsSelection.connectorId} onValueChange={(v) => ttsRoute.selectTtsConnector(v)} options={ttsRoute.ttsConnectors.map((c) => ({
                value: c.id,
                label: c.label || c.id,
      ...
      {voiceError && (<div className="shrink-0 border-b border-red-200 bg-red-50 px-6 py-2 text-xs text-red-700">
          {voiceError}
        </div>)}
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div className="w-56 shrink-0 overflow-y-auto border-r border-gray-100 px-4 py-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <button type="button" onClick={handleAutoRecommend} disabled={autoLoading} className="flex items-center gap-1 rounded-md px-2 py-1 text-[11px] font-medium text-indigo-600 transition-colors hover:bg-indigo-50 disabled:opacity-50">
              {autoLoading && (<span className="inline-block h-3 w-3 animate-spin rounded-full border border-indigo-400 border-t-transparent"/>)}
          ...
          <div className="space-y-1">
            ...
            return (<button key={ch.name} type="button" onClick={() => onSelectCharacter(ch.name)} className={`block w-full rounded-lg px-3 py-2.5 text-left text-xs transition-colors ${isSelected
                  ...
                  {casting && (<p className="mt-0.5 truncate text-[10px] opacity-60">{casting.voiceName}</p>)}
                </button>);
        })}
          </div>
        </div>
        ...
        <div className="min-w-0 flex-1 overflow-y-auto px-6 py-6">
          {selected && selectedCasting ? (<div className="mx-auto max-w-md">
              ...
              <div className="mb-4">
                ...
                {loadingVoices ? (<div className="flex items-center gap-2 py-2">
                  ...
                  </div>) : availableVoices.length === 0 ? (<p className="text-xs text-gray-500">{t('cast.noVoices')}</p>) : (<Select value={selectedCasting.voiceId} onValueChange={(v) => {
              ...
              {/* Speaking rate */}
              <Slider className="mb-4" label={t('cast.speakingRate')} value={selectedCasting.speakingRate} onValueChange={(v) => onUpdateCasting(selected, { speakingRate: v })} min={0.5} max={2.0} step={0.1} formatValue={(v) => `${v.toFixed(1)}x`}/>

              {/* Pitch */}
              <Slider className="mb-4" label={t('cast.pitch')} value={selectedCasting.pitch} onValueChange={(v) => onUpdateCasting(selected, { pitch: v })} min={-10} max={10} step={1} formatValue={(v) => String(v)}/>

              {/* Emotion */}
              <div className="mb-5">
                <label className="mb-1.5 block text-xs font-medium text-gray-600">{t('cast.emotionLabel')}</label>
                <input type="text" value={selectedCasting.emotion ?? ''} onChange={(e) => onUpdateCasting(selected, { emotion: e.target.value || undefined })} placeholder={t('cast.emotionPlaceholder')} className="w-full rounded-md border border-gray-200 px-3 py-2.5 text-sm text-gray-900 outline-none transition-colors placeholder:text-gray-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"/>
              </div>

              {/* Preview */}
              <button type="button" onClick={() => onPreviewVoice(selectedCasting)} disabled={previewPlaying !== null || loadingVoices} className="flex w-full items-center justify-center gap-2 rounded-md border border-gray-200 px-4 py-2.5 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-50 disabled:opacity-50">
            ...
            </div>) : selected ? (<div className="flex h-full items-center justify-center">
