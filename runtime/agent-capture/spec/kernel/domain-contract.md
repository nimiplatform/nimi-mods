...
- `generatedImage` 的默认正式产物形态应为角色锚点图，而不是任意镜头感角色图片。
...
- 风格要求默认应通过自然语言对话表达，而不是额外拆出独立风格输入项。
- 对话应优先把角色落实到人物主体相关的细节上，例如体态轮廓、服装、材质、配饰、手持道具、发型、色彩与画风。
- 除非用户明确要求，背景只作为辅助氛围存在，不应主导 brief 或生成方向。
...
- 当前结果替换策略优先于多候选保留策略。
- 默认正式图像结果应采用固定焦距倾向，避免夸张广角、强透视和戏剧性镜头语言。
- 默认正式图像结果应以人物全身完整入画、主体清晰、背景弱化、姿态稳定为优先，而不是以海报感或剧情瞬间为优先。
- 默认正式图像结果应优先清晰表达服装、材质、配饰、手持道具与画风，而不是把大量注意力放在环境场景叙事上。
...
- brief 应优先概括人物主体相关的视觉特征，例如轮廓、服装、材质、配饰、道具与画风；背景仅在其显著影响角色方向时才应进入 brief 核心。
...
- `sourceImage`、selected existing agent 背景、当前 `generatedImage`、以及针对当前结果的最新修正，都可成为当前生成上下文的组成部分。
