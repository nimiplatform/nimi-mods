...
`name`、`bio`、`personaSeed`、`tags`、`characterReadout` 的初始生成必须通过 `runtime.ai.text.generate` 完成。

**规则**:
- 文本初始生成与图像生成共同构成一次 `Generate Agent` 结果。
- 文本后续允许轻量人工编辑，但 Agent-Capture 不以表单手填作为默认入口。
- `characterReadout` 属于结果读取文本，不是新的 canonical truth 字段。
...
## AC-CAP-009 — existing agent 背景查询能力

Agent-Capture 若允许用户选择 existing agent 作为背景上下文，必须通过 SDK/host 暴露的稳定 data query surface 获取该 agent 背景。

**规则**:
- existing agent 背景读取必须通过 `data.query.data-api.creator.agents.list` 与 `data.query.data-api.creator.agents.get` 完成。
- 不得在 mod 内直接拼接 Realm REST 路径、调用未声明 raw request，或绕过 `hookClient.data.query(...)`。
- selected agent 背景可读取受控的 creator-scoped 语义字段，例如 `description`、`greeting`、有限 identity 信息与有限 appearance 线索，但这些字段只用于形成当前状态化辅助上下文，不得在 mod 内升级为新的 canonical truth 来源，也不得被直接当作必须兑现的最终生图约束。
