...
import { DiagnosticsPanel, ErrorBox, InfoBox, RawJsonSection, } from './components.js';
import { RouteSelect, type RouteSelectOption, } from './route-select.js';
import { type ModRuntimeClient, type ModRuntimeResolvedBinding, type RuntimeCanonicalCapability, type RuntimeRouteSource } from "@nimiplatform/sdk/mod";
    ...
    return (<div className="relative flex flex-col gap-5">
      <div className="grid gap-4 bg-[#f8fafc] lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <div className="flex min-h-[440px] flex-col overflow-hidden rounded-[28px] border border-gray-200 bg-white shadow-[0_18px_44px_rgba(15,23,42,0.06)]">
          {showRouteDialog ? (<div className="border-b border-gray-100 bg-[#fbfcfd] px-5 py-5">
              <div className="mb-5 flex items-start justify-between gap-4">
                ...
                <button type="button" onClick={() => setShowRouteDialog(false)} className="rounded-full border border-gray-200 p-2 text-gray-500 transition hover:border-gray-300 hover:text-gray-700" title={locale.common.collapse}>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <label className="flex flex-col gap-2 text-sm">
                  ...
                  <RouteSelect value={activeSource} options={sourceSelectOptions} onChange={(value) => applySource(value as RuntimeRouteSource)} disabled={!state.snapshot}/>
                </label>
                <label className="flex flex-col gap-2 text-sm">
                  <span className="text-gray-500">{locale.route.connector}</span>
                  <RouteSelect value={activeSource === 'cloud' ? activeConnectorId : ''} options={connectorSelectOptions} onChange={applyConnector} disabled={!state.snapshot || activeSource !== 'cloud'}/>
                </label>
                <label className="flex flex-col gap-2 text-sm">
                  <span className="text-gray-500">{locale.route.modelOverride}</span>
                  <RouteSelect value={activeModelInOptions ? activeModel : ''} options={modelSelectOptions} onChange={(value) => {
              ...
              <label className="mt-4 flex flex-col gap-2 text-sm">
                ...
                <input className="rounded-2xl border border-gray-200 bg-white px-3 py-3 outline-none focus:border-emerald-400" value={manualModelDraft} onChange={(event) => setManualModelDraft(event.target.value)} onBlur={() => applyManualModel(manualModelDraft)} placeholder={locale.route.modelIdPlaceholder}/>
              </label>

              <div className="mt-6 flex items-center justify-between gap-3">
                <button type="button" onClick={() => onStateChange((prev) => ({ ...prev, binding: null }))} className="rounded-full border border-gray-200 px-4 py-2 text-sm text-gray-600 transition hover:border-gray-300 hover:text-gray-900">
                  {locale.route.useDefaultRoute}
                </button>
                <div className="flex items-center gap-3">
                  <button type="button" onClick={onRouteReload} className="rounded-full border border-gray-200 px-4 py-2 text-sm text-gray-600 transition hover:border-gray-300 hover:text-gray-900">
          ...
          <textarea ref={textareaRef} className="min-h-[220px] flex-1 resize-none border-0 bg-transparent px-5 py-5 text-[17px] leading-8 text-gray-900 outline-none placeholder:text-gray-400" value={text} onChange={(event) => setText(event.target.value)} placeholder={locale.textEmbed.inputPlaceholder}/>

          <div className="border-t border-gray-100 bg-[#fbfcfd] px-4 py-3">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="relative" ref={modelMenuRef}>
                <button type="button" onClick={() => setShowModelMenu((prev) => !prev)} className="inline-flex items-center gap-2 rounded-full border border-gray-200 bg-white px-3 py-2 text-xs font-medium text-gray-600 transition hover:border-gray-300 hover:text-gray-900">
                ...
                {showModelMenu ? (<div className="absolute bottom-[calc(100%+10px)] left-0 z-30 w-72 rounded-[24px] border border-gray-200 bg-white p-2 shadow-[0_20px_50px_rgba(15,23,42,0.16)]">
                    <div className="mb-1 px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-gray-400">
                      {locale.route.routeModel}
                    </div>
                    {modelMenuOptions.map((model) => (<button key={model} type="button" onClick={() => {
                    ...
                    setShowModelMenu(false);
                        ...
                        {activeModel === model ? (<span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-semibold text-emerald-700">{locale.common.ready}</span>) : null}
                      </button>))}
              ...
              <button type="button" disabled={state.busy || !asString(text)} onClick={() => { void handleRun(); }} className="flex h-11 w-11 items-center justify-center rounded-full bg-[#111827] text-white transition hover:bg-[#1f2937] disabled:cursor-not-allowed disabled:bg-gray-200 disabled:text-gray-400" title={locale.textEmbed.run}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M5 12h14"/>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] text-gray-500">
              ...
              {activeSource === 'cloud' && activeConnectorId ? (<span className="rounded-full bg-gray-100 px-2.5 py-1 font-medium">
                  {locale.textEmbed.connector}: <span className="font-mono text-gray-700">{activeConnector?.label || activeConnectorId}</span>
                </span>) : null}
              {responseMeta?.modelResolved ? (<span className="rounded-full bg-emerald-50 px-2.5 py-1 font-medium text-emerald-700">
        ...
        <div className="flex min-h-[440px] flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-[linear-gradient(180deg,_rgba(248,250,252,0.96)_0%,_rgba(241,245,249,0.96)_100%)] shadow-[0_18px_44px_rgba(15,23,42,0.06)]">
          <div className="flex items-center justify-between border-b border-slate-200 bg-white/70 px-4 py-3">
            ...
            {embedOutput ? (<div className="flex items-center gap-2">
                ...
                <button type="button" onClick={handleCopyVector} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-medium text-slate-600 transition hover:border-slate-300 hover:text-slate-900">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 font-mono text-[13px] leading-7 text-slate-700">
            {embedOutput?.vectorText ? (<pre className="whitespace-pre-wrap break-all">{embedOutput.vectorText}</pre>) : state.busy ? (<div className="flex h-full flex-col items-center justify-center gap-3 text-center text-slate-400">
              ...
              </div>) : (<div className="flex h-full flex-col items-center justify-center gap-3 text-center text-slate-400">
                <div className="flex h-14 w-14 items-center justify-center rounded-full border border-slate-200 bg-white">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M4 7h16"/>
                    <path d="M4 12h16"/>
