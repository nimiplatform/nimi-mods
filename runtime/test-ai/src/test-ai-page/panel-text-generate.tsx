...
import { asString, bindingForConnector, bindingForModel, bindingForSource, makeEmptyDiagnostics, resolveEffectiveBinding, resolveRouteModelPickerState, toPrettyJson, useTestAiLocale, } from './core.js';
import type { CapabilityState } from './core.js';
import { DiagnosticsPanel, InfoBox, RawJsonSection, } from './components.js';
import { RouteSelect, type RouteSelectOption, } from './route-select.js';
import { type ModRuntimeClient, type ModRuntimeResolvedBinding, type RuntimeRouteSource } from "@nimiplatform/sdk/mod";
...
export function TextGeneratePanel(props: TextGeneratePanelProps) {
    ...
    const { effectiveBinding, activeSource, activeConnectorId, activeModel, modelOptions, cloudCatalogMissing, activeModelInOptions, } = resolveRouteModelPickerState(state.snapshot, state.binding);
    ...
    const titleHero = (<div className="mb-5 flex flex-col items-center text-center">
      <h1 className="text-[58px] font-black uppercase tracking-[0.08em] text-[#0f172a] sm:text-[72px]">
        {locale.textGenerate.heroTitle}
      </h1>
    </div>);
    const composer = (<div className="mx-auto w-full max-w-4xl">
      <div className="relative rounded-[34px] border border-gray-200 bg-white px-5 pb-14 pt-3 shadow-[0_20px_70px_rgba(15,23,42,0.08)] transition-all focus-within:border-[#4ECCA3]/50 focus-within:shadow-[0_20px_70px_rgba(78,204,163,0.16)]">
        <textarea ref={promptRef} className="min-h-[56px] w-full resize-none overflow-y-auto border-0 bg-transparent pr-2 text-[16px] leading-7 text-gray-900 outline-none placeholder:text-gray-400" value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder={locale.textGenerate.placeholder}/>
        ...
        <div className="absolute bottom-4 left-4 flex items-center gap-2">
          <div className="relative" ref={advancedRef}>
            ...
            setShowModelMenu(false);
        }} className="flex h-11 w-11 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-500 transition hover:border-gray-300 hover:text-gray-800" title={locale.textGenerate.advancedSettings}>
            ...
            {showAdvanced ? (<div className="absolute bottom-14 left-0 z-20 w-[360px] rounded-[26px] border border-gray-200 bg-white p-4 shadow-[0_22px_55px_rgba(15,23,42,0.16)]">
                ...
                <div className="space-y-4">
                  <label className="block">
                    ...
                    <textarea className="h-24 w-full resize-none rounded-2xl border border-gray-200 bg-gray-50 px-3 py-3 text-sm leading-6 text-gray-800 outline-none focus:border-[#4ECCA3] focus:bg-white" value={system} onChange={(event) => setSystem(event.target.value)} placeholder={locale.textGenerate.systemPromptPlaceholder}/>
                  </label>
                  <div>
                    <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-gray-400">{locale.textGenerate.temperature}</div>
                    <input type="range" min="0" max="2" step="0.1" value={temperatureValue} onChange={(event) => setTemperature(event.target.value)} className="h-2 w-full cursor-pointer appearance-none rounded-full bg-gray-200 accent-[#4ECCA3]"/>
                ...
                return (<button key={option} type="button" onClick={() => setMaxTokens(isAuto ? '' : option)} className={active
        ...
        <div className="absolute bottom-4 right-4 flex items-center gap-2">
          <div className="relative" ref={modelMenuRef}>
            ...
            {showModelMenu ? (<div className="absolute bottom-14 right-0 z-20 w-[320px] rounded-[26px] border border-gray-200 bg-white p-2 shadow-[0_22px_55px_rgba(15,23,42,0.16)]">
                ...
                return (<button key={model} type="button" onClick={() => selectModel(model)} className="flex w-full items-center justify-between rounded-2xl px-3 py-2.5 text-left text-sm text-gray-700 transition hover:bg-gray-50">
          ...
          <button type="button" disabled={state.busy || !asString(prompt)} onClick={() => { void handleRun(); }} className="flex h-11 w-11 items-center justify-center rounded-full bg-[#111827] text-white transition hover:bg-[#1f2937] disabled:cursor-not-allowed disabled:bg-gray-200 disabled:text-gray-400" title={locale.textGenerate.send}>
    ...
    return (<div className="test-ai-scroll-shell relative flex h-full min-h-[720px] flex-col overflow-y-auto overflow-x-hidden bg-[#f8fafc]">
      <style>{`
      ...
      <div className="mx-auto flex min-h-full w-full max-w-5xl flex-1 flex-col px-8 pb-10 pt-6">
        {isEmptyState ? (<div className="flex min-h-[calc(100vh-180px)] flex-1 items-center justify-center">
            ...
            <div className="mx-auto mb-4 flex w-full max-w-4xl justify-end">
              {state.output ? (<button type="button" onClick={() => setShowConversation((prev) => !prev)} className="inline-flex items-center gap-2 rounded-full border border-[#4ECCA3]/20 bg-[#4ECCA3]/10 px-4 py-2 text-sm text-[#2E8D73] transition hover:bg-[#4ECCA3]/14">
                  <span>{showConversation ? locale.common.hideConversation : locale.common.showConversation}</span>
                  <svg className={`h-4 w-4 transition-transform ${showConversation ? 'rotate-180' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="m6 9 6 6 6-6"/>
                  </svg>
                </button>) : null}
            </div>

            {showConversation && state.output ? (<div className="mx-auto mb-6 flex w-full max-w-4xl flex-col gap-5">
                ...
                <div className="rounded-[30px] border border-gray-200 bg-white px-6 py-5 shadow-[0_20px_45px_rgba(15,23,42,0.06)]">
                  <div className="mb-3 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-400">
                    ...
                    {state.result === 'passed' ? (<span className="rounded-full bg-green-100 px-2 py-0.5 text-[10px] tracking-normal text-green-700">{locale.common.ready}</span>) : null}
                  </div>
                  <div className="whitespace-pre-wrap text-[15px] leading-8 text-gray-800">{asString(state.output)}</div>
                </div>
              </div>) : null}

            {state.busy && !state.output ? (<div className="mx-auto mb-6 w-full max-w-4xl rounded-[28px] border border-gray-200 bg-white px-6 py-5 text-sm text-gray-600 shadow-[0_20px_45px_rgba(15,23,42,0.06)]">
            ...
            {state.error ? (<div className="mx-auto mb-6 w-full max-w-4xl rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                {state.error}
              </div>) : null}
            ...
            {state.busy && state.busyLabel === locale.textGenerate.warmingLocalModel ? (<div className="mt-4">
                <InfoBox message={locale.textGenerate.prewarmingNotice}/>
              </div>) : null}

            {(state.rawResponse || state.diagnostics.requestParams || state.diagnostics.resolvedRoute || state.diagnostics.responseMetadata) ? (<div className="mt-4 pb-8">
                <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white">
                  <button type="button" onClick={() => setShowDeveloperDetails((prev) => !prev)} className="flex w-full items-center gap-2 px-4 py-3 text-left text-sm font-medium text-gray-700">
                    <svg className={`h-4 w-4 transition-transform ${showDeveloperDetails ? 'rotate-90' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <path d="m9 6 6 6-6 6"/>
                    </svg>
                    <span>{locale.common.developerDetails}</span>
                  </button>
                  {showDeveloperDetails ? (<div className="border-t border-gray-200 p-4">
      ...
      {showRouteDialog ? (<div className="absolute inset-0 z-30 flex items-center justify-center bg-black/25 px-6">
          <div className="w-full max-w-2xl rounded-[30px] bg-white p-6 shadow-[0_30px_80px_rgba(15,23,42,0.2)]">
            <div className="mb-5 flex items-start justify-between gap-4">
              ...
              <button type="button" onClick={() => setShowRouteDialog(false)} className="rounded-full border border-gray-200 p-2 text-gray-500 transition hover:border-gray-300 hover:text-gray-700">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M18 6 6 18"/>
                  <path d="m6 6 12 12"/>
                </svg>
            ...
            {state.routeError ? (<div className="mb-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
                {state.routeError}
              </div>) : null}
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <label className="flex flex-col gap-2 text-sm">
                ...
                <RouteSelect value={activeSource} options={sourceOptions} onChange={(value) => applySource(value as RuntimeRouteSource)} disabled={!state.snapshot}/>
              </label>
              <label className="flex flex-col gap-2 text-sm">
                <span className="text-gray-500">{locale.route.connector}</span>
                <RouteSelect value={activeSource === 'cloud' ? activeConnectorId : ''} options={connectorOptions} onChange={applyConnector} disabled={!state.snapshot || activeSource !== 'cloud'}/>
              </label>
              <label className="flex flex-col gap-2 text-sm">
                <span className="text-gray-500">{locale.route.model}</span>
                <RouteSelect value={activeModelInOptions ? activeModel : ''} options={modelSelectOptions} onChange={applyManualModel} disabled={!state.snapshot || modelOptions.length === 0}/>
              </label>
            </div>
            ...
            <label className="mt-4 flex flex-col gap-2 text-sm">
              ...
              <input className="rounded-2xl border border-gray-200 bg-white px-3 py-3 outline-none focus:border-[#4ECCA3]" value={manualModelDraft} onChange={(event) => setManualModelDraft(event.target.value)} onBlur={() => applyManualModel(manualModelDraft)} placeholder={locale.route.modelIdPlaceholder}/>
            </label>

            <div className="mt-6 flex items-center justify-between">
              <button type="button" onClick={onRouteReload} disabled={state.routeLoading} className="rounded-full border border-gray-200 bg-white px-4 py-2 text-sm text-gray-700 transition hover:border-gray-300 hover:bg-gray-50 disabled:opacity-50">
                {state.routeLoading ? locale.common.refreshing : locale.common.refresh}
              </button>
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => onStateChange((prev) => ({ ...prev, binding: null }))} className="rounded-full border border-gray-200 bg-white px-4 py-2 text-sm text-gray-700 transition hover:border-gray-300 hover:bg-gray-50">
