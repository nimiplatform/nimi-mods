...
function CompanionPresetSelect(props: {
    ...
    return (<label key={preset.key} className="flex flex-col gap-1 text-xs">
    ...
    return (<>
      ...
      <div className="grid grid-cols-2 gap-2">
        <label className="flex flex-col gap-1 text-xs">
          ...
          <input className="rounded-md border border-gray-300 bg-white px-2 py-1 font-mono text-xs" value={draft.size} onChange={(event) => updateDraft({ size: event.target.value })} placeholder="1024x1024" list="test-ai-image-size-options"/>
      ...
      {isMediaImageWorkflow ? (<div className="rounded-xl border border-gray-200 bg-white p-3">
          ...
          {artifactLoading ? (<div className="rounded-md bg-blue-50 p-2 text-[11px] text-blue-700">
          ...
          <div className="rounded-lg border border-gray-100 bg-gray-50 p-3">
            ...
            <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
              {coreCompanionPresets.map((preset) => (<CompanionPresetSelect key={preset.key} preset={preset} draft={draft} artifactLoading={artifactLoading} presetArtifacts={companionPresetArtifacts[preset.key] || []} onChange={(value) => updateDraft({ [preset.key]: value } as Partial<ImageWorkflowDraftState>)}/>))}
            </div>
          </div>
          <div className="mt-3 rounded-lg border border-gray-100 bg-gray-50 p-3">
            ...
            <div className="grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-3">
              {extendedCompanionPresets.map((preset) => (<CompanionPresetSelect key={preset.key} preset={preset} draft={draft} artifactLoading={artifactLoading} presetArtifacts={companionPresetArtifacts[preset.key] || []} onChange={(value) => updateDraft({ [preset.key]: value } as Partial<ImageWorkflowDraftState>)}/>))}
            </div>
          </div>
          {!artifactLoading && !artifactError && !hasKnownCompanionArtifacts ? (<div className="mt-2 rounded-md bg-amber-50 p-2 text-[11px] text-amber-700">
      ...
      <details className="rounded-xl border border-gray-200 bg-gray-50 p-3 text-xs">
        ...
        <div className="mt-2 grid grid-cols-1 gap-2 md:grid-cols-2">
          <label className="flex max-w-xs flex-col gap-1 text-xs">
            ...
            <select className="rounded-md border border-gray-300 bg-white px-2 py-1 font-mono text-xs" value={draft.responseFormatMode} onChange={(event) => updateDraft({ responseFormatMode: event.target.value as ImageResponseFormatMode })}>
          ...
          <label className="flex max-w-xs flex-col gap-1 text-xs">
            ...
            <input className="rounded-md border border-gray-300 bg-white px-2 py-1 font-mono text-xs" value={draft.seed} onChange={(event) => updateDraft({ seed: event.target.value })} placeholder={locale.image.layerOptional}/>
            <span className="text-[11px] text-gray-400">
          ...
          <label className="flex max-w-xs flex-col gap-1 text-xs">
            ...
            <input className="rounded-md border border-gray-300 bg-white px-2 py-1 font-mono text-xs" value={draft.timeoutMs} onChange={(event) => updateDraft({ timeoutMs: event.target.value })} placeholder="600000"/>
            <span className="text-[11px] text-gray-400">
        ...
        {isMediaImageWorkflow ? (<div className="mt-3 rounded-lg border border-gray-200 bg-white p-3">
            <div className="flex items-center justify-between gap-2">
              ...
              <button type="button" className="rounded-md border border-gray-300 bg-white px-2 py-1 text-xs" onClick={onAddComponent}>
                {locale.common.addComponent}
              </button>
            </div>
            <datalist id="test-ai-image-workflow-slots">
              {COMMON_IMAGE_WORKFLOW_SLOTS.map((slot) => (<option key={slot} value={slot}/>))}
            </datalist>
            {artifactLoading ? (<div className="mt-2 rounded-md bg-blue-50 p-2 text-[11px] text-blue-700">
                {locale.image.loadingArtifacts}
              </div>) : null}
            {artifactError ? (<div className="mt-2 rounded-md bg-red-50 p-2 text-[11px] text-red-700">{artifactError}</div>) : null}
            {!artifactLoading && !artifactError && artifacts.length === 0 ? (<div className="mt-2 rounded-md bg-amber-50 p-2 text-[11px] text-amber-700">
                {locale.image.noInstalledArtifacts}
              </div>) : null}
            <div className="mt-3 flex flex-col gap-2">
              {draft.componentDrafts.length === 0 ? (<div className="rounded-md bg-gray-50 p-2 text-[11px] text-gray-500">
              ...
              {draft.componentDrafts.map((component) => {
                ...
                return (<div key={component.id} className="grid grid-cols-1 gap-2 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]">
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-gray-500">{locale.image.slot}</span>
                      <input className="rounded-md border border-gray-300 bg-white px-2 py-1 font-mono text-xs" value={component.slot} onChange={(event) => onComponentChange(component.id, 'slot', event.target.value)} list="test-ai-image-workflow-slots" placeholder="vae_path / llm_path / ..."/>
                    </label>
                    <label className="flex flex-col gap-1 text-xs">
                    ...
                    <div className="flex items-end">
                      <button type="button" className="rounded-md border border-gray-300 bg-white px-2 py-1 text-xs" onClick={() => onRemoveComponent(component.id)}>
            ...
            <div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-2">
              <label className="flex flex-col gap-1 text-xs">
                ...
                <input className="rounded-md border border-gray-300 bg-white px-2 py-1 font-mono text-xs" value={draft.step} onChange={(event) => updateDraft({ step: event.target.value })} placeholder="25"/>
              </label>
              <label className="flex flex-col gap-1 text-xs">
                <span className="text-gray-500">{locale.image.cfgScale}</span>
                <input className="rounded-md border border-gray-300 bg-white px-2 py-1 font-mono text-xs" value={draft.cfgScale} onChange={(event) => updateDraft({ cfgScale: event.target.value })} placeholder={locale.image.layerOptional}/>
              </label>
              <label className="flex flex-col gap-1 text-xs">
                <span className="text-gray-500">{locale.image.sampler}</span>
                <input className="rounded-md border border-gray-300 bg-white px-2 py-1 font-mono text-xs" value={draft.sampler} onChange={(event) => updateDraft({ sampler: event.target.value })} placeholder="euler / dpmpp2m / ..."/>
              </label>
              <label className="flex flex-col gap-1 text-xs">
                <span className="text-gray-500">{locale.image.scheduler}</span>
                <input className="rounded-md border border-gray-300 bg-white px-2 py-1 font-mono text-xs" value={draft.scheduler} onChange={(event) => updateDraft({ scheduler: event.target.value })} placeholder={locale.image.layerOptional}/>
              </label>
            </div>
            <label className="mt-3 flex flex-col gap-1 text-xs">
              <span className="text-gray-500">{locale.image.optionsPerLine}</span>
              <textarea className="h-20 resize-y rounded-md border border-gray-300 bg-white p-2 font-mono text-xs" value={draft.optionsText} onChange={(event) => updateDraft({ optionsText: event.target.value })} placeholder={'diffusion_model\noffload_params_to_cpu:true'}/>
            </label>
            <label className="mt-3 flex flex-col gap-1 text-xs">
              <span className="text-gray-500">{locale.image.rawProfileOverrides}</span>
              <textarea className="h-24 resize-y rounded-md border border-gray-300 bg-white p-2 font-mono text-xs" value={draft.rawProfileOverridesText} onChange={(event) => updateDraft({ rawProfileOverridesText: event.target.value })} placeholder={'{"clip_skip": 2}'}/>
              <span className="text-[11px] text-gray-400">
                {locale.image.rawProfileOverridesHint}
              </span>
            </label>
          </div>) : (<div className="mt-3 rounded-md bg-blue-50 p-2 text-[11px] text-blue-700">
