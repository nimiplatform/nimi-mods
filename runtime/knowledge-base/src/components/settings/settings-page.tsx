...
function NumberField(props: {
    ...
    return (<div className="flex flex-col gap-1.5">
      ...
      <input type="number" value={props.value} min={props.min} max={props.max} step={props.step ?? 1} onChange={(e) => props.onChange(Number(e.target.value))} className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs text-gray-900 focus:border-indigo-300 focus:outline-none focus:ring-1 focus:ring-indigo-300"/>
    </div>);
}
function SelectField(props: {
    ...
    return (<div className="flex flex-col gap-1.5">
      <FieldLabel>{props.label}</FieldLabel>
      <select value={props.value} onChange={(e) => props.onChange(e.target.value)} className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs text-gray-900 focus:border-indigo-300 focus:outline-none focus:ring-1 focus:ring-indigo-300">
...
function InputField(props: {
    ...
    return (<div className="flex flex-col gap-1.5">
      <FieldLabel>{props.label}</FieldLabel>
      <input value={props.value} list={props.listId} onChange={(e) => props.onChange(e.target.value)} placeholder={props.placeholder} className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs text-gray-900 placeholder:text-gray-400 focus:border-indigo-300 focus:outline-none focus:ring-1 focus:ring-indigo-300"/>
      {props.children}
    </div>);
}
function ToggleSwitch({ checked, onChange }: {
    ...
    return (<button type="button" role="switch" aria-checked={checked} onClick={() => onChange(!checked)} className={`relative inline-flex h-6 w-10 shrink-0 cursor-pointer rounded-full transition-colors ${checked ? 'bg-indigo-600' : 'bg-gray-200'}`}>
      <span className={`pointer-events-none inline-block h-[18px] w-[18px] translate-y-[3px] rounded-full bg-white shadow-sm transition-transform ${checked ? 'translate-x-[19px]' : 'translate-x-[3px]'}`}/>
    </button>);
}
// ---------------------------------------------------------------------------
...
function RoutePanel(props: {
    ...
    const selectedConnector = useMemo(() => {
        ...
        if (!id)
            return props.routeOptions?.connectors[0] || null;
    ...
    return (<div className="flex-1 rounded-lg bg-gray-50 p-4">
      ...
      <div className="flex flex-col gap-3">
        ...
        {props.source === 'cloud' && (<>
            ...
            <InputField label={t('settings.model')} value={props.model} placeholder={t('settings.modelPlaceholder')} listId={props.modelListId} onChange={props.onModelChange}>
              <datalist id={props.modelListId}>
                {cloudModels.map((m) => (<option key={`${props.title}-m-${m}`} value={m}/>))}
              </datalist>
            </InputField>
          </>)}
      ...
      <div className="flex flex-col gap-4 px-6 pb-6">
        ...
        <SettingsCard title={t('settings.routeTitle')} description={t('settings.routeDescription')}>
          <div className="mb-3 flex justify-end">
            {props.onRefreshRouteOptions && (<Button variant="secondary" size="sm" onClick={props.onRefreshRouteOptions}>
                {t('common.refresh')}
              </Button>)}
          </div>
