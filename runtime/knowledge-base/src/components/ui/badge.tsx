...
export function Badge({ children, className }: BadgeProps) {
    return (<span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium ${className ?? ''}`}>
      {children}
    </span>);
}
const STATUS_CONFIG: Record<KBDocumentStatus, {
    bg: string;
    text: string;
}> = {
...
function CheckCircleIcon() {
    return (<svg className="h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export function StatusBadge({ status }: {
    ...
    return (<Badge className={`${config.bg} ${config.text}`}>
      {status === 'ready' && <CheckCircleIcon />}
      {status === 'error' && <AlertCircleIcon />}
      {isProcessing && (<span className="inline-block h-1.5 w-1.5 animate-pulse rounded-full bg-current"/>)}
