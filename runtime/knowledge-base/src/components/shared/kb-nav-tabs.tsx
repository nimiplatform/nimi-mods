...
export function KBNavTabs(props: KBNavTabsProps) {
    ...
    return (<nav className="flex gap-1 bg-white px-4">
      {TABS.map((tab) => {
            const isActive = activeTab === tab.id;
            const count = tab.id === 'documents' ? documentCount
                : tab.id === 'chat' ? conversationCount
                    : undefined;
            return (<button key={tab.id} type="button" onClick={() => onTabChange(tab.id)} className={`inline-flex items-center gap-1.5 rounded-md px-3 py-2 text-xs font-medium transition-colors ${isActive
                    ? 'bg-indigo-50 text-indigo-600'
                    : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700'}`}>
            <tab.Icon />
            {t(`tabs.${tab.id}`)}
            {count !== undefined && count > 0 && (<span className={`ml-0.5 rounded-full px-1.5 py-0.5 text-[9px] font-semibold ${isActive ? 'bg-indigo-100 text-indigo-700' : 'bg-gray-100 text-gray-500'}`}>
                {count}
              </span>)}
          </button>);
        })}
