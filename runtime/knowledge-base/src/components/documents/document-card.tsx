...
function FileIcon({ sourceKind, mimeType }: { sourceKind: string; mimeType: string }) {
  if (sourceKind === 'url') {
    return (
...
export function DocumentCard(props: DocumentCardProps) {
  ...
  return (
    <div className={`flex items-center gap-4 rounded-lg border bg-white p-4 transition-colors hover:border-gray-300 ${
      ...
      <div className="min-w-0 flex-1">
        ...
        {doc.errorReason && (
          <p className="mt-1.5 text-[11px] text-red-500">{doc.errorReason}</p>
        )}
          ...
          <>
            ...
            <div className="absolute right-0 top-8 z-20 min-w-[120px] rounded-lg border border-gray-200 bg-white py-1 shadow-lg">
              {isError && onRetry && (
                <button
                  type="button"
                  onClick={() => { onRetry(doc.id); setMenuOpen(false); }}
