...
export function MessageList(props: MessageListProps) {
    ...
    return (<div ref={scrollRef} className="flex-1 overflow-y-auto px-6 py-5">
      <div className="flex flex-col gap-4">
        {turns.map((turn, idx) => {
            const isLastAssistant = idx === turns.length - 1 && turn.role === 'assistant';
            const isStreamingTurn = isLastAssistant && isSending;
            return (<MessageBubble key={turn.id} turn={turn} onCitationClick={onCitationClick} isStreaming={isStreamingTurn} streamingText={isStreamingTurn ? streamingText : undefined}/>);
        })}
      </div>
    </div>);
}
