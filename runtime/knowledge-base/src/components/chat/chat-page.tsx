...
function ChevronDownIcon({ className }: {
    ...
    return (<svg className={className ?? 'h-3.5 w-3.5'} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function MessageSquareIcon() {
    return (<svg className="h-12 w-12 text-gray-200" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round">
...
function QuickQuestion({ text, onClick }: {
    ...
    return (<button type="button" onClick={onClick} className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-left text-xs text-gray-600 transition-colors hover:border-indigo-200 hover:bg-indigo-50">
      {text}
    </button>);
}
    ...
    return (<div className="flex h-full min-h-0">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="border-b border-gray-100 bg-white">
          <button type="button" onClick={() => setRouteBannerOpen(!routeBannerOpen)} className="flex w-full items-center justify-between px-5 py-2 text-[11px] text-gray-500 hover:bg-gray-50">
          ...
          {routeBannerOpen && (<div className="border-t border-gray-100 bg-gray-50 px-5 py-3">
              ...
              <button type="button" onClick={() => { void controller.refreshRouteOptions(); }} className="mt-2 rounded-md border border-gray-200 bg-white px-2.5 py-1 text-[11px] font-medium text-gray-600 hover:bg-gray-50">
                {t('common.refresh')}
              </button>
            </div>)}
        </div>

        {!hasReadyDocs ? (<div className="flex h-full flex-col items-center justify-center">
          ...
          </div>) : !store.activeConversation ? (<div className="flex h-full flex-col">
            ...
            <ScopeSelector documents={store.documents} selectedDocIds={scopeDocIds} onChange={setScopeDocIds}/>
            <MessageList turns={turns} onCitationClick={(chunkId) => ui.setCitationPanelChunkId(chunkId)} isSending={ui.isSending} streamingText={ui.streamingText}/>
            {ui.error && (<div className="mx-5 mb-2 flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">
                <span className="flex-1">{ui.error}</span>
                <button type="button" onClick={() => ui.clearError()} className="shrink-0 text-red-400 hover:text-red-600">
                  ✕
                </button>
              </div>)}
            <CitationPanel citations={citations} activeCitationChunkId={ui.citationPanelChunkId} onClose={() => ui.setCitationPanelChunkId(null)}/>
