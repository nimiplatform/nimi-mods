// ---------------------------------------------------------------------------
// Data-API registrar — registers 8 data capabilities (SSOT §6)
// ---------------------------------------------------------------------------

import type { HookClient } from '@nimiplatform/sdk/mod/types';
import {
  KB_DATA_API_DOCUMENTS_LIST,
  KB_DATA_API_DOCUMENTS_IMPORT,
  KB_DATA_API_DOCUMENTS_DELETE,
  KB_DATA_API_SEARCH,
  KB_DATA_API_CONVERSATIONS_LIST,
  KB_DATA_API_CONVERSATIONS_GET,
  KB_DATA_API_CONVERSATIONS_UPSERT,
  KB_DATA_API_CONVERSATIONS_DELETE,
} from '../contracts.js';
...
export async function registerKBDataCapabilities(input: {
  ...
  await hookClient.data.register({
    ...
    handler: async (query) => {
      ...
      if (!params.query) return { chunks: [] };

      const store = useKnowledgeBaseStore.getState();
      // This is a simplified search path for cross-mod consumers
      // The full RAG pipeline is used internally by the chat controller
      const results = store.vectorStore.search(
        [], // Cross-mod consumers need to provide embedding externally
        params.topK ?? store.settings.topK,
        params.threshold ?? store.settings.similarityThreshold,
        params.documentIds,
      );
  ...
  await hookClient.data.register({
    ...
    handler: async (query) => {
      const { conversationId } = (query ?? {}) as { conversationId?: string };
  ...
  await hookClient.data.register({
    ...
    handler: async (query) => {
      const { conversation } = (query ?? {}) as { conversation?: Record<string, unknown> };
