...
export function StepInterview() {
  ...
  const lastFailedTurnRef = useRef<{ userText: string; userMessageId?: string } | null>(null);
  ...
  const handleSendMessage = useCallback(async (
    ...
    try {
      ...
      if (useMintYouStore.getState().currentRequestId !== requestId) return;
  ...
  const progressPercent = Math.min(
    100,
    Math.round((interviewValidTurnCount / MIN_VALID_TURNS) * 100),
  );
  ...
  return (
    <div className="ui-sync-card mx-auto my-4 flex h-full max-w-lg flex-col overflow-hidden">
      ...
      {/* Extension notice */}
      {extended && !isComplete && (
        <div className="bg-blue-50 px-4 py-1.5 text-xs text-blue-700">
      ...
      {isComplete ? (
        <div className="border-t border-gray-200 px-4 py-4">
          {degraded && (
            <p className="mb-2 text-xs text-amber-600">{t('Interview.degradedNotice')}</p>
          )}
          <button
        ...
        <>
          <InterviewInput
            disabled={isThinking || isTyping || isError}
