...
export function StepProgressBar({ currentStep, className = '' }: StepProgressBarProps) {
    ...
    return (<div className={`ui-sync-card flex w-full min-w-0 items-center gap-1 px-4 py-3 ${className}`}>
      {VISIBLE_STEPS.map((step, idx) => {
            ...
            {idx > 0 && (<div className={`h-px flex-1 ${isCompleted ? 'bg-[#4ECCA3]' : 'bg-gray-200'}`}/>)}
            <div className="flex flex-col items-center gap-1">
              <div className={`flex h-6 w-6 items-center justify-center rounded-full text-xs font-medium ${isCompleted
              ...
              <span className={`text-[10px] whitespace-nowrap ${isCurrent ? 'font-medium text-[#4ECCA3]' : 'text-gray-400'}`}>
                {t(step.labelKey)}
              </span>
            </div>
          </React.Fragment>);
