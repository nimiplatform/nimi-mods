...
function readPhotoTrustContext(): {
    ...
    const currentUserId = String(params.get('mint_you_current_user_id')
        || params.get('currentUserId')
        || '').trim();
    const otherUserId = String(params.get('mint_you_other_user_id')
...
export function StepResult() {
    ...
    if (loading) {
        return (<div className="ui-sync-card mx-auto my-4 flex max-w-lg flex-col items-center gap-4 p-8">
    ...
    if (createdAgentId) {
        ...
        return (<div className="ui-sync-card mx-auto my-4 flex max-w-lg flex-col items-center gap-6 p-8">
        ...
        {worldId && photoTrustReady && (<div className="w-full max-w-sm">
            <PhotoTrustPanel currentUserId={photoTrustContext.currentUserId} otherUserId={photoTrustContext.otherUserId} worldId={worldId} otherHasPhoto={photoTrustContext.otherHasPhoto}/>
          </div>)}
        {worldId && !photoTrustReady && (<p className="text-center text-xs text-gray-500">
