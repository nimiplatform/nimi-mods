...
export function StepConfirm() {
    ...
    const { basicInfo, traitResult, dnaSynthesis, selectedInterests, selfReportedMbti, traitOverrides, referenceImageUrl, worldId, error, loading, } = store;
    const { world: oasisWorld, loading: oasisLoading, error: oasisError } = useOasisWorldQuery();
    if (!basicInfo || !traitResult || !dnaSynthesis)
        return null;
    ...
    return (<div className="mx-auto my-4 max-w-lg space-y-4 p-4">
      ...
      <div>
        ...
        <input value={oasisWorldName} readOnly disabled={oasisLoading} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-[#4ECCA3] focus:outline-none focus:ring-1 focus:ring-[#4ECCA3]"/>
        <p className="mt-1 text-xs text-gray-500">{t('Confirm.worldFixedHint')}</p>
        {oasisError && (<p className="mt-1 text-xs text-red-600">{oasisError}</p>)}
      </div>
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button onClick={handleConfirm} disabled={loading || oasisLoading || !oasisWorldId} className="ui-sync-btn ui-sync-btn-primary flex-1 rounded-lg bg-[#4ECCA3] px-4 py-2.5 text-sm font-medium text-white hover:bg-[#3DBB92] disabled:opacity-50">
