...
import { areMintYouRouteBindingsEqual, sanitizeMintYouRouteBinding, } from '../route-binding.js';
...
export function MintYouSettingsDrawer({ open, onClose, }: MintYouSettingsDrawerProps) {
    ...
    const { routeOptions, loading, error, reloadRouteOptions, } = useMintYouRouteOptions();
    const sanitizedOverride = useMemo(() => sanitizeMintYouRouteBinding(routeBinding, routeOptions), [routeBinding, routeOptions]);
    ...
    const filteredModelOptions = useMemo(() => filterModelOptions(modelOptions, modelQuery), [modelOptions, modelQuery]);
    ...
    return (<>
      <div className={`absolute inset-0 z-20 bg-slate-900/12 backdrop-blur-[1px] transition-opacity duration-300 ${open ? 'opacity-100' : 'pointer-events-none opacity-0'}`} onClick={onClose} aria-hidden={!open}/>

      <aside className={`absolute inset-y-0 right-0 z-30 w-[360px] max-w-[92vw] border-l border-white/60 bg-[#f7fbfb] shadow-[-12px_0_28px_rgba(15,23,42,0.1)] transition-transform duration-300 ease-[cubic-bezier(0.2,0.7,0.2,1)] ${open ? 'translate-x-0' : 'translate-x-full pointer-events-none'}`} aria-hidden={!open}>
        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between border-b border-gray-200 px-4 py-4">
            ...
            <button type="button" onClick={onClose} className="ui-sync-btn ui-sync-btn-secondary inline-flex h-9 w-9 items-center justify-center rounded-full border text-gray-600" aria-label={t('SettingsDrawer.close')}>
          ...
          <div className="flex-1 space-y-4 overflow-y-auto px-4 py-4">
            ...
            <section className="ui-sync-card space-y-4 rounded-[24px] p-4">
              ...
              {error ? (<div className="rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
              ...
              <div>
                ...
                <select value={activeSource} onChange={(event) => handleSourceChange(event.target.value)} disabled={!routeOptions} className="h-10 w-full rounded-2xl border border-gray-200 bg-white px-3 text-sm text-gray-900 disabled:bg-gray-100 disabled:text-gray-400">
              ...
              <div>
                ...
                <select value={activeConnectorId} disabled={!routeOptions || activeSource !== 'cloud'} onChange={(event) => handleConnectorChange(event.target.value)} className="h-10 w-full rounded-2xl border border-gray-200 bg-white px-3 text-sm text-gray-900 disabled:bg-gray-100 disabled:text-gray-400">
                  <option value="">--</option>
                  {connectors.map((connector) => (<option key={`mint-you-settings-connector-${connector.id}`} value={connector.id}>
                      {connector.label || connector.id}
                    </option>))}
                </select>
                {activeSource === 'cloud' && connectors.length === 0 ? (<p className="mt-1 text-[11px] text-amber-700">{t('SettingsDrawer.noConnectors')}</p>) : null}
              </div>

              <div>
                <p className="mb-1.5 text-xs font-medium text-gray-500">{t('SettingsDrawer.model')}</p>
                <input list="mint-you-settings-model-list" value={modelQuery} onChange={(event) => {
            const nextValue = event.target.value;
            setModelQuery(nextValue);
            if (modelOptions.includes(nextValue)) {
                applyModel(nextValue);
            }
        }} onBlur={() => applyModel(modelQuery)} placeholder={t('SettingsDrawer.modelPlaceholder')} disabled={!routeOptions} className="h-10 w-full rounded-2xl border border-gray-200 bg-white px-3 text-sm text-gray-900 outline-none transition-colors focus:border-[#4ECCA3] focus:ring-1 focus:ring-[#4ECCA3] disabled:bg-gray-100 disabled:text-gray-400"/>
                <datalist id="mint-you-settings-model-list">
                  {filteredModelOptions.map((model) => (<option key={`mint-you-settings-model-${model}`} value={model}/>))}
                </datalist>
                {modelOptions.length === 0 ? (<p className="mt-1 text-[11px] text-amber-700">{t('SettingsDrawer.noModels')}</p>) : null}
                {modelOptions.length > 0 && filteredModelOptions.length === 0 ? (<p className="mt-1 text-[11px] text-amber-700">{t('SettingsDrawer.noMatchingModels')}</p>) : null}
              </div>

              <div className="flex gap-2">
                <button type="button" onClick={() => setRouteBinding(null)} className="ui-sync-btn ui-sync-btn-secondary h-10 flex-1 rounded-2xl border px-3 text-sm font-medium text-gray-700">
                  {t('SettingsDrawer.useRuntimeDefault')}
                </button>
                <button type="button" onClick={() => {
            void reloadRouteOptions({ forceRefresh: true });
        }} disabled={loading} className="ui-sync-btn ui-sync-btn-secondary h-10 rounded-2xl border px-3 text-sm font-medium text-gray-700 disabled:opacity-50">
