...
export function PersonaCard({ displayName, dnaPrimary, dnaSecondary, mbti, greeting, personalitySummary, formality, sentiment, relationshipMode, interests, referenceImageUrl, compact, }: PersonaCardProps) {
    const { t } = useModTranslation('mint-you');
    return (<div className="ui-sync-card rounded-xl border border-gray-200 bg-white p-4">
      {/* Header */}
      <div className="flex items-start gap-3">
        {/* Avatar area */}
        <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-gray-100">
          {referenceImageUrl ? (<img src={referenceImageUrl} alt={displayName} className="h-full w-full rounded-full object-cover"/>) : (<span className="text-xl text-gray-400">{displayName.charAt(0).toUpperCase()}</span>)}
        </div>
      ...
      <div className="mt-3 flex flex-wrap gap-1">
        {dnaSecondary.map((trait) => (<span key={trait} className="rounded-full border border-gray-200 px-2 py-0.5 text-xs text-gray-600">
            {trait}
          </span>))}
      </div>
      ...
      {!compact && (<>
          ...
          {/* Interests */}
          {interests.length > 0 && (<div className="mt-3 flex flex-wrap gap-1">
              {interests.map((tag) => (<span key={tag} className="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-500">
                  {tag}
                </span>))}
            </div>)}
        </>)}
