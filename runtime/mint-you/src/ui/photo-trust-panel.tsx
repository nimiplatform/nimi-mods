...
import { readPhotoAuthSnapshot, requestPhoto, respondToRequest, revokeAccess, } from '../services/photo-auth.js';
...
export function PhotoTrustPanel({ currentUserId, otherUserId, worldId, otherHasPhoto, }: PhotoTrustPanelProps) {
    ...
    useEffect(() => {
        if (snapshot.state !== 'DECLINED' || snapshot.cooldownRemainingMs <= 0)
            return;
    ...
    return (<div className="rounded-lg border border-gray-200 p-3">
      ...
      {/* State badge */}
      <div className="mb-2">
        <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${authState === 'MUTUAL' ? 'bg-green-100 text-green-700'
      ...
      {!otherHasPhoto && (<p className="text-xs text-gray-400">{t('PhotoTrust.noPhoto')}</p>)}

      {otherHasPhoto && authState === 'NONE' && (<button onClick={handleRequest} disabled={!requestAllowed || busy} className="rounded-lg bg-[#4ECCA3] px-3 py-1.5 text-xs font-medium text-white hover:bg-[#3DBB92] disabled:opacity-50">
          {t('PhotoTrust.requestReveal')}
        </button>)}

      {isPendingOutgoing && (<p className="text-xs text-gray-500">{t('PhotoTrust.pendingRequest')}</p>)}

      {isPendingIncoming && (<div className="space-y-2">
          <p className="text-xs text-gray-500">{t('PhotoTrust.pendingRequest')}</p>
          <div className="flex gap-2">
            <button onClick={() => handleRespond(true)} disabled={busy} className="rounded-lg bg-[#4ECCA3] px-3 py-1.5 text-xs font-medium text-white hover:bg-[#3DBB92]">
              {t('PhotoTrust.accept')}
            </button>
            <button onClick={() => handleRespond(false)} disabled={busy} className="rounded-lg border border-gray-300 px-3 py-1.5 text-xs text-gray-600 hover:bg-gray-50">
              {t('PhotoTrust.decline')}
            </button>
          </div>
        </div>)}

      {authState === 'MUTUAL' && (<div className="space-y-2">
          <p className="text-xs text-green-600">{t('PhotoTrust.mutualAccess')}</p>
          <button onClick={handleRevoke} disabled={busy} className="rounded-lg border border-red-200 px-3 py-1.5 text-xs text-red-600 hover:bg-red-50">
            {t('PhotoTrust.revoke')}
          </button>
        </div>)}

      {authState === 'DECLINED' && cooldown > 0 && (<p className="text-xs text-gray-400">
          {t('PhotoTrust.cooldownActive', { time: formatCooldown(cooldown) })}
        </p>)}

      {authState === 'DECLINED' && cooldown <= 0 && (<button onClick={handleRequest} disabled={busy} className="rounded-lg bg-[#4ECCA3] px-3 py-1.5 text-xs font-medium text-white hover:bg-[#3DBB92]">
