...
import { INTEREST_CATEGORIES, getTagsByCategory, } from '../data/interest-tags.js';
import { MBTI_VALUES, MINTYOU_AUDIT, SOCIAL_PROFILE_LIMITS, type MbtiValue, } from '../contracts.js';
...
function SummaryStat(props: {
    ...
    return (<div className="ui-sync-soft-card rounded-2xl px-4 py-3">
...
export function StepInterestTags() {
    ...
    const selectedInterestLabels = useMemo(() => selectedInterests.map((tagId) => t(`InterestTags.tag.${tagId}`)), [selectedInterests, t]);
    const canProceed = (selectedInterests.length >= SOCIAL_PROFILE_LIMITS.minInterests
    ...
    return (<div className="mx-auto my-4 max-w-5xl space-y-4 px-4 pb-6">
        ...
        <div className="relative grid gap-5 md:grid-cols-[1.45fr_0.85fr] md:items-start">
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {selectedInterestLabels.length > 0 ? selectedInterestLabels.map((label) => (<span key={label} className="rounded-full border border-[#b5efe0] bg-[#edfffa] px-3 py-1 text-xs font-medium text-[#0f766e]">
                  {label}
                </span>)) : (<span className="text-sm text-gray-400">{t('InterestTags.selectedEmpty')}</span>)}
            </div>
          </div>

          <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-1">
            <SummaryStat label={t('InterestTags.summary.interests')} value={t('InterestTags.summary.interestsValue', {
            count: selectedInterests.length,
            max: SOCIAL_PROFILE_LIMITS.maxInterests,
        })}/>
            <SummaryStat label={t('InterestTags.summary.mbti')} value={selfReportedMbti || t('InterestTags.summary.mbtiUnset')}/>
          </div>
        </div>
      </section>
      ...
      <section className="ui-sync-card space-y-4 p-5">
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          {INTEREST_CATEGORIES.map((category) => (<section key={category} className="ui-sync-soft-card rounded-[24px] p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                return (<button key={tag.id} type="button" onClick={() => toggleInterest(tag.id)} disabled={disabled} className={`ui-sync-btn rounded-full border px-3 py-2 text-sm font-medium transition-colors ${active
                        ? 'ui-sync-btn-primary border-[#4ECCA3] bg-[#4ECCA3] text-white'
                        : 'ui-sync-btn-secondary border-gray-200 text-gray-700 hover:border-[#8fe6d1]'} ${disabled ? 'cursor-not-allowed opacity-40' : ''}`}>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_0.95fr]">
        <section className="ui-sync-card space-y-4 p-5">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {MBTI_VALUES.map((value) => (<button key={value} type="button" onClick={() => setSelfReportedMbti((current) => current === value ? null : value)} className={`ui-sync-btn rounded-2xl border px-3 py-2.5 text-sm font-semibold transition-colors ${selfReportedMbti === value
                ? 'ui-sync-btn-primary border-[#4ECCA3] bg-[#4ECCA3] text-white'
          ...
          <div className="flex items-center justify-between rounded-2xl border border-dashed border-gray-200 bg-gray-50/80 px-4 py-3">
            ...
            <button type="button" onClick={() => setSelfReportedMbti(null)} className="ui-sync-btn ui-sync-btn-ghost rounded-full border px-3 py-1.5 text-xs font-semibold">
        ...
        <section className="ui-sync-card space-y-4 p-5">
          ...
          <div className="ui-sync-soft-card rounded-2xl p-3">
            <input value={currentFocus} onChange={(event) => setCurrentFocus(event.target.value.slice(0, SOCIAL_PROFILE_LIMITS.currentFocusMaxLength))} placeholder={t('InterestTags.currentFocusPlaceholder')} className="h-12 w-full border-none bg-transparent px-0 py-0 text-sm text-gray-800 shadow-none focus:ring-0"/>
      ...
      {errors.length > 0 ? (<div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3">
          {errors.map((error, index) => (<p key={`${error}-${index}`} className="text-sm text-red-600">{error}</p>))}
        </div>) : null}

      <div className="flex gap-3 pt-1">
        <button type="button" onClick={() => store.goBack()} className="ui-sync-btn ui-sync-btn-secondary rounded-2xl border border-gray-300 px-5 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50">
          {t('Common.back')}
        </button>
        <button type="button" onClick={handleSubmit} disabled={!canProceed} className="ui-sync-btn ui-sync-btn-primary flex-1 rounded-2xl bg-[#4ECCA3] px-5 py-3 text-sm font-semibold text-white hover:bg-[#3DBB92] disabled:opacity-50">
          {t('InterestTags.next')}
        </button>
      </div>
    </div>);
