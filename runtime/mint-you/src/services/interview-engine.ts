import { z } from 'zod';
import { PRIMARY_ARCHETYPES, SECONDARY_TRAITS, RELATIONSHIP_MODES, FORMALITY_VALUES, SENTIMENT_VALUES, MINTYOU_REASON, } from '../contracts.js';
import { InterviewTurnOutputSchema } from '../schemas.js';
import type { InterviewMessage, InterviewTurnSignal, InterviewTurnOutput, InterviewStatus, MintYouResult, } from '../types.js';
...
for (const k of PRIMARY_ARCHETYPES)
    VALID_SIGNAL_KEYS.add(`primary.${k}`);
for (const k of SECONDARY_TRAITS)
    VALID_SIGNAL_KEYS.add(`secondary.${k}`);
for (const k of RELATIONSHIP_MODES)
    VALID_SIGNAL_KEYS.add(`relationship.${k}`);
for (const k of FORMALITY_VALUES)
    VALID_SIGNAL_KEYS.add(`communication.formality.${k}`);
for (const k of SENTIMENT_VALUES)
    VALID_SIGNAL_KEYS.add(`communication.sentiment.${k}`);
...
function computeCoverage(signals: InterviewTurnSignal[]): Set<CoverageGroup> {
    ...
    for (const s of signals) {
        ...
        if (parts[0] === 'primary')
            covered.add('primary');
        else if (parts[0] === 'secondary')
            covered.add('secondary');
        else if (parts[0] === 'relationship')
            covered.add('relationship');
        else if (parts[0] === 'communication' && parts[1] === 'formality')
            covered.add('formality');
        else if (parts[0] === 'communication' && parts[1] === 'sentiment')
            covered.add('sentiment');
    }
    return covered;
}
function buildCoverageNote(covered: Set<CoverageGroup>): string {
    ...
    if (!covered.has('primary'))
        uncovered.push('their core personality archetype');
    if (!covered.has('relationship'))
        uncovered.push('how they connect with people');
    if (!covered.has('formality'))
        uncovered.push('their communication style');
    if (!covered.has('sentiment'))
        uncovered.push('their general outlook on life');
    if (!covered.has('secondary'))
        uncovered.push('their secondary personality quirks');
...
function buildSystemPrompt(input: {
    ...
    const { interests, signals, turnCount, validTurnCount, currentFocus, language, } = input;
...
// ── Signal filtering ──
function filterSignals(raw: Array<{
    key: string;
    weight: number;
    evidence: string;
}>, turnIndex: number, messageId: string): InterviewTurnSignal[] {
    const result: InterviewTurnSignal[] = [];
    for (const s of raw) {
        if (!VALID_SIGNAL_KEYS.has(s.key))
            continue;
        if (!VALID_WEIGHTS.has(s.weight))
            continue;
        result.push({
            turnIndex,
            messageId,
...
export async function processInterviewTurn(input: InterviewTurnInput): Promise<MintYouResult<InterviewTurnResult>> {
    const { runtimeClient, userMessage, userMessageId, messages, signals, memoryDigest, turnCount, validTurnCount, interests, currentFocus, language, binding, } = input;
    ...
    for (let attempt = 0; attempt < MAX_TURN_ATTEMPTS; attempt++) {
        try {
            ...
            if (!validation.success) {
                ...
                if (attempt < MAX_TURN_ATTEMPTS - 1)
                    continue; // retry
                return {
                    ok: false,
                    error: {
                        reasonCode: MINTYOU_REASON.INTERVIEW_TURN_FAILED,
                        message: mintYouMessage('Messages.interviewValidationFailed', 'Interview turn validation failed: {{issues}}', { issues }),
                        actionHint: mintYouMessage('Messages.interviewRetryHint', 'Try sending your message again.'),
                    },
                };
            }
            ...
            return {
                ...
                error: {
                    reasonCode: MINTYOU_REASON.INTERVIEW_TURN_FAILED,
                    message: mintYouMessage('Messages.interviewTurnFailed', 'Interview turn failed: {{detail}}', { detail: msg }),
                    actionHint: mintYouMessage('Messages.interviewRetryHint', 'Try sending your message again.'),
                },
            };
        }
    ...
    return {
        ...
        error: {
            reasonCode: MINTYOU_REASON.INTERVIEW_TURN_FAILED,
            message: mintYouMessage('Messages.interviewTurnFailedAfterRetries', 'Interview turn failed after retries.'),
            actionHint: mintYouMessage('Messages.interviewRetryHint', 'Try sending your message again.'),
        },
    };
}
