...
import { loadSession, saveSession, clearSession, isSessionExpired, buildSessionSnapshot, } from '../services/session-manager.js';
        ...
        if (existing) {
            if (isSessionExpired(existing)) {
                ...
                store.setError({
                    ...
                    message: mintYouMessage('Messages.sessionExpired', 'Session data expired after 7 days of inactivity.'),
                ...
                const resumeStep = existing.currentStep === 'agent-create' && !existing.createdAgentId
                    ? 'user-confirm'
                    : existing.currentStep;
                store.goToStep(resumeStep);
                if (existing.basicInfo)
                    store.setBasicInfo(existing.basicInfo);
                if (existing.selectedInterests.length > 0
                ...
                if (existing.traitResult)
                    store.setTraitResult(existing.traitResult);
                if (existing.dnaSynthesis)
                    store.setDnaSynthesis(existing.dnaSynthesis);
                if (existing.traitOverrides)
                    store.setTraitOverrides(existing.traitOverrides);
                if (existing.referenceImageUrl)
                    store.setReferenceImageUrl(existing.referenceImageUrl);
                if (existing.worldId)
                    store.setWorldId(existing.worldId);
                if (existing.createdAgentId)
                    store.setCreatedAgentId(existing.createdAgentId);
                emitMintYouLog({
