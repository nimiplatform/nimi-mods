...
import { CORE_DATA_API_FRIENDS_WITH_DETAILS_LIST, requireLocalChatCoreQueryBridge, } from './core-query-bridge.js';
import { getTargetsListCache, getTargetsListInFlight, setTargetsListCache, setTargetsListInFlight, } from './cache-store.js';
...
function inferAgentFlag(source: Record<string, unknown>): boolean | null {
    if (source.isAgent === true)
        return true;
    if (source.isAgent === false)
        return false;
    if (hasAgentProfileSignal(source))
        return true;
...
function readAgentWorldId(friend: Record<string, unknown>): string | null {
    ...
    if (fromRoot)
        return fromRoot;
    const metadata = asRecord(friend.agent && typeof friend.agent === 'object'
        ? friend.agent
...
function toBaseTargetFromFriend(friend: Record<string, unknown>): LocalChatTarget | null {
    ...
    const agentMetadata = asRecord(friend.agent && typeof friend.agent === 'object'
        ? friend.agent
        : friend.agentMetadata);
...
export async function listLocalChatTargets(context: LocalChatReadContext): Promise<LocalChatTarget[]> {
    ...
    const task = (async () => {
        ...
        try {
            const targets = await withReadContext(context, async () => {
                const payload = await requireLocalChatCoreQueryBridge().query(CORE_DATA_API_FRIENDS_WITH_DETAILS_LIST, {});
