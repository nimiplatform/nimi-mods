import { filterModelsForScenario, filterModelsForSpeechSynthesis } from "@nimiplatform/sdk/mod";
type LegacyScenario = 'chat' | 'tts' | 'stt';
type LegacyExtendedScenario = LegacyScenario | 'image' | 'video';
type CanonicalScenario = 'text.generate' | 'audio.synthesize' | 'audio.transcribe' | 'image.generate' | 'video.generate';
...
function capabilitiesForModel(modelCapabilities: Record<string, string[]> | undefined, modelId: string): string[] {
    ...
    if (!normalizedModelId)
        return [];
    const entry = Object.entries(modelCapabilities).find(([candidate]) => String(candidate || '').trim().toLowerCase() === normalizedModelId);
    if (!entry)
        return [];
...
function matchesScenarioByCapability(capabilities: string[], scenario: ExtendedScenario): boolean {
    ...
    if (normalizedScenario === 'chat') {
        return hasAny('chat', 'text', 'completion', 'text.generate');
    }
    if (normalizedScenario === 'tts') {
        return hasAny('tts', 'audio.synthesize', 'speech.synthesize');
    }
    if (normalizedScenario === 'stt') {
        return hasAny('stt', 'audio.transcribe', 'speech.transcribe');
    }
    if (normalizedScenario === 'image') {
        return hasAny('image', 't2i', 'i2i', 'image.generate');
    }
    return hasAny('video', 't2v', 'video.generate', 'text.video');
}
...
export function resolveModelsForScenario(input: {
    ...
    if (allModels.length === 0)
        return [];
    const matchedByCapabilities = allModels.filter((modelId) => (matchesScenarioByCapability(capabilitiesForModel(input.modelCapabilities, modelId), input.scenario)));
...
function matchesLocalRuntimeModelByHeuristic(model: LocalRuntimeModelLike, scenario: ExtendedScenario): boolean {
    const candidates = filterModelsByHeuristic([String(model.model || '').trim(), String(model.localModelId || '').trim()].filter(Boolean), scenario);
    return candidates.length > 0;
}
export function isLocalRuntimeModelReady(model: Pick<LocalRuntimeModelLike, 'status' | 'goRuntimeStatus'> | null | undefined): boolean {
    if (!model) {
        return false;
    }
...
export function findLocalRuntimeModelForBinding<T extends LocalRuntimeModelLike>(input: {
    ...
    if (localModelId) {
        const byLocalModelId = input.models.find((candidate) => (normalizeLocalRuntimeLookup(candidate.localModelId) === localModelId));
        if (byLocalModelId) {
            return byLocalModelId;
        }
    }
    if (goRuntimeLocalModelId) {
        const byGoRuntimeLocalModelId = input.models.find((candidate) => (normalizeLocalRuntimeLookup(candidate.goRuntimeLocalModelId) === goRuntimeLocalModelId));
    ...
    return input.models.find((candidate) => (normalizeLocalRuntimeLookup(candidate.model) === model
        || normalizeLocalRuntimeLookup(candidate.localModelId) === model)) || null;
}
export function resolveLocalRuntimeModelsForScenario<T extends LocalRuntimeModelLike>(input: {
    ...
    const matchedByCapabilities = allModels.filter((model) => (matchesScenarioByCapability(model.capabilities || [], input.scenario)));
    if (matchedByCapabilities.length > 0) {
        return matchedByCapabilities;
    }
    const matchedByHeuristic = allModels.filter((model) => (matchesLocalRuntimeModelByHeuristic(model, input.scenario)));
    if (matchedByHeuristic.length > 0) {
        return matchedByHeuristic;
    }
    return allModels;
