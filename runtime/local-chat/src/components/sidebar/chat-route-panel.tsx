...
import { useModTranslation, normalizeRuntimeRouteSource, type RuntimeRouteBinding, type RuntimeRouteOptionsSnapshot } from "@nimiplatform/sdk/mod";
type Props = {
    open: boolean;
    onToggle: () => void;
...
export function ChatRoutePanel(props: Props) {
    ...
    const { open, onToggle, activeChatSource, activeChatConnectorId, activeChatModel, chatRouteOptions, chatModelQuery, setChatModelQuery, chatModelOptions, filteredChatModelOptions, onRouteSourceChange, onRouteConnectorChange, onRouteModelChange, onClearRouteBinding, } = props;
      ...
      {open ? (<div className="mt-3">
          ...
          <div className="space-y-2">
            <div>
              ...
              <select value={activeChatSource} onChange={(event) => onRouteSourceChange(normalizeRuntimeRouteSource(event.target.value))} className="h-8 w-full rounded-xl border border-gray-200 bg-white px-2 text-xs text-gray-900">
            ...
            <div>
              ...
              <select value={activeChatConnectorId} disabled={activeChatSource !== 'cloud'} onChange={(event) => onRouteConnectorChange(event.target.value)} className="h-8 w-full rounded-xl border border-gray-200 bg-white px-2 text-xs text-gray-900 disabled:bg-gray-100 disabled:text-gray-400">
                <option value="">--</option>
                {(chatRouteOptions?.connectors || []).map((connector) => (<option key={`route-connector-${connector.id}`} value={connector.id}>{connector.label || connector.id}</option>))}
              </select>
            </div>

            <div>
              <p className="mb-1 text-gray-500">{t('ChatRoute.model')}</p>
              <input list="local-chat-chat-model-list" value={chatModelQuery} onChange={(event) => {
              ...
              <datalist id="local-chat-chat-model-list">
                {filteredChatModelOptions.map((model) => (<option key={`route-model-${model}`} value={model}/>))}
              </datalist>
              ...
              {chatModelOptions.length > 0 && filteredChatModelOptions.length === 0 ? (<p className="mt-1 text-[11px] text-amber-700">{t('ChatRoute.noMatchingModels')}</p>) : null}
            </div>

            {showEmptyLocalCta ? (<div className="rounded-lg border border-amber-200 bg-amber-50 p-2 space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" className="lc-btn lc-btn-warning h-7 px-2 text-[11px] font-medium" onClick={() => onRouteSourceChange('cloud')}>
                    {t('ChatRoute.switchToCloud')}
                  </button>
                </div>
              </div>) : null}

            <button type="button" onClick={onClearRouteBinding} className="lc-btn lc-btn-secondary h-9 w-full px-2 text-xs font-semibold">
