...
function localMediaModels(options: RuntimeStatusSidebarProps['imageRouteOptions'] | RuntimeStatusSidebarProps['videoRouteOptions'], scenario: 'image.generate' | 'video.generate'): string[] {
...
export function MediaRoutePanel(props: Props) {
    ...
    const imageLocalModels = useMemo(() => localMediaModels(props.imageRouteOptions, 'image.generate'), [props.imageRouteOptions]);
    const videoLocalModels = useMemo(() => localMediaModels(props.videoRouteOptions, 'video.generate'), [props.videoRouteOptions]);
    ...
    const fullyAutoWithoutOverrides = (props.imageRouteSource === 'auto'
    ...
    const content = (<div className="min-h-0 space-y-4 lc-panel-expand">
          {props.loading ? (<p className="rounded-lg border border-sky-100 bg-sky-50 px-2 py-1.5 text-[11px] text-slate-600">
              {t('MediaRoute.loadingHint')}
            </p>) : null}
          {showSkeleton ? (<div className="space-y-3">
              {[0, 1].map((index) => (<div key={`media-route-skeleton-${index}`} className="rounded-xl border border-gray-200 bg-white p-2">
          ...
          {imageConnectorMissing || videoConnectorMissing ? (<p className="rounded-lg border border-amber-200 bg-amber-50 px-2 py-1.5 text-[11px] text-amber-700">
              {t('MediaRoute.connectorRequiredHint')}
            </p>) : null}
          {fullyAutoWithoutOverrides ? (<p className="rounded-lg border border-gray-200 bg-gray-50 px-2 py-1.5 text-[11px] text-gray-600">
          ...
          <div className="space-y-2 rounded-xl border border-gray-200 bg-white p-2">
            ...
            <select value={props.imageRouteSource} onChange={(event) => props.onImageRouteSourceChange(event.target.value as 'auto' | 'local' | 'cloud')} className="h-8 w-full rounded-lg border border-gray-200 bg-white px-2 text-xs text-gray-900">
            ...
            <select value={props.imageConnectorId} disabled={props.imageRouteSource !== 'cloud'} onChange={(event) => props.onImageConnectorChange(event.target.value)} className="h-8 w-full rounded-lg border border-gray-200 bg-white px-2 text-xs text-gray-900 disabled:bg-gray-100 disabled:text-gray-400">
              <option value="">--</option>
              {props.imageConnectors.map((connector) => (<option key={`image-connector-${connector.id}`} value={connector.id}>{connector.label || connector.id}</option>))}
            </select>
            <input list="local-chat-image-model-list" value={props.imageModel} onChange={(event) => props.onImageModelChange(event.target.value)} placeholder={t('MediaRoute.modelPlaceholder')} className="h-8 w-full rounded-lg border border-gray-200 bg-white px-2 text-xs text-gray-900 outline-none transition-colors focus:border-mint-500 focus:ring-1 focus:ring-mint-500"/>
            <datalist id="local-chat-image-model-list">
              {imageModelOptions.map((model) => (<option key={`image-model-${model}`} value={model}/>))}
            </datalist>
          </div>
          ...
          <div className="space-y-2 rounded-xl border border-gray-200 bg-white p-2">
            ...
            <select value={props.videoRouteSource} onChange={(event) => props.onVideoRouteSourceChange(event.target.value as 'auto' | 'local' | 'cloud')} className="h-8 w-full rounded-lg border border-gray-200 bg-white px-2 text-xs text-gray-900">
              <option value="auto">{t('MediaRoute.auto')}</option>
              <option value="local">{t('MediaRoute.local')}</option>
              <option value="cloud">{t('MediaRoute.cloud')}</option>
            </select>
            <select value={props.videoConnectorId} disabled={props.videoRouteSource !== 'cloud'} onChange={(event) => props.onVideoConnectorChange(event.target.value)} className="h-8 w-full rounded-lg border border-gray-200 bg-white px-2 text-xs text-gray-900 disabled:bg-gray-100 disabled:text-gray-400">
              <option value="">--</option>
              {props.videoConnectors.map((connector) => (<option key={`video-connector-${connector.id}`} value={connector.id}>{connector.label || connector.id}</option>))}
            </select>
            <input list="local-chat-video-model-list" value={props.videoModel} onChange={(event) => props.onVideoModelChange(event.target.value)} placeholder={t('MediaRoute.modelPlaceholder')} className="h-8 w-full rounded-lg border border-gray-200 bg-white px-2 text-xs text-gray-900 outline-none transition-colors focus:border-mint-500 focus:ring-1 focus:ring-mint-500"/>
            <datalist id="local-chat-video-model-list">
              {videoModelOptions.map((model) => (<option key={`video-model-${model}`} value={model}/>))}
            </datalist>
          </div>
            </>)}
          </div>);
    ...
    return (<div className="lc-card rounded-2xl p-3 text-xs">
      <button type="button" onClick={props.onToggle} aria-expanded={props.open} className="flex h-7 w-full items-center justify-between text-left text-[13px] font-semibold text-gray-700">
