...
export function resolveStageConversationSlice(input: {
    ...
    const shouldShowPendingUserTurn = (input.sendPhase === 'awaiting-first-beat' || input.sendPhase === 'streaming-first-beat') && lastMessage?.role === 'user';
    ...
    const assistantMessages = messages.filter((message) => message.role === 'assistant' && String(message.meta?.turnId || '').trim() === latestAssistantTurnId);
...
export const StageDialogueCard = React.memo(function StageDialogueCard({ agentAvatarUrl, agentName, theme: _theme, currentUserDisplayName, currentUserAvatarUrl, messages, sendPhase, playingVoiceMessageId, voiceTranscriptVisibleById, onPlayVoiceMessage, onVoiceContextMenu, messagesEndRef, scrollRootRef, }: StageDialogueCardProps) {
    const { t } = useModTranslation('local-chat');
    const { userMessage, assistantMessages, pendingFirstBeat } = React.useMemo(() => resolveStageConversationSlice({
        messages,
        sendPhase,
    }), [messages, sendPhase]);
    ...
    return (<div className="lc-stage-dialogue-shell w-full rounded-[30px] border border-emerald-100/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.97),rgba(248,250,252,0.92))] p-4 shadow-[0_24px_60px_rgba(15,23,42,0.08)] backdrop-blur-xl" style={shellStyle}>
      ...
      <div ref={scrollRootRef} data-local-chat-scroll-root="true" className="max-h-[44vh] overflow-y-auto overscroll-contain rounded-[24px] border border-slate-200/75 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(248,250,252,0.95))] px-4 py-4 backdrop-blur-sm">
        {showEmptyState ? (<div className="flex min-h-[180px] flex-col items-center justify-center gap-3 px-6 text-center">
            ...
            {userMessage ? (<ChatBubble message={userMessage} agentAvatarUrl={agentAvatarUrl} agentName={agentName} userAvatarUrl={currentUserAvatarUrl} userName={currentUserDisplayName} voicePlayingMessageId={playingVoiceMessageId} isVoiceTranscriptVisible={Boolean(voiceTranscriptVisibleById[userMessage.id])} onPlayVoiceMessage={onPlayVoiceMessage} onVoiceContextMenu={onVoiceContextMenu} showAvatar showTimestamp position="single" displayContext="stage"/>) : null}

            {assistantMessages.map((message, index) => (<ChatBubble key={message.id} message={message} agentAvatarUrl={agentAvatarUrl} agentName={agentName} userAvatarUrl={currentUserAvatarUrl} userName={currentUserDisplayName} voicePlayingMessageId={playingVoiceMessageId} isVoiceTranscriptVisible={Boolean(voiceTranscriptVisibleById[message.id])} onPlayVoiceMessage={onPlayVoiceMessage} onVoiceContextMenu={onVoiceContextMenu} showAvatar={index === 0 || index === assistantMessages.length - 1} showTimestamp={index === assistantMessages.length - 1} position={assistantMessages.length <= 1 ? 'single' : index === 0 ? 'start' : index === assistantMessages.length - 1 ? 'end' : 'middle'} displayContext="stage"/>))}

            {showPendingFirstBeat ? (<ConversationTypingBubble agentAvatarUrl={agentAvatarUrl} agentName={agentName} agentRoleLabel={t('ChatBubble.agentPending')} thinkingLabel={t('Header.presenceThinking')}/>) : null}
