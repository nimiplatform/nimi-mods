...
export function CompactConversationHeader({ selectedTarget, selectedTargetAvatarUrl, selectedTargetInitial, loadingTargetDetail, interactionSnapshot, hasInputText, isSending, sendPhase, messages, playingVoiceMessageId, onBackToTargetStage, onOpenSelectedTargetProfile, onOpenSettings, onReturnToStage, }: CompactConversationHeaderProps) {
    ...
    return (<div className="relative overflow-hidden border-b border-white/70 px-6 py-3">
      ...
      <div className="relative z-10 flex items-center justify-between gap-4">
        <div className="flex min-w-0 items-center gap-3">
          <button type="button" onClick={onBackToTargetStage} className="lc-btn lc-btn-secondary h-9 w-9 rounded-full text-slate-700" aria-label={t('Header.backToTargets')} title={t('Header.backToTargets')}>
            {ICON_ARROW_LEFT}
          </button>

          <button type="button" onClick={onOpenSelectedTargetProfile} className="flex min-w-0 items-center gap-3 rounded-full bg-white/76 px-2.5 py-1.5 text-left shadow-[0_10px_26px_rgba(15,23,42,0.08)] transition-transform duration-200 hover:translate-y-[-1px]" aria-label={t('Header.openProfileDrawer')} title={t('Header.openProfileDrawer')}>
            {selectedTargetAvatarUrl ? (<img src={selectedTargetAvatarUrl} alt={selectedTarget.displayName} className="h-10 w-10 shrink-0 rounded-full object-cover ring-1 ring-black/5"/>) : (<span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white text-sm font-black ring-1 ring-black/5" style={{ color: theme.text }}>
            ...
            <span className="min-w-0">
              ...
              <span className="mt-0.5 inline-flex items-center gap-1.5 text-xs text-slate-500">
                <span className={`inline-block h-2 w-2 rounded-full ${presenceState.busy ? 'animate-pulse' : ''}`} style={{ background: theme.accentStrong }}/>
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={onReturnToStage} className="lc-btn lc-btn-secondary h-9 w-9 rounded-full text-slate-700" aria-label={t('Header.returnToStage')} title={t('Header.returnToStage')}>
            {ICON_STAGE}
          </button>
          <button type="button" onClick={onOpenSettings} className="lc-btn lc-btn-secondary h-9 w-9 rounded-full text-slate-700" aria-label={t('Header.openSettings')} title={t('Header.openSettings')}>
            {ICON_SETTINGS}
          </button>
        </div>
      </div>
