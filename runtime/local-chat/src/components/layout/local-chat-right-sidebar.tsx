...
class RuntimeSidebarBoundary extends React.Component<SidebarBoundaryProps, SidebarBoundaryState> {
    ...
    override render() {
        ...
        return (<div className="flex h-full flex-col justify-between border-l border-[var(--lc-border)] bg-[#f7fafb] p-4">
        ...
        <button type="button" onClick={this.props.onClose} className="lc-btn lc-btn-secondary h-10 rounded-full px-4 text-sm font-semibold">
          {this.props.closeLabel}
        </button>
      </div>);
    }
}
export function LocalChatRightSidebar({ isRuntimeSidebarOpen, runtimeSidebarProps, voiceContextMenu, voiceTranscriptVisibleById, onToggleVoiceTranscript, onCloseSidebar, }: LocalChatRightSidebarProps) {
    ...
    return (<>
      <div className="h-full shrink-0 overflow-hidden transition-[width,opacity,transform] duration-300 ease-[cubic-bezier(0.2,0.7,0.2,1)]" style={{
        ...
        <div className="h-full" style={{ width: `${RUNTIME_SIDEBAR_WIDTH_PX}px` }}>
          {shouldRenderSidebar ? (<div className={`h-full transition-opacity duration-300 ${isRuntimeSidebarOpen ? 'opacity-100 delay-75' : 'opacity-0'}`}>
              <RuntimeSidebarBoundary resetKey={isRuntimeSidebarOpen ? 'open' : 'closed'} title={t('RightSidebar.inspectCrashedTitle')} body={t('RightSidebar.inspectCrashedBody')} closeLabel={t('RightSidebar.closeInspect')} onClose={onCloseSidebar}>
                <RuntimeStatusSidebar {...runtimeSidebarProps} sidebarVisible={isRuntimeSidebarOpen}/>
              </RuntimeSidebarBoundary>
            </div>) : (<div className="lc-sidebar-skeleton flex h-full flex-col p-4">
      ...
      {voiceContextMenu ? (<div className="fixed z-50 min-w-[160px] rounded-xl border border-gray-200 bg-white p-1.5 shadow-xl" style={{ left: `${voiceContextMenu.x}px`, top: `${voiceContextMenu.y}px`, animation: 'panel-scale-in 0.15s ease-out both' }} onClick={(event) => event.stopPropagation()}>
          <button type="button" className="w-full rounded-lg px-3 py-2 text-left text-sm text-gray-800 transition-colors hover:bg-gray-100" onClick={() => onToggleVoiceTranscript(voiceContextMenu.messageId)}>
