...
export const LocalChatCharacterRail = React.memo(function LocalChatCharacterRail({ selectedTarget, selectedTargetAvatarUrl, selectedTargetInitial, theme, avatarAnchorRef, interactionSnapshot, loadingTargetDetail, hasInputText, isSending, sendPhase, messages, playingVoiceMessageId, onBackToTargets, onOpenSelectedTargetProfile, }: LocalChatCharacterRailProps) {
    ...
    return (<aside className="relative flex min-h-0 w-[clamp(360px,30vw,600px)] shrink-0 flex-col overflow-hidden border-r border-white/70 bg-[linear-gradient(180deg,rgba(250,252,252,0.98),rgba(244,247,248,0.96))]">
      ...
      <div className="relative z-10 flex h-full min-h-0 flex-col">
        <div className="shrink-0 border-b border-white/70 px-6 py-3">
          <button type="button" onClick={onBackToTargets} className="lc-btn lc-btn-secondary h-10 w-10 rounded-full text-slate-700" aria-label={t('Header.backToTargets')} title={t('Header.backToTargets')}>
            {ICON_ARROW_LEFT}
          </button>
        </div>
        ...
        <div className="flex min-h-0 flex-1 flex-col px-5 pb-5 pt-4">
          ...
          <div className="shrink-0 space-y-4 pb-4 text-center">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2">
              <span className="inline-flex items-center gap-2 rounded-full border px-3.5 py-2 text-xs font-semibold shadow-[0_10px_24px_rgba(15,23,42,0.05)]" style={{
            ...
            color: theme.text,
              ...
              <span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-semibold shadow-[0_10px_24px_rgba(15,23,42,0.05)] ${relationshipBadgeClass(interactionSnapshot?.relationshipState || 'new')}`}>
