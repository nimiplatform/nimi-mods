...
export function LocalChatTargetPane({ visibleTargets, loadingTargets, selectedTargetId, setSelectedTargetId, targetSearchText, setTargetSearchText, }: LocalChatTargetPaneProps) {
    ...
    }, [setTargetSearchText, targetSearchText]);
    ...
    return (<section className="relative flex h-full min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div className={`relative z-10 flex min-h-0 flex-1 flex-col px-6 pb-6 pt-6 transition-opacity duration-200 ${transitioningTargetId ? 'opacity-0' : 'opacity-100'}`}>
        <div className="relative min-h-0 flex-1 overflow-hidden rounded-[34px] border border-white/80 bg-white/44 shadow-[0_8px_28px_rgba(15,23,42,0.06)]">
          ...
          <div ref={stageViewportRef} className="h-full overflow-y-scroll overflow-x-hidden overscroll-contain px-4 py-5" onMouseMove={handleStageMouseMove} onMouseLeave={handleStageMouseLeave}>
            {visibleTargets.length === 0 ? (<div className="flex h-full min-h-[360px] items-center justify-center">
              ...
              </div>) : (<div className="relative min-h-[680px] w-full" style={{ height: `${bubbleLayout.height}px` }}>
                ...
                return (<div key={target.id} data-bubble-id={target.id} className="absolute" style={{
                        left: `${layout.left}px`,
                        top: `${layout.top}px`,
                    ...
                    <div className="h-full w-full" style={{
                      ...
                      <button type="button" onClick={() => handleSelectTarget(target.id)} className="group relative flex items-center justify-center rounded-full outline-none focus-visible:ring-4 focus-visible:ring-mint-200" style={{
                        width: `${layout.size}px`,
                        height: `${layout.size}px`,
                    }} aria-label={target.displayName} aria-pressed={isSelected}>
                        <span className="pointer-events-none absolute inset-[-10px] rounded-full opacity-70 transition-[opacity,transform] duration-200 ease-out group-hover:opacity-100 group-hover:scale-[1.05] group-focus-visible:opacity-100" style={{
                        background: `radial-gradient(circle, ${theme.accentSoft} 0%, transparent 68%)`,
                    }}/>
                        <span className={`pointer-events-none absolute inset-[-3px] rounded-full border transition-[opacity,transform,box-shadow] duration-200 ease-out ${unreadBadge ? 'lc-bubble-ring' : ''}`} style={{
                        ...
                        <span className="pointer-events-none absolute inset-0 rounded-full border border-white/80 shadow-[0_8px_16px_rgba(15,23,42,0.06)]" style={{
                        ...
                        <span className="absolute inset-[8px] overflow-hidden rounded-full border border-white/80 bg-white/90">
                          {target.avatarUrl ? (<img src={target.avatarUrl} alt={target.displayName} className="h-full w-full object-cover"/>) : (<span className="flex h-full w-full items-center justify-center text-3xl font-black" style={{ color: theme.text }}>
                              {getTargetInitial(target)}
                            </span>)}
                        </span>
                        {onlineState ? (<span className={`absolute bottom-[16%] right-[15%] h-4 w-4 rounded-full border-2 border-white ${onlineState === 'online' ? 'bg-mint-500' : 'bg-slate-300'}`}/>) : null}
                        {unreadBadge ? (<span className="absolute -right-2 top-2 inline-flex h-7 min-w-[28px] items-center justify-center rounded-full bg-slate-900 px-2 text-[11px] font-bold text-white shadow-[0_10px_20px_rgba(15,23,42,0.2)]">
                            {unreadBadge}
                          </span>) : null}
                      </button>

                      <p className="absolute left-1/2 -translate-x-1/2 truncate text-center text-xs font-semibold text-slate-700" style={{
