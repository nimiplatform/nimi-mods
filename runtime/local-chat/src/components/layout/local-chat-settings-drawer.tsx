...
import { useModTranslation, filterModelOptions, normalizeRuntimeRouteSource, type RuntimeRouteBinding } from "@nimiplatform/sdk/mod";
...
function SegmentButton(props: {
    ...
    return (<button type="button" onClick={props.onClick} className={`rounded-full px-3 py-1.5 text-xs font-semibold transition-colors ${props.active
...
function ToggleRow(props: {
    ...
    return (<button type="button" onClick={() => props.onChange(!props.checked)} className="flex w-full items-center justify-between gap-3 rounded-2xl border border-gray-200 bg-white px-4 py-3 text-left transition-colors hover:border-mint-200 hover:bg-mint-50/40">
      <div>
        <p className="text-sm font-semibold text-gray-900">{props.label}</p>
        <p className="mt-0.5 text-xs text-gray-500">{props.hint}</p>
      </div>
      <span className={`inline-flex h-7 w-12 shrink-0 items-center rounded-full p-1 transition-colors ${props.checked ? 'bg-mint-500 justify-end' : 'bg-gray-200 justify-start'}`}>
...
function CollapsibleSection(props: {
    ...
    return (<div className="mt-2 rounded-xl border border-dashed border-gray-200 bg-gray-50/60">
      <button type="button" onClick={props.onToggle} className="flex h-9 w-full items-center justify-between px-3 text-left text-[11px] font-semibold text-gray-500">
        <span>{props.title}</span>
        <span className={`text-gray-400 transition-transform duration-200 ${props.open ? 'rotate-180' : ''}`}>{CHEVRON_ICON}</span>
      </button>
      {props.open ? (<div className="px-3 pb-3 lc-panel-expand">
          {props.children}
        </div>) : null}
    </div>);
}
// ── Inline Chat Route (replaces importing ChatRoutePanel with full model resolution) ──
function InlineChatRoute(props: {
    rsp: RuntimeStatusSidebarProps;
}) {
    const { t } = useModTranslation('local-chat');
    const { rsp } = props;
    const routeBinding = rsp.routeBinding || null;
    const chatRouteOptions = rsp.chatRouteOptions;
    const effectiveChatBinding: RuntimeRouteBinding | null = (routeBinding
    ...
    const filteredChatModelOptions = useMemo(() => filterModelOptions(chatModelOptions, chatModelQuery), [chatModelOptions, chatModelQuery]);
    ...
    return (<div className="space-y-2 text-xs">
      <div>
        ...
        <select value={activeChatSource} onChange={(e) => rsp.onRouteSourceChange(normalizeRuntimeRouteSource(e.target.value))} className={selectClassName}>
      ...
      <div>
        ...
        <select value={activeChatConnectorId} disabled={activeChatSource !== 'cloud'} onChange={(e) => rsp.onRouteConnectorChange(e.target.value)} className={`${selectClassName} disabled:bg-gray-100 disabled:text-gray-400`}>
          <option value="">--</option>
          {(chatRouteOptions?.connectors || []).map((c) => (<option key={`rc-${c.id}`} value={c.id}>{c.label || c.id}</option>))}
        </select>
      </div>
      <div>
        <p className="mb-1 text-gray-500">{t('ChatRoute.model')}</p>
        <input list="settings-chat-model-list" value={chatModelQuery} onChange={(e) => {
        ...
        }} onBlur={(e) => commitChatModelQuery(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') {
        e.preventDefault();
        commitChatModelQuery(chatModelQuery);
    } }} placeholder={t('ChatRoute.modelPlaceholder')} className={`h-8 w-full rounded-xl bg-white px-2 text-xs text-gray-900 outline-none transition-colors focus:ring-1 ${hasPendingModelChange
        ...
        <datalist id="settings-chat-model-list">
          {filteredChatModelOptions.map((m) => (<option key={`scm-${m}`} value={m}/>))}
        </datalist>
      ...
      {showEmptyLocalCta ? (<div className="rounded-lg border border-amber-200 bg-amber-50 p-2 text-[11px] text-amber-800">
          {t('ChatRoute.emptyLocalHint')}
        </div>) : null}
      <button type="button" onClick={onClearRouteBinding} className="lc-btn lc-btn-secondary h-8 w-full px-2 text-xs font-semibold">
...
export const LocalChatSettingsDrawer = React.memo(function LocalChatSettingsDrawer(props: LocalChatSettingsDrawerProps) {
    const { t } = useModTranslation('local-chat');
    const { open, onClose, productSettings, enableVoice, onToggleProductSetting, onMediaAutonomyChange, onVoiceAutonomyChange, onVoiceConversationModeChange, onVisualComfortLevelChange, runtimeSidebarProps, } = props;
    ...
    return (<div className={`absolute inset-y-0 right-0 z-30 w-[360px] max-w-[92vw] border-l border-white/70 bg-[#f8fbfb] shadow-[-8px_0_24px_rgba(15,23,42,0.08)] transition-transform duration-280 ease-[cubic-bezier(0.2,0.7,0.2,1)] ${open ? 'translate-x-0' : 'translate-x-full pointer-events-none'}`} aria-hidden={!open}>
      <div className="flex h-full min-h-0 flex-col overflow-hidden">
        <div className="flex h-14 shrink-0 items-center justify-between border-b border-gray-200 px-4">
          ...
          <button type="button" onClick={onClose} className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-600 transition-colors hover:bg-gray-50" aria-label={t('SettingsDrawer.close')}>
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto overscroll-contain px-4 py-4" style={{ willChange: 'transform' }}>
          ...
          <section className="space-y-3 rounded-[24px] border border-white/80 bg-white/88 p-4 shadow-[0_4px_16px_rgba(15,23,42,0.04)]">
            ...
            <CollapsibleSection title={t('SettingsDrawer.chatRouteConfig')} open={chatRouteOpen} onToggle={() => setChatRouteOpen((v) => !v)}>
          ...
          <section className="space-y-3 rounded-[24px] border border-white/80 bg-white/88 p-4 shadow-[0_4px_16px_rgba(15,23,42,0.04)]">
            ...
            <ToggleRow label={t('SettingsDrawer.voiceConversationMode')} hint={t('SettingsDrawer.voiceConversationModeHint')} checked={productSettings.voiceConversationMode === 'on'} onChange={(value) => onVoiceConversationModeChange(value ? 'on' : 'off')}/>
            <ToggleRow label={t('SettingsDrawer.autoPlayVoiceReplies')} hint={t('SettingsDrawer.autoPlayVoiceRepliesHint')} checked={productSettings.autoPlayVoiceReplies} onChange={(value) => onToggleProductSetting('autoPlayVoiceReplies', value)}/>
            <CollapsibleSection title={t('SettingsDrawer.voiceRouteConfig')} open={voiceRouteOpen} onToggle={() => setVoiceRouteOpen((v) => !v)}>
              <VoicePanel embedded open onToggle={() => { }} enableVoice={enableVoice} selectedVoiceId={runtimeSidebarProps.selectedVoiceId} ttsRouteSource={runtimeSidebarProps.ttsRouteSource} ttsConnectorId={runtimeSidebarProps.ttsConnectorId} ttsModel={runtimeSidebarProps.ttsModel} sttRouteSource={runtimeSidebarProps.sttRouteSource} sttConnectorId={runtimeSidebarProps.sttConnectorId} sttModel={runtimeSidebarProps.sttModel} ttsConnectors={runtimeSidebarProps.ttsConnectors} sttConnectors={runtimeSidebarProps.sttConnectors} localTtsRouteAvailable={runtimeSidebarProps.localTtsRouteAvailable} localSttRouteAvailable={runtimeSidebarProps.localSttRouteAvailable} speechVoices={runtimeSidebarProps.speechVoices} onVoiceIdChange={runtimeSidebarProps.onVoiceIdChange} onTtsRouteSourceChange={runtimeSidebarProps.onTtsRouteSourceChange} onTtsConnectorChange={runtimeSidebarProps.onTtsConnectorChange} onTtsModelChange={runtimeSidebarProps.onTtsModelChange} onSttRouteSourceChange={runtimeSidebarProps.onSttRouteSourceChange} onSttConnectorChange={runtimeSidebarProps.onSttConnectorChange} onSttModelChange={runtimeSidebarProps.onSttModelChange}/>
            </CollapsibleSection>
          </section>
          ...
          <section className="space-y-3 rounded-[24px] border border-white/80 bg-white/88 p-4 shadow-[0_4px_16px_rgba(15,23,42,0.04)]">
            ...
            {productSettings.mediaAutonomy !== 'off' ? (<div>
            ...
            <CollapsibleSection title={t('SettingsDrawer.mediaRouteConfig')} open={mediaRouteOpen} onToggle={() => setMediaRouteOpen((v) => !v)}>
              <MediaRoutePanel embedded open loading={runtimeSidebarProps.isMediaRuntimeSidebarLoading || false} onToggle={() => { }} imageRouteOptions={runtimeSidebarProps.imageRouteOptions} videoRouteOptions={runtimeSidebarProps.videoRouteOptions} imageResolvedRoute={runtimeSidebarProps.imageResolvedRoute || null} videoResolvedRoute={runtimeSidebarProps.videoResolvedRoute || null} isImageRouteProbeLoading={runtimeSidebarProps.isImageRouteProbeLoading || false} isVideoRouteProbeLoading={runtimeSidebarProps.isVideoRouteProbeLoading || false} imageRouteSource={runtimeSidebarProps.imageRouteSource} videoRouteSource={runtimeSidebarProps.videoRouteSource} imageConnectorId={runtimeSidebarProps.imageConnectorId} imageModel={runtimeSidebarProps.imageModel} videoConnectorId={runtimeSidebarProps.videoConnectorId} videoModel={runtimeSidebarProps.videoModel} imageConnectors={runtimeSidebarProps.imageConnectors} videoConnectors={runtimeSidebarProps.videoConnectors} onImageRouteSourceChange={runtimeSidebarProps.onImageRouteSourceChange} onImageConnectorChange={runtimeSidebarProps.onImageConnectorChange} onImageModelChange={runtimeSidebarProps.onImageModelChange} onVideoRouteSourceChange={runtimeSidebarProps.onVideoRouteSourceChange} onVideoConnectorChange={runtimeSidebarProps.onVideoConnectorChange} onVideoModelChange={runtimeSidebarProps.onVideoModelChange}/>
            </CollapsibleSection>
          </section>
          ...
          <section className="space-y-3 rounded-[24px] border border-white/80 bg-white/88 p-4 shadow-[0_4px_16px_rgba(15,23,42,0.04)]">
            ...
            <ToggleRow label={t('SettingsDrawer.allowProactiveContact')} hint={t('SettingsDrawer.allowProactiveContactHint')} checked={productSettings.allowProactiveContact} onChange={(value) => onToggleProductSetting('allowProactiveContact', value)}/>
          </section>
        </div>
      </div>
    </div>);
