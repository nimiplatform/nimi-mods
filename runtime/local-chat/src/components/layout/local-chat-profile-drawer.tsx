...
function relationshipBadgeClass(value: InteractionSnapshot['relationshipState'] | 'new'): string {
    if (value === 'friendly') {
...
function relationshipStateLabel(value: InteractionSnapshot['relationshipState'] | 'new', t: (key: string) => string): string {
    if (value === 'friendly')
        return t('ProfileDrawer.relationshipStateFriendly');
    if (value === 'warm')
        return t('ProfileDrawer.relationshipStateWarm');
    if (value === 'intimate')
        return t('ProfileDrawer.relationshipStateIntimate');
...
export function LocalChatProfileDrawer(props: LocalChatProfileDrawerProps) {
    ...
    const { open, selectedTarget, selectedTargetAvatarUrl, selectedTargetInitial, interactionSnapshot, onClose, onOpenSelectedTargetProfile, onClearChatHistory, onClearMemory, } = props;
    return (<div className={`absolute inset-y-0 right-0 z-30 w-[380px] max-w-[94vw] border-l border-white/70 bg-[#f8fbfb] shadow-[-12px_0_32px_rgba(15,23,42,0.10)] transition-transform duration-300 ease-[cubic-bezier(0.2,0.7,0.2,1)] ${open ? 'translate-x-0' : 'translate-x-full'}`} aria-hidden={!open}>
      <div className="flex h-full flex-col">
        <div className="flex h-14 items-center justify-between border-b border-gray-200 px-4">
          ...
          <button type="button" onClick={onClose} className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-600 transition hover:bg-gray-50" aria-label={t('ProfileDrawer.close')}>
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-4 py-4">
          <section className="rounded-[24px] border border-white/80 bg-[linear-gradient(135deg,rgba(255,255,255,0.98),rgba(237,247,247,0.86))] p-4 shadow-[0_16px_40px_rgba(15,23,42,0.06)]">
            <div className="flex items-start gap-3">
              <div className="flex h-14 w-14 items-center justify-center overflow-hidden rounded-[20px] bg-gradient-to-br from-mint-100 via-sky-50 to-brand-100">
                {selectedTargetAvatarUrl ? (<img src={selectedTargetAvatarUrl} alt={selectedTarget?.displayName || 'Agent'} className="h-full w-full object-cover"/>) : (<span className="text-lg font-semibold text-mint-700">{selectedTargetInitial}</span>)}
              </div>
            ...
            <button type="button" onClick={onOpenSelectedTargetProfile} disabled={!selectedTarget} className="mt-4 inline-flex items-center rounded-full border border-gray-200 bg-white px-3 py-1.5 text-xs font-semibold text-gray-700 transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50">
          ...
          <section className="rounded-[24px] border border-white/80 bg-white/88 p-4 shadow-[0_16px_40px_rgba(15,23,42,0.06)]">
            ...
            <div className="mt-3">
              <span className={`inline-flex items-center gap-2 rounded-full border px-3.5 py-2 text-sm font-semibold shadow-[0_10px_24px_rgba(15,23,42,0.05)] ${relationshipBadgeClass(interactionSnapshot?.relationshipState || 'new')}`}>
          ...
          <section className="space-y-3 rounded-[24px] border border-white/80 bg-white/88 p-4 shadow-[0_16px_40px_rgba(15,23,42,0.06)]">
            ...
            <button type="button" onClick={onClearChatHistory} className="inline-flex items-center gap-2 rounded-full border border-rose-200 bg-rose-50 px-4 py-2 text-xs font-semibold text-rose-700 transition hover:bg-rose-100">
          ...
          <section className="space-y-3 rounded-[24px] border border-white/80 bg-white/88 p-4 shadow-[0_16px_40px_rgba(15,23,42,0.06)]">
            ...
            <button type="button" onClick={onClearMemory} className="inline-flex items-center gap-2 rounded-full border border-rose-200 bg-rose-50 px-4 py-2 text-xs font-semibold text-rose-700 transition hover:bg-rose-100">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                <path d="M3 6h18"/>
                <path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/>
                <path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/>
