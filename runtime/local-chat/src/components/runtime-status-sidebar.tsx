...
import { bindingsEqual, formatRouteBindingLabel, formatRouteSnapshotLabel, hasPendingChatModelChange, sourceLabel, } from './sidebar/runtime-status-state.js';
import { useModTranslation, filterModelOptions, type RuntimeRouteBinding } from "@nimiplatform/sdk/mod";
const ICON_SHIELD = (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export function RuntimeStatusSidebar(props: RuntimeStatusSidebarProps) {
    ...
    const { healthStatus, checkingHealth, chatRouteOptions, speechVoices, selectedVoiceId, ttsRouteSource, sttRouteSource, localTtsRouteAvailable, localSttRouteAvailable, autoBoundSource, autoBoundModel, chatCapabilityMatched, dependencyCapabilities, dependencyStatus, dependencyReasonCode, dependencyUpdatedAt, dependencyRepairActions, latestPromptTrace, latestTurnAudit, routeSnapshot, onRouteSourceChange, onRouteConnectorChange, onRouteModelChange, onVoiceIdChange, onTtsRouteSourceChange, onTtsConnectorChange, onSttRouteSourceChange, onHealthCheck, onOpenRuntimeSetup, inspectSettings, enableVoice, sidebarVisible = true, } = props;
    ...
    const hasValidTokenApiOverride = (routeBinding?.source === 'cloud'
        ? ((chatRouteOptions?.connectors.length || 0) === 0
            || chatRouteOptions?.connectors.some((connector) => connector.id === routeBinding.connectorId))
        : true);
    const effectiveChatBinding: RuntimeRouteBinding | null = (routeBinding && hasValidTokenApiOverride
        ? routeBinding
    ...
    const chatModelOptions = useMemo(() => dedupeModelIds(chatModelOptionsRaw), [chatModelOptionsRaw]);
    ...
    const visibleSpeechVoices = useMemo(() => resolveVisibleSpeechVoices({
        speechVoices,
    }), [speechVoices]);
    useEffect(() => {
    ...
    const missingRequiredDependencies = useMemo(() => dependencyCapabilities.filter((item) => item.required && item.resolved && !item.matched), [dependencyCapabilities]);
    ...
    const overrideApplied = Boolean(routeBinding
    ...
    const dependencyStatusLabel = (showMediaDependencyPending
    ...
    return (<aside className="flex h-full min-h-0 w-80 shrink-0 flex-col overflow-y-auto border-l border-[var(--lc-border)] bg-[#f4f8f9]">
      ...
      <div className="space-y-3 p-4">
        <div className="lc-card rounded-2xl p-3 text-xs transition-all duration-200">
          ...
          {hasPendingModelInput ? (<p className="mt-1 text-[11px] text-amber-700">{t('RuntimeSidebar.pendingRouteHint')}</p>) : null}
          <div className="mt-2 flex flex-wrap gap-1.5">
            ...
            <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold ${chatCapabilityMatched ? 'bg-mint-100 text-mint-700' : 'bg-amber-100 text-amber-800'}`}>
              {chatCapabilityMatched ? t('RuntimeSidebar.capabilityHit') : t('RuntimeSidebar.capabilityMissing')}
            </span>
            <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold ${showMediaDependencyPending
            ...
            {visibleDependencyReasonCode ? (<span className="inline-flex rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-semibold text-gray-600">
                {visibleDependencyReasonCode}
              </span>) : null}
          </div>
          {failedCapabilityLabels.length > 0 ? (<p className="mt-2 text-[11px] text-amber-700">
              {failedCapabilityLabels.join(', ')} {t('RuntimeSidebar.capabilityMissing')}
            </p>) : null}
          {dependencyUpdatedTimeLabel ? (<p className="mt-2 text-[10px] text-gray-500">
              {t('RuntimeSidebar.dependencyUpdatedAt')}: {dependencyUpdatedTimeLabel}
            </p>) : null}
          <button type="button" className="lc-btn lc-btn-secondary mt-2 h-7 px-2 text-[11px] font-medium" onClick={onRefreshMediaDependencies} disabled={isMediaRuntimeSidebarLoading}>
            {isMediaRuntimeSidebarLoading ? t('RuntimeSidebar.mediaDependencyChecking') : t('RuntimeSidebar.refreshMediaDependencies')}
          </button>
          {!chatCapabilityMatched || missingRequiredDependencies.length > 0 || dependencyRepairActions.length > 0 ? (<button type="button" className="lc-btn lc-btn-warning mt-2 h-7 px-2 text-[11px] font-medium" onClick={onOpenRuntimeSetup}>
              {t('RuntimeSidebar.dependencyCta')}
            </button>) : null}
          {dependencyRepairActions.length > 0 ? (<div className="mt-2 space-y-1">
              {dependencyRepairActions.slice(0, 3).map((action) => (<button key={`runtime-repair-${action.actionId}`} type="button" className="lc-btn lc-btn-warning block h-7 px-2 text-left text-[11px]" onClick={onOpenRuntimeSetup} title={action.reasonCode}>
                  {action.label}
                </button>))}
            </div>) : null}
        </div>

        <ChatRoutePanel open={openPanel === 'chat'} onToggle={() => togglePanel('chat')} activeChatSource={activeChatSource} activeChatConnectorId={activeChatConnectorId} activeChatModel={effectiveChatBinding?.model || ''} chatRouteOptions={chatRouteOptions} chatModelQuery={chatModelQuery} setChatModelQuery={setChatModelQuery} chatModelOptions={chatModelOptions} filteredChatModelOptions={filteredChatModelOptions} onRouteSourceChange={onRouteSourceChange} onRouteConnectorChange={onRouteConnectorChange} onRouteModelChange={onRouteModelChange} onClearRouteBinding={onClearRouteBinding}/>

        <VoicePanel open={openPanel === 'voice'} onToggle={() => togglePanel('voice')} enableVoice={enableVoice} selectedVoiceId={selectedVoiceId} ttsRouteSource={ttsRouteSource} ttsConnectorId={props.ttsConnectorId} ttsModel={props.ttsModel} sttRouteSource={sttRouteSource} sttConnectorId={props.sttConnectorId} sttModel={props.sttModel} ttsConnectors={props.ttsConnectors} sttConnectors={props.sttConnectors} localTtsRouteAvailable={localTtsRouteAvailable} localSttRouteAvailable={localSttRouteAvailable} speechVoices={visibleSpeechVoices} onVoiceIdChange={onVoiceIdChange} onTtsRouteSourceChange={onTtsRouteSourceChange} onTtsConnectorChange={onTtsConnectorChange} onTtsModelChange={props.onTtsModelChange} onSttRouteSourceChange={onSttRouteSourceChange} onSttConnectorChange={props.onSttConnectorChange} onSttModelChange={props.onSttModelChange}/>

        <MediaRoutePanel open={openPanel === 'media'} loading={isMediaRuntimeSidebarLoading} onToggle={() => togglePanel('media')} imageRouteOptions={props.imageRouteOptions} videoRouteOptions={props.videoRouteOptions} imageResolvedRoute={props.imageResolvedRoute || null} videoResolvedRoute={props.videoResolvedRoute || null} isImageRouteProbeLoading={isImageRouteProbeLoading} isVideoRouteProbeLoading={isVideoRouteProbeLoading} imageRouteSource={props.imageRouteSource} videoRouteSource={props.videoRouteSource} imageConnectorId={props.imageConnectorId} imageModel={props.imageModel} videoConnectorId={props.videoConnectorId} videoModel={props.videoModel} imageConnectors={props.imageConnectors} videoConnectors={props.videoConnectors} onImageRouteSourceChange={props.onImageRouteSourceChange} onImageConnectorChange={props.onImageConnectorChange} onImageModelChange={props.onImageModelChange} onVideoRouteSourceChange={props.onVideoRouteSourceChange} onVideoConnectorChange={props.onVideoConnectorChange} onVideoModelChange={props.onVideoModelChange}/>

        {inspectSettings.diagnosticsVisible ? (<DiagnosticsPanel open={openPanel === 'diagnostics'} onToggle={() => togglePanel('diagnostics')} latestPromptTrace={latestPromptTrace} latestTurnAudit={latestTurnAudit} healthStatus={healthStatus} checkingHealth={checkingHealth} onHealthCheck={onHealthCheck}/>) : null}
      </div>
    </aside>);
}
