...
import { LOCAL_CHAT_DATA_API_CHAT_TARGET_DETAIL, } from '../contracts.js';
...
function normalizePreviewText(value: unknown): string | null {
    ...
    if (!normalized)
        return null;
    if (normalized.length <= LOCAL_PREVIEW_MAX_LENGTH)
        return normalized;
...
async function buildTargetsWithLocalPreview(source: LocalChatTarget[], viewerId: string): Promise<LocalChatTarget[]> {
    ...
    const previewsByTargetId = new Map(previews.map((preview) => [preview.targetId, preview]));
...
export function areTargetsRenderEquivalent(previous: ReadonlyArray<LocalChatTarget>, next: ReadonlyArray<LocalChatTarget>): boolean {
...
export function useLocalChatTargets(input: UseLocalChatTargetsInput) {
    ...
    const selectedTargetBase = useMemo(() => targets.find((target) => target.id === selectedTargetId) || null, [selectedTargetId, targets]);
    ...
    const selectedTargetHasDetail = useMemo(() => Boolean(selectedTargetBase && targetDetailsById[selectedTargetBase.id]), [selectedTargetBase, targetDetailsById]);
    ...
    useEffect(() => {
        if (!selectedTargetBase || selectedTargetHasDetail)
            return;
        let cancelled = false;
