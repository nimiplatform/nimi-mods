...
import { isDegenerateAssistantReply, isPromptEchoReply, sanitizeAssistantReply, } from '../../services/view/reply.js';
import type { TurnInvokeInput } from './request-builder.js';
import type { LocalChatReplyPacingPlan } from '../../state/index.js';
import type { AssistantPlanSegment, LocalChatTurnAiClient, SegmentParseMode, } from './types.js';
...
const LOCAL_CHAT_TEXT_STREAM_TIMEOUT_MS = 20000;
const LOCAL_CHAT_TEXT_FALLBACK_TIMEOUT_MS = 20000;
...
function splitIntoSegments(text: string, allowMultiReply: boolean, segmentationMode: ReplySegmentationMode): SplitSegmentsResult {
    const normalizedText = normalizeSegmentText(text);
    if (!normalizedText) {
        return {
            segments: [localChatMessage('TurnFeedback.noReplyAvailable', 'Sorry, I do not have a usable reply right now. Please try again.')],
...
export function splitStreamReplyIntoSegments(text: string, allowMultiReply: boolean, segmentationMode: ReplySegmentationMode = 'adaptive'): SplitSegmentsResult {
