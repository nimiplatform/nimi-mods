...
import { commitAssistantMessage, persistUserTurns, scheduleAssistantTurnDeliveries, type TurnDeliveryScheduleHandle, } from './session-persist.js';
...
import { appendTurnsToSession, createLocalChatTurnRecord, listLocalChatSessions, } from '../../state/index.js';
...
import { createRendererFlowId, getPromptLocale } from "@nimiplatform/sdk/mod";
type OrchestratedBeat = ReturnType<typeof orchestrateBeatModalities>[number];
type ConcreteMediaDecision = Exclude<MediaExecutionDecision, {
    kind: 'none';
}>;
...
function bindMediaDecisionToDelivery(decision: ConcreteMediaDecision, deliveryId: string): ConcreteMediaDecision {
    return {
        ...decision,
        intent: {
            ...decision.intent,
...
export async function runLocalChatTurnSend(input: {
    ...
    if (!context.selectedTarget) {
        context.setStatusBanner({
            kind: 'warning',
            message: localChatMessage('TurnFeedback.noAgentAvailable', 'No Agent friend available. Please add an Agent friend in Contacts first.'),
        });
        return;
    }
    ...
    try {
        ...
        if (context.synthesizeVoice) {
            ...
            if (voiceDeliveries.length > 0) {
                const synthResults = await Promise.allSettled(voiceDeliveries.map((delivery) => context.synthesizeVoice!(delivery.content)
                    .then((response) => ({
                    id: delivery.id,
                    playbackMeta: createPersistableVoicePlaybackCacheMeta(response),
                }))));
                for (const result of synthResults) {
                    if (result.status !== 'fulfilled' || !result.value.playbackMeta)
                        continue;
