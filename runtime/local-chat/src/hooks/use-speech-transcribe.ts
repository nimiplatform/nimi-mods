...
function resolveRouteBinding(source: UseSpeechTranscribeInput['sttRouteSource']): {
    source: 'local' | 'cloud';
    connectorId: string;
    model: string;
} | undefined {
...
function resolveStartErrorCode(error: unknown): string {
    const name = String((error as {
        name?: unknown;
    })?.name || '').trim();
    if (name === 'NotAllowedError' || name === 'SecurityError')
        return 'LOCAL_CHAT_STT_PERMISSION_DENIED';
    if (name === 'NotFoundError' || name === 'OverconstrainedError')
        return 'LOCAL_CHAT_STT_INPUT_DEVICE_NOT_FOUND';
    if (name === 'NotReadableError' || name === 'AbortError')
        return 'LOCAL_CHAT_STT_INPUT_DEVICE_UNAVAILABLE';
...
export function useSpeechTranscribe(input: UseSpeechTranscribeInput) {
    ...
    const finalizeRecording = useCallback(async (mode: 'transcribe' | 'cancel', reasonCode?: string) => {
