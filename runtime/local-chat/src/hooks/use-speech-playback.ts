...
import { isDirectVoicePlaybackUri, resolveCachedVoicePlaybackSource, } from '../services/voice/playback-source.js';
...
export function useSpeechPlayback(input: {
    ...
    const attachVoiceAudioHandlers = useCallback((value: {
        ...
        value.audio.onended = () => {
            if (voicePlaybackTokenRef.current !== value.playbackToken)
                return;
            clearVoiceAudio();
            setPlayingVoiceMessageId(null);
            logRendererEvent({
    ...
    const playVoiceMessage = useCallback(async (message: ChatMessage) => {
        ...
        if (!input.enableVoice) {
            input.setStatusBanner({
                kind: 'info',
                message: localChatMessage('VoiceFeedback.disabled', 'Voice is disabled in Default Settings.'),
            });
            return;
        }
        ...
        try {
            ...
            if (isVoiceUnsupportedTtsFailure(reasonCode, actionHint)) {
                input.setStatusBanner({
                    ...
                    actionLabel: input.onVoiceUnsupported
                        ? localChatMessage('VoiceFeedback.refreshVoiceList', 'Refresh Voice List')
                        : undefined,
