...
import { isMediaPlannerPromptText, isPerceptionPromptText, isTailPlanPromptText, RESTRAINED_STYLE_RE, SEXUAL_CONTENT_BOUNDARY_RE, } from './helpers/prompt-matchers.mjs';
import { configureLocalChatCoreQueryBridge, CORE_DATA_API_AGENT_MEMORY_RECALL_FOR_ENTITY, type LocalChatTarget, } from '../src/data/index.ts';
...
async function withSendFlowHarness(run: (harness: ReturnType<typeof createHarness>) => Promise<void>): Promise<void> {
...
function createHarness() {
    ...
    return {
        ...
        async execute(input: {
            ...
            const isMediaPlannerPrompt = (payload: Record<string, unknown>) => isMediaPlannerPromptText(String(payload.prompt || ''));
            const isPerceptionPrompt = (payload: Record<string, unknown>) => isPerceptionPromptText(String(payload.prompt || ''));
...
test('send-flow delivers all planned text beats while session id bootstraps from empty state', async () => {
    await withSendFlowHarness(async (harness) => {
        ...
        assert.deepEqual(assistantMessages.map((message) => message.content), [
            '真的辛苦你了，先让我接住你。',
...
test('send-flow planner auto path generates video with reference image content when available', async () => {
    await withSendFlowHarness(async (harness) => {
        ...
        assert.equal(String(capturedVideoPayload?.content?.[0]?.text || '').includes('请生成一段延续当前聊天场景'), true);
...
test('send-flow explicit media image keeps turn metadata so stage slice includes it', async () => {
    await withSendFlowHarness(async (harness) => {
        ...
        assert.equal(slice.assistantMessages.some((message) => message.id === imageMessage?.id), true);
    });
});
