...
export function isKismetRouteHealthHealthy(health: {
    healthy?: boolean;
    status?: string;
    reasonCode?: string;
}): boolean {
...
export function useKismetRoute() {
    const { routeSource, setRouteSource, routeBinding, setRouteBinding, chatRouteOptions, setChatRouteOptions, routeOptionsLoading, setRouteOptionsLoading, routeOptionsError, setRouteOptionsError, } = useKismetStore();
        ...
        try {
            ...
            if (result) {
                setChatRouteOptions((prev) => {
                    if (!result)
                        return prev;
                    if (result.connectors.length === 0 && (prev?.connectors.length || 0) > 0)
                        return prev;
                    return result;
                });
            }
    ...
    useEffect(() => {
        if (!routeBinding || routeBinding.source !== 'cloud')
            return;
