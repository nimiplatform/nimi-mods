...
type GenerateJsonViaAiOutput<T> = {
    ...
    rawResponse: KismetAiRawResponse;
} | {
    ok: false;
    error: KismetError;
    rawResponse?: KismetAiRawResponse;
...
function isRouteUnavailable(input: {
    reasonCode: string;
    message: string;
}): boolean {
    ...
    return (message.includes('route unavailable')
...
function normalizeAiFailure(error: unknown): NormalizedAiFailure {
    ...
    const rawMessage = String(error instanceof Error ? error.message : record.message || error || '').trim();
    ...
    return {
        ...
        actionHint: rawActionHint || (routeUnavailable
            ? kismetMessage('Messages.aiGenerateRouteHint', 'Switch to Prompt Import mode, or check Runtime route config and retry.')
