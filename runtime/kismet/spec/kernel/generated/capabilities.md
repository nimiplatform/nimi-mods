# Generated Capabilities

> AUTO-GENERATED by `scripts/generate-mod-spec-kernel-docs.mjs --mod runtime/kismet`. DO NOT EDIT.
> Source: `runtime/kismet/spec/kernel/tables/capabilities.yaml`

```yaml
version: 2
capabilities:
  - key: runtime.ai.text.generate
    direction: consume
    purpose: generate natal, daily, and compatibility narratives
    source_rule: KIS-CAP-002
  - key: runtime.route.list.options
    direction: consume
    purpose: discover route and model options for AI generation
    source_rule: KIS-CAP-002
  - key: runtime.route.resolve
    direction: consume
    purpose: resolve the active AI route before generation
    source_rule: KIS-CAP-002
  - key: runtime.route.check.health
    direction: consume
    purpose: verify configured route health for sidebar status and recovery
    source_rule: KIS-CAP-002
  - key: storage.sqlite.query
    direction: consume
    purpose: query local profiles and route state from host sqlite
    source_rule: KIS-CAP-002
  - key: storage.sqlite.execute
    direction: consume
    purpose: persist local profiles and route state into host sqlite
    source_rule: KIS-CAP-002
  - key: ui.register.ui-extension.app.sidebar.mods
    direction: consume
    purpose: register mod navigation
    source_rule: KIS-CAP-001
  - key: ui.register.ui-extension.app.content.routes
    direction: consume
    purpose: register Kismet page
    source_rule: KIS-CAP-001
internal_assets:
  - asset: deterministic-bazi-derivation
    storage: mod-local source code
    source_rule: KIS-CAP-004
  - asset: city-catalog
    storage: mod-local checked-in data
    source_rule: KIS-CAP-004
  - asset: local-share-profiles
    storage: mod host sqlite
    source_rule: KIS-COMP-001
```
