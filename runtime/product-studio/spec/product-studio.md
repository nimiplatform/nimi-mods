...
**素材策略**:
- 参考产品图、场景图、卖点都属于可选素材，不是创建项目或保存模板的硬前置。
- Prompt 工坊支持保存纯文本模板；只有在实际执行 `multimodal` 生成时，输入集中才要求至少有一张图片。
- Prompt 工坊中的附带图片会以稳定顺序持久化，支持项目素材和临时粘贴图片混用；执行时也允许在模板基础上临时补图或换图。
...
- **左侧面板（素材区）** — 可折叠，约 30% 宽度
  - Tab 1 "参考图"：产品参考图列表（网格，可为空），支持上传、设默认、删除
...
- 素材区支持拖拽上传
- 未上传参考图时不阻塞进入工作台，但展示“推荐补充产品参考图”的提示
...
- 历史生成列表：横向滚动查看多次结果对比
- multimodal 结果必须可回看“本次使用的场景图”和“本次输入图片快照”
