...
确认 PromptConfig（含 prompt + 附带图片）。

前置检查（阻塞）:
- `multimodal` 模式下，`attachedImageIds` 不可为空 → `PS_NO_IMAGES_FOR_MULTIMODAL`
- 若附带图片文件不存在 → `PS_SCENE_IMAGE_LOAD_FAILED`
- 若无可用图像生成服务路由 → `PS_ROUTE_NO_IMAGE_PROVIDER`

### gen-execute（图像生成）
通过 `runtime.media.image.generate`（PS-CAP-004）发起调用：
- `multimodal` 模式: 携带 `attachedImageIds` 对应图片 + `refinedPrompt`
