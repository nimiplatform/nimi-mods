...
- 请求携带 `referenceImages`（来自 `PromptConfig.attachedImageIds`）+ prompt
...
Product Studio 注册以下数据域 API（见 `capabilities.yaml` → `data_register`）:

| 能力声明 | 对应实体 |
|---------|---------|
| `data.register.data-api.product-studio.projects.*` | Project（PS-DOM-001） |
| `data.register.data-api.product-studio.references.*` | ReferenceImage（PS-DOM-002） |
| `data.register.data-api.product-studio.scenes.*` | SceneImage（PS-DOM-004） |
| `data.register.data-api.product-studio.selling-points.*` | SellingPoint（PS-DOM-003） |
| `data.register.data-api.product-studio.prompts.*` | PromptConfig（PS-DOM-005） |
| `data.register.data-api.product-studio.batches.*` | BatchJob（PS-DOM-006） |
| `data.register.data-api.product-studio.gallery.*` | GeneratedImage（PS-DOM-007） |
