...
它不直接创建 canonical world，也不直接编辑 Realm `asset`。它负责生成和整理私有场景工作态对象，其中 `ScenePack` 是用户可感知的主要产出物，`SceneCard` 是 pack 内最小素材单元。

核心边界：

- `Scene-Atlas` 负责把现实照片变成可用的场景工作态对象
- 上层市场负责接收可发布的 `ScenePack` 并将其纳入正式 `asset` / `listing` 流通层

## 2. Domain Invariants

- `SA-DOM-001`: `SceneCard` 是单图场景素材卡，不是独立发布对象。
- `SA-DOM-002`: `ScenePack` 是 Scene-Atlas 的主要产出物，也是 Scene-Atlas 内唯一允许进入发布交接的场景包容器。
- `SA-DOM-003`: `ScenePack` 和 `SceneCard` 属于 mod 私有场景工作态，不得伪装成 Realm `asset`。
...
- `SA-DOM-008`: 已发布的 `ScenePack` 仍可继续编辑，但编辑对象始终是 `ScenePack`，不是上层 `asset`。
...
| `SA-DOM-003` | scene working state / asset boundary | `ScenePack` 是私有场景工作态，不是 Realm `asset` |
