  ...
  return (
    <section className="rounded-xl border border-gray-200 bg-white p-4">
      ...
      {missingModules.length > 0 ? (
        ...
        <div className="mt-2 rounded-md border border-emerald-200 bg-emerald-50 px-2.5 py-2 text-xs text-emerald-800">
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-2">
        <ModuleEditor
          label="timeModel"
          value={worldview.timeModel}
          onChange={(next) => props.onWorldviewPatchChange(updateWorldview(worldview, (draft) => {
            draft.timeModel = next;
          }))}
        />
        <ModuleEditor
          label="spaceTopology"
          value={worldview.spaceTopology}
          onChange={(next) => props.onWorldviewPatchChange(updateWorldview(worldview, (draft) => {
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-2">
        <ModuleEditor
          label="visualGuide"
          value={worldview.visualGuide}
          onChange={(next) => props.onWorldviewPatchChange(updateWorldview(worldview, (draft) => {
            draft.visualGuide = next;
          }))}
        />
        <ModuleEditor
          label="narrativeHooks"
          value={worldview.narrativeHooks}
          onChange={(next) => props.onWorldviewPatchChange(updateWorldview(worldview, (draft) => {
