...
export function EvidenceEditor(props: EvidenceEditorProps) {
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-gray-50 p-2.5">
      ...
      {missingRequired ? (
        <p className="mt-1 text-[11px] text-red-700">Primary event requires at least 1 evidence ref.</p>
      ) : null}
      <div className="mt-2 space-y-2">
        {props.value.length === 0 ? (
          <p className="text-[11px] text-gray-500">No evidence refs yet.</p>
        ) : props.value.map((item, index) => (
          <div key={`evidence-${index}`} className="rounded-md border border-gray-200 bg-white p-2">
            {props.contextText ? (
              <div className="mb-2 rounded border border-gray-100 bg-gray-50 px-2 py-1">
                <div className="flex flex-wrap items-center gap-2">
                  <button
                  type="button"
                  className="rounded border border-gray-300 bg-white px-2 py-0.5 text-[10px] font-semibold text-gray-700 disabled:opacity-50"
                  disabled={!item.excerpt.trim() || props.contextText.indexOf(item.excerpt.trim()) < 0}
                    onClick={() => {
                ...
                {Number.isFinite(item.offsetStart)
                  ...
                  && item.offsetEnd <= props.contextText.length ? (
                    <pre className="mt-1.5 max-h-20 overflow-auto whitespace-pre-wrap rounded border border-gray-200 bg-white p-1 text-[10px] text-gray-700">
            ...
            <button
              type="button"
              className="mt-2 rounded border border-red-300 bg-white px-2 py-0.5 text-[11px] font-semibold text-red-700"
              onClick={() => {
