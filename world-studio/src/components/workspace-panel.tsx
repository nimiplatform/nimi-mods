...
export function WorkspacePanel(props: {
  ...
  useEffect(() => {
    setWorldOptionId((current) => {
      if (current && worlds.some((item) => item.id === current)) return current;
  ...
  useEffect(() => {
    setDraftOptionId((current) => {
      if (current && drafts.some((item) => item.id === current)) return current;
      return resolveSelectedId(drafts, selectedDraftId);
    });
  }, [drafts, selectedDraftId]);

  const selectedWorld = useMemo(
    () => worlds.find((item) => item.id === worldOptionId) || null,
    [worldOptionId, worlds],
  );
  const selectedDraft = useMemo(
    () => drafts.find((item) => item.id === draftOptionId) || null,
    [draftOptionId, drafts],
  );
