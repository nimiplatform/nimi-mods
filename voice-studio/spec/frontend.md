...
本文档定义 Voice Studio 在 desktop 中的前端架构：组件树、状态管理、Step 间导航、每个 Step 的 UI 结构。
...
└── src/
    ...
    ├── runtime-mod.ts        # createVoiceStudioRuntimeMod()
...
await hookClient.ui.register({
  ...
  extension: {
    type: 'nav-item',
    tabId: 'voice-studio',
    label: t('page.title', 'Voice Studio'),
    icon: 'microphone',
    strategy: 'append',
  },
});
...
const LazyPage = React.lazy(() => import('./voice-studio-page.js'));
await hookClient.ui.register({
...
await registerVoiceStudioDataCapabilities({ hookClient });
```

## 3. 组件树

```
<VoiceStudioPage>                          ← 顶层页面入口
  └─ useVoiceStudioPageController()        ← Controller Hook（编排全部逻辑）
      ├─ useVoiceStudioStore()             ← Zustand 持久状态
      ├─ useVoiceStudioUiState()           ← React 临时 UI 状态
      ├─ useVoiceStudioClients()           ← hookClient + aiClient
      └─ renders:
          <VoiceStudioShell>               ← 外壳布局
...
```typescript
function useVoiceStudioUiState() {
  // 步骤导航
...
```typescript
function useVoiceStudioPageController() {
  // 1. Clients
  const { hookClient, aiClient } = useVoiceStudioClients();
...
**播放引擎 hook**:
```typescript
function usePlaybackEngine(store: VoiceStudioStore) {
...
function computeCanAdvance(step: Step, store: VoiceStudioStore): boolean {
  switch (step) {
...
├── voice-studio-page.tsx                 # 顶层页面入口
│
├── controllers/
│   ├── voice-studio-page-controller.ts   # 主 Controller Hook
│   └── use-voice-studio-ui-state.ts      # 临时 UI 状态
│
├── state/
│   ├── voice-studio-store.ts             # Zustand store
...
│
├── data/
│   └── voice-studio-data-registrar.ts    # Data capability 注册
│
├── locales/
│   ├── en.ts
│   └── zh.ts
...
| 3 | Data capabilities | 注册 `data-api.voice-studio.*` 供其他 mod 查询 |
| 4 | i18n | 注册翻译文件 |
| 5 | 权限声明 | `mod.manifest.yaml` capabilities 列表 |
| 6 | 构建配置 | `tsconfig.build.json` + 打包到 `dist/mods/voice-studio/index.js` |
