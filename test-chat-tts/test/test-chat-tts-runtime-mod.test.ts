...
test('test-chat-tts runtime mod setup registers runtime-aligned UI surfaces', async () => {
  ...
  const mod = createTestChatTtsRuntimeMod();

  try {
    assert.ok(mod.capabilities.includes('runtime.ai.text.generate'));
    assert.ok(mod.capabilities.includes('runtime.media.tts.list.voices'));
    assert.ok(mod.capabilities.includes('runtime.media.tts.synthesize'));
    ...
    assert.equal(typeof getTestChatTtsRuntimeClient().route.listOptions, 'function');
    assert.deepEqual(
      uiRegistrations.map((entry) => entry.slot),
      [TEST_CHAT_TTS_NAV_SLOT, TEST_CHAT_TTS_ROUTE_SLOT],
    );
  } finally {
    restoreHost();
  }
