...
function toIssueMessage(error: unknown): string {
  ...
  const record = error as { issues?: Array<{ path?: Array<string | number>; message?: string }> };
...
function normalizeTurnInput(input: NarrativeTurnInput): NarrativeTurnInputNormalized {
  ...
  return {
    ...
    routeHint: String(input.routeHint || '').trim() || 'chat/default',
    routeOverride: asRecord(input.routeOverride),
