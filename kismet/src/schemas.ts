...
export const KismetDailyFortuneResultSchema = z.object({
  ...
  luckyElements: z.array(ElementKeySchema).min(1).max(2),
  luckyDirections: z.array(z.string().min(1)).min(1).max(3),
  luckyColors: z.array(z.string().min(1)).min(1).max(3),
  luckyNumbers: z.array(z.number().int().min(1).max(9)).min(1).max(3),
