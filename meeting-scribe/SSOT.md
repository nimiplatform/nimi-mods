...
AI 会议助手 mod。上传会议录音，自动转录（含说话人分离）、生成结构化摘要与待办事项。支持 token-api（云端）和 local-only（本地）两种路由模式，满足企业隐私合规场景。
...
| AI (STT) | aiClient.transcribeAudio (Gemini token-api / local Whisper) |
...
- Gemini token-api STT 转录（含说话人分离）
- 一次性结构化分析（摘要 + 决议 + 待办）
